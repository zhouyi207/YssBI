...
describe("Rust-generated node-system golden contracts", () => {
  ...
  it.each([
    [
      ...
      (catalog: Record<string, unknown>) => {
        catalog.compatibility = true;
      },
    ],
    [
      "missing key",
      (catalog: Record<string, unknown>) => {
        deleteKey(catalog, "locale");
      },
    ],
    [
      "number array fingerprint",
      (catalog: Record<string, unknown>) => {
  ...
  it("strictly accepts and rejects every diagnostic location variant", () => {
    const variants = [
      ...
      {
        value: { kind: "connection", connectionId: "contract-connection" },
