...
function backendIpcError(code: string, details: Record<string, unknown> | null = null): IpcError {
  return new IpcError({
    kind: "backend",
...
describe("ProjectService execution contract", () => {
  ...
  it.each([
    ...
    "preserves the backend rejection when the $terminal.type consumer throws",
