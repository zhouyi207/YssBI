...
describe("fetchChartPreview project lifecycle ownership", () => {
  beforeEach(() => {
    vi.restoreAllMocks();
  ...
  it.each(["histogram", "scatter", "line"] as const)(
    ...
    async (chartType) => {
      ...
      if (chartType === "histogram") {
        ...
        vi.spyOn(ChartService, "getPlotColumnPair").mockReturnValue(
          request.promise as Promise<PlotColumnPairPayload>,
        );
      }

      const completion = fetchChartPreview(document(chartType), identity());
      if (chartType === "histogram") {
