...
const forbiddenFeedbackModules = new Set(["sonner", "@/components/ui/sonner", "@/shared/ui/Toast"]);
...
function isLoggerNotifyCall(node: ts.CallExpression): boolean {
  ...
  return (
    ts.isPropertyAccessExpression(notify) &&
...
describe("user feedback architecture contract", () => {
  ...
  it("keeps production feedback out of diagnostic notification and toaster channels", () => {
    ...
    const offenders = withProductionTypeScriptProject(({ sourceFile }) =>
      files.flatMap((path) => feedbackOffenders(path, sourceFile(path))),
    );
