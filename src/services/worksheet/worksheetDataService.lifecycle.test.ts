...
const distribution: ColumnDistribution[] = [
  { columnName: 'amount', kind: 'numeric', bins: [] },
];
  ...
  it.each(['histogram', 'scatter', 'line'] as const)(
    ...
    async (chartType) => {
      ...
      if (chartType === 'histogram') {
        vi.spyOn(DatabaseService, 'getColumnDistribution')
          .mockReturnValue(request.promise as Promise<ColumnDistribution[]>);
      } else {
        vi.spyOn(WorksheetService, 'getPlotColumnPair')
          .mockReturnValue(request.promise as Promise<PlotColumnPairPayload>);
      }

      const completion = fetchWorksheetPreview(document(chartType), identity());
      if (chartType === 'histogram') {
        expect(DatabaseService.getColumnDistribution)
