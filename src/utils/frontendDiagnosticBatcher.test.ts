...
describe('createFrontendDiagnosticBatcher', () => {
  ...
  it('flushes one bounded batch after the maximum delay', async () => {
    ...
    expect(submit.mock.calls[0]?.[0].map((item: FrontendDiagnosticEntryDto) => item.message))
      .toEqual(['one', 'two']);
    batcher.dispose();
  });
  ...
  it('flushes at the batch limit and truncates oversized messages', async () => {
    ...
    expect(new TextEncoder().encode(submit.mock.calls[0]?.[0][0].message).byteLength).toBeLessThanOrEqual(5);
    batcher.dispose();
  });
  ...
  it('keeps only the newest pending entries while a submission is in flight', async () => {
    ...
    first.resolve();
    await batcher.flush();
    expect(submit.mock.calls.slice(1).map((call) => call[0][0].message))
