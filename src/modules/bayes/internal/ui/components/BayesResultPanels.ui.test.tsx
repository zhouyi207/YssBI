...
import { BayesCsvExportButton, BayesResultFolderButton } from "./BayesResultPanels";
...
describe("Bayes result action feedback", () => {
  ...
  it("shows an export IPC failure beside its button", async () => {
    ...
    act(() =>
      root.render(
        <BayesCsvExportButton result={result} kind="posterior_samples" fileName="posterior.csv" />,
      ),
    );
  ...
  it("shows export success locally", async () => {
    ...
    expect(host.querySelector('[role="status"]')?.textContent).toBe(
      "bayes.results.messages.exportSuccess",
    );
  });
});
