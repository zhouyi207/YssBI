...
export function SymbolRoleStep({
  ...
  const [priorDistribution, setPriorDistribution] =
    useState<PriorSpecDTO["distribution"]>("normal");
  ...
  return (
    <Card>
      <CardHeader>
        <PanelTitle title={t("bayes.symbols.title")} issues={issues} />
      </CardHeader>
      <CardContent className="space-y-3">
        <div className="rounded-md border border-border">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead>{t("bayes.symbols.symbol")}</TableHead>
                <TableHead>{t("bayes.symbols.role")}</TableHead>
                <TableHead>{t("bayes.symbols.data")}</TableHead>
                <TableHead>{t("bayes.symbols.column")}</TableHead>
                <TableHead>{t("bayes.symbols.prior")}</TableHead>
                <TableHead>{t("bayes.symbols.bounds")}</TableHead>
                <TableHead className="w-32">{t("bayes.symbols.actions")}</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {symbolsInDisplayOrder(draft.symbols).map((symbol) => (
                <TableRow key={symbol.name}>
                  <TableCell>
                    <LatexInline formulaText={latexSymbol(symbol.name)} />
                  </TableCell>
                  <TableCell>{roleLabel(symbol.role, t)}</TableCell>
                  <TableCell>{dataSourceLabel(draft, symbol.role, datasets, t)}</TableCell>
                  <TableCell className="font-mono">
                    {columnLabel(draft, symbol.name, symbol.role, t)}
                  </TableCell>
                  <TableCell className="font-mono">
                    {priorLabel(draft, symbol.name, symbol.role, t)}
                  </TableCell>
                  <TableCell className="font-mono">{boundsLabel(draft, symbol.name)}</TableCell>
                  <TableCell>
                    <Button size="sm" variant="outline" onClick={() => beginEdit(symbol.name)}>
        ...
        <SymbolConfigDialog
          ...
          onRoleChange={(nextRole) => {
            setRole(nextRole);
            if (editingSymbol)
              setDetailValue(defaultSymbolDetailValue(draft, editingSymbol, nextRole));
          }}
          ...
          onConstraintChange={(nextConstraint) => {
            ...
            if (!isPriorCompatibleWithConstraint(priorDistribution, nextConstraint)) {
              const nextPrior = defaultPriorForConstraint(
                nextConstraint,
                editingSymbol ?? "parameter",
              );
...
function symbolsInDisplayOrder(
  symbols: readonly BayesModelDraftDTO["symbols"][number][],
): BayesModelDraftDTO["symbols"] {
  const roleOrder: Record<BayesSymbolRoleDTO, number> = {
    dependent: 0,
    independent: 1,
    parameter: 2,
  };
  return symbols
    ...
    .sort(
      (left, right) =>
        roleOrder[left.symbol.role] - roleOrder[right.symbol.role] || left.index - right.index,
    )
    .map(({ symbol }) => symbol);
}
...
function symbolDetailValue(
  draft: BayesModelDraftDTO,
  name: string,
  role: BayesSymbolRoleDTO,
): string {
  if (role === "dependent")
    return draft.responseBinding?.symbol === name ? draft.responseBinding.column : "";
  if (role === "independent") return draft.dataBindings[name] ?? "";
  return (
    draft.parameters.find((parameter) => parameter.name === name)?.prior.distribution ?? "normal"
  );
}

function defaultSymbolDetailValue(
  draft: BayesModelDraftDTO,
  name: string,
  role: BayesSymbolRoleDTO,
): string {
...
function dataSourceLabel(
  ...
  return (
    datasets.find((dataset) => dataset.sourceId === draft.dataset?.sourceId)?.displayName ??
    t("bayes.dataBinding.unknownDataSource")
  );
}
