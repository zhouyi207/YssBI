...
export function NodeInspectPanel({ graphPath, nodeId }: { graphPath: string; nodeId: string }) {
  ...
  const projectedNode = useGraphRead(
    (snapshot) => snapshot.graphEntities[graphPath]?.nodes[nodeId],
  );
  const node = projectedNode ? (structuredClone(projectedNode) as unknown as NodeData) : undefined;
