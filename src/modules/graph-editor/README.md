...
每个画布实例持有自己的连线反馈 store，Context 只传稳定的 store 引用。节点外观、Pin 外观和连线分别按节点 ID、Pin ID 或连线 ID 订阅淡化、高亮、目标和替换反馈；节点正文及参数控件不订阅全局 active Pin。反馈复用当前图投影的方向、类型和容量校验，在 layout effect 中发布，鼠标移动仅使目标或连线路径变化的视图更新。选择及拖拽预览继续归所属物理面板所有。
