...
describe("NodePalette", () => {
  ...
  it("renders child categories beneath their parent in declared order", () => {
    ...
    expect(
      host.querySelector('[data-catalog-item-key="item:static:statistics.logit.fit"]')?.textContent,
    ).toContain("逻辑回归");
  });
  ...
  it("selects the Rust-issued descriptor with its catalog locale", () => {
    ...
    const item = Array.from(host.querySelectorAll("button")).find((button) =>
      button.textContent?.includes("加法"),
    );
  ...
  it("keeps same-type resources distinct across search, refresh, and locale changes", () => {
    ...
    catalogState.current = {
      ...
      searchIndex: getLocalizedSearchIndex(multiCatalog),
      refresh: vi.fn(),
    };
    renderPalette();
    const input = host.querySelector("input")!;
    ...
    const refreshed = {
      ...
      items: multiCatalog.items.map((item) =>
        item.resourcePath === second.resourcePath ? { ...item, title: "刷新 Other" } : item,
      ),
    };
    catalogState.current = {
      status: "ready",
      error: null,
      catalog: refreshed,
      searchIndex: getLocalizedSearchIndex(refreshed),
      refresh: vi.fn(),
    };
    act(() =>
      root.render(
