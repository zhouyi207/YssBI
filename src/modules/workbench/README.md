...
工具栏按操作范围组织：主区分组显示“浮动整个分组”及原生最大化/还原；浮窗显示“停靠整个浮窗”、浮动窗口操作菜单及原生最大化/还原。窗口级入口通过原生 `onRenderTabSet` 放入浮窗首个分组，分组最大化时放入该最大化分组。图标统一为 14px，工具栏按钮高度为 24px；原生标题拖动、停靠拖柄、标签溢出和双击最大化沿用库实现。

原生“Float selected tab”工具栏图标在新建和恢复布局时统一关闭；单标签浮动留在标签右键菜单。浮窗内仅有多个标签时提供“将此标签分离为新浮窗”，多个分组且目标分组含多个标签时另提供分离分组；不显示把唯一内容再次浮动的重复入口。整个浮窗的关闭放入明确命名的窗口菜单，与标签自身的关闭按钮区分。停靠保留内部布局并放回主区；关闭将浮窗全部 tab 一次提交给现有协调者，取消未保存确认时不物理删除任何 tab。原生批量删除 Action 也进入该协调者。所有定制使用公开属性和插槽，不扩展依赖 API。
