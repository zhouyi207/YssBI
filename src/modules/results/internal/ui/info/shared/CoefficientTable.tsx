              ...
              <TableHead className={`${infoStatsHeadCompactClass} cursor-help`}>
                <Tooltip>
                  ...
                  <TooltipContent side="top" className="max-w-xs">
                    exp(β)。变量系数：x 每增加 1 单位，几率变为原来的 exp(β) 倍；常数项：当所有 x 为
                    0 时 y=1 的基准几率
                  </TooltipContent>
                </Tooltip>
              </TableHead>
        ...
        <TableBody>
          {coefficients.map((coeff, idx) => {
            ...
            return (
              ...
              >
                ...
                {oddsTooltip != null && (
                  <TableCell
                    className={`${infoStatsCellRightClass} text-amber-700/90 dark:text-amber-300/90`}
                  >
                    <Tooltip>
                      <TooltipTrigger asChild>
          ...
          {ar1Rho != null && (
            <TableRow className={infoStatsRowOddClass}>
              ...
              {hasCategorical && (
                <TableCell className={`${infoStatsCellCompactClass} text-muted-foreground`}>
                  —
                </TableCell>
              )}
              <TableCell className={`${infoStatsCellRightClass} text-foreground`}>
