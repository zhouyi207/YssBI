...
function EmphasisNumber({
  ...
  const flex =
    align === "right" ? "flex items-center justify-end gap-2" : "flex items-center gap-2";
  return (
    <div className={flex}>
...
function extremumRowIndices(nums: (number | null | undefined)[], mode: "min" | "max"): Set<number> {
  ...
  const m =
    mode === "max" ? Math.max(...pairs.map((p) => p.v)) : Math.min(...pairs.map((p) => p.v));
...
export const VARSocComponent: FC<{ data: VARSocResultData }> = ({ data }) => {
  ...
  const highlight = useMemo(
    () => ({
      ...
      lr: extremumRowIndices(
        rows.map((r) => r.lr),
        "max",
      ),
      lr_df: extremumRowIndices(
        rows.map((r) => r.lr_df),
        "min",
      ),
      lr_p: extremumRowIndices(
        rows.map((r) => r.lr_p),
        "min",
      ),
      fpe: extremumRowIndices(
        rows.map((r) => r.fpe),
        "min",
      ),
      aic: extremumRowIndices(
        rows.map((r) => r.aic),
        "min",
      ),
      hqic: extremumRowIndices(
        rows.map((r) => r.hqic),
        "min",
      ),
      sbic: extremumRowIndices(
        rows.map((r) => r.sbic),
        "min",
      ),
    }),
    ...
    <ReportLayout
      ...
      subtitle={
        <p className="text-xs text-muted-foreground">
          Endogenous: {var_names.join(", ")} · table lag 0…{maxlag} · n={num_observation} (common
          sample T−maxlag, Stata <code className="text-muted-foreground">varsoc</code>)
        </p>
      }
    >
      ...
      <p className="mt-2 px-1 text-[10px] text-muted-foreground">
        ...
        加亮：FPE、AIC、HQIC、SBIC、P 取列内最小（同 Stata varsoc 信息准则 *）；LL、LR
        取列内最大；df 取列内最小；Lag 不标。并列则多行同标。
      </p>
    </ReportLayout>
  );
};
