export const zhCN = {
  tableComposition: {
    by_name: "按列名对齐（缺失列补空值）",
    by_position: "按列位置对齐",
    inner: "内连接",
    left: "左连接",
    right: "右连接",
    full: "全外连接",
  },
  conversion: {
    categorical: "分类",
    ordinal: "顺序",
    datetime: "日期时间",
    identifier: "标识",
    date: "日期",
    time: "时间",
    seconds: "秒",
    milliseconds: "毫秒",
    microseconds: "微秒",
    nanoseconds: "纳秒",
    domainHelp: "填写原始值及标签。顺序类型按行从低到高；留空继承源值域。",
    code: "原始值",
    label: "标签",
    addValue: "添加值",
    removeValue: "删除值",
    moveUp: "上移",
    moveDown: "下移",
    applyDomain: "应用值域",
    positiveValue: "二元正值（用于二元映射时必选）",
    previousValues: "上一页",
    nextValues: "下一页",
    duplicateValues: "原始值不能重复。",
    unspecified: "未指定",
    numeric: "数值",
    text: "文本",
    binary: "二元",
    auto: "自动",
    integer: "整数",
    real: "实数",
  },
  reportLayout: {
    title: "报告布局",
    sessionOnly: "调整章节顺序和显示内容。布局仅用于当前报告视图，重新打开后恢复默认。",
    moveUp: "上移{{section}}",
    moveDown: "下移{{section}}",
    json: "导入或导出布局",
    jsonHelp: "载入当前布局后可复制或编辑 JSON。导入的布局必须引用当前报告的结果。",
    export: "载入当前布局",
    apply: "应用布局",
    sections: {
      equation: "回归方程",
      modelSummary: "模型概览",
      anova: "方差分析",
      coefficientTable: "系数表",
      hypothesisTest: "假设检验",
      diagnostics: "诊断概览",
      residualPlot: "残差图",
      observations: "拟合值和残差表",
      acfPacf: "自相关与偏自相关",
      serialTests: "序列相关检验",
    },
    errors: {
      invalidJson: "JSON 格式有误，报告保留原布局。",
      tooLarge: "布局过大或章节过多（{{path}}），报告保留原布局。",
      invalidShape: "布局包含无效字段或结构（{{path}}），报告保留原布局。",
      unsupportedVersion: "不支持此布局版本，报告保留原布局。",
      sourceMismatch: "布局引用的结果与当前报告不符，报告保留原布局。",
      unknownSection: "不支持此章节类型（{{path}}），报告保留原布局。",
      invalidId: "章节标识须为字母开头的字母、数字、下划线或连字符，最长 64 字符（{{path}}）。",
      duplicateSection: "章节类型或标识重复（{{path}}），报告保留原布局。",
    },
  },
  common: {
    loading: "加载中...",
    initializing: "正在初始化...",
    ready: "就绪",
    idle: "空闲",
    error: "错误",
    running: "运行中",
    completed: "已完成",
    restoreDefaults: "恢复默认",
    restoreAllDefaults: "恢复所有默认设置",
    restoring: "恢复中...",
    minimize: "最小化",
    maximize: "最大化",
    restore: "还原",
    close: "关闭",
    refresh: "刷新",
    retry: "重试",
    save: "保存",
    export: "导出",
    reset: "重置",
    undo: "撤销",
    redo: "重做",
    delete: "删除",
    search: "搜索",
    cancel: "取消",
    confirm: "确定",
    incidentId: "事件编号",
    errorCode: "错误代码",
    unexpectedError: "发生意外错误",
    filePublicationUncertain:
      "目标文件可能已更新，但无法确认保存完成。请检查目标文件后再决定是否重试。",
  },
  notifications: {
    databaseEditor: {
      saveFailed: "保存更改失败：{{error}}",
      forceCastFailed: "强制转换列类型失败：{{error}}",
      pasteFailed: "粘贴未能完成（已应用 {{appliedCount}} 个单元格）：{{error}}",
    },
    editor: {
      chartSaveFailed: "保存图表失败：{{error}}",
      chartDeleteFailed: "删除图表失败：{{error}}",
      graphSaveFailed: "保存图失败：{{error}}",
      documentSaveFailed: "保存「{{title}}」失败：{{error}}",
      resourceCatalogRefreshing: "资源目录已过期，正在刷新后再创建节点",
    },
    parameter: {
      updateFailed: "更新参数失败：{{error}}",
      stale: "编辑已过期，已恢复最新值",
      conflict: "编辑与较新的值冲突，已恢复最新值",
      rejected: "编辑被拒绝（{{code}}），已恢复最新值",
      enterNumber: "请输入数字",
      enterFiniteNumber: "请输入有限数值",
      enterInteger: "请输入整数",
      enterSupportedInteger: "请输入支持范围内的整数",
      unsupportedNumericType: "不支持的数值类型",
    },
    project: {
      notLoaded: "项目尚未加载",
      saveAsFailed: "另存为失败：{{error}}",
      openResourceBeforeSaving: "请先打开一个图或图表",
      saveFailed: "保存失败：{{error}}",
      problemsBlockExecution: "请先解决图中的阻断问题再运行",
      loadFailed: "加载项目失败",
    },
    chart: {
      createFailed: "创建图表失败：{{error}}",
      duplicateFailed: "复制图表失败：{{error}}",
    },
    projectPicker: {
      refreshFailed: "刷新项目列表失败：{{error}}",
      scanFailed: "扫描项目失败：{{error}}",
      cleanupFailed: "清理无效项目失败：{{error}}",
      createRecovery: "项目 {{name}} 需要恢复：{{outcome}}",
      openFailed: "打开项目失败：{{error}}",
      importFailed: "导入项目失败：{{error}}",
      removeFailed: "从列表中移除项目失败：{{error}}",
      deleteRecovery: "项目文件已删除，但需要恢复操作：{{outcome}}",
      favoriteFailed: "更新收藏状态失败：{{error}}",
    },
    externalUrl: {
      openFailed: "无法打开外部链接。",
    },
    julia: {
      runtime: {
        title: "Julia 运行时",
        description: "Julia 计算环境与工作进程状态",
        openBayes: "打开贝叶斯分析",
      },
      statusFailed: "获取 Julia 状态失败：{{error}}",
      installFailed: "安装 Julia 失败：{{error}}",
    },
    newProject: {
      defaultPathFailed: "获取默认项目位置失败：{{error}}",
      browseFailed: "选择项目位置失败：{{error}}",
      createFailed: "创建项目失败：{{error}}",
    },
    sidebar: {
      actionFailed: "侧边栏操作失败：{{error}}",
    },
  },
  editor: {
    initializationFailed: "初始化失败: {{error}}",
    close: {
      dirtyTitle: "保存更改？",
      dirtyMessage: "“{{name}}”有未保存的更改。",
      save: "保存",
      discard: "放弃更改",
      cancel: "取消",
      failed: "无法关闭面板。",
    },
  },
  panel: {
    primarySideBar: "主侧栏",
    assistant: "助手",
    assistantEmptyTitle: "统计分析助手",
    assistantEmptyDescription: "围绕研究问题、数据质量、方法假设与诊断开展可复现分析。",
    assistantComposerLabel: "Assistant 消息草稿",
    assistantComposerPlaceholder: "输入消息草稿…",
    assistantSendUnavailable: "配置模型提供方后即可发送。",
    assistantSend: "发送消息",
    assistantCopy: "复制回复",
    assistantCopied: "已复制",
    assistantCopyCode: "复制代码",
    assistantCopyFailed: "复制失败，请重试",
    assistantCode: "代码",
    assistantSource: "参考来源",
    assistantTool: "工具调用",
    assistantScrollToBottom: "回到最新消息",
    assistantCancel: "停止当前分析",
    assistantStatusInitializing: "正在连接 Harness…",
    assistantStatusProviderUnavailable: "请在设置 → AI 中配置 OpenAI",
    assistantStatusReady: "Ctrl+Enter 发送",
    assistantStatusRunning: "正在分析…",
    assistantStatusError: "Assistant 暂时不可用",
    assistantErrors: {
      assistant_authentication_failed: "认证失败，请检查 API Key 及模型访问权限。",
      assistant_rate_limited: "服务已限流或额度不足，请检查额度后稍后重试。",
      assistant_provider_request_rejected: "服务拒绝了请求，请检查模型、API 地址及接口兼容性。",
      assistant_provider_connection_failed: "无法连接模型服务，请检查网络、代理和 API 地址后重试。",
      assistant_provider_unavailable: "模型服务暂时不可用，请稍后重新发送。",
      assistant_invalid_provider_response: "模型服务返回了无法解析的响应，请检查接口兼容性后重试。",
      assistant_provider_configuration_invalid: "模型配置无效，请检查模型名称和 API 地址。",
      assistant_turn_failed: "本次分析失败，可以重新发送消息。",
      assistant_turn_timed_out: "本次分析超时，可以重新发送消息。",
      assistant_stream_failed: "助手消息同步失败，请重新打开助手面板。",
      assistant_session_failed: "无法建立助手会话，请重新打开助手面板。",
      project_session_unavailable: "项目尚未就绪，请打开项目后重试。",
      harness_session_not_active: "助手会话已失效，请重新打开助手面板。",
    },
    assistantActivity: "正在执行：{{activity}}",
    assistantPlan: "统计分析计划",
    assistantPlanMode: "分析模式",
    assistantPlanWorkflow: "工作流",
    assistantMemoryCount: "记忆 {{count}}",
    assistantMemoryTitle: "当前会话记忆",
    assistantMemoryDelete: "删除记忆",
    assistantToolRunning: "执行中",
    assistantToolsRunning: "工具调用 · {{completed}}/{{count}}",
    assistantToolsFinished: "工具调用结束 · {{count}} 项",
    assistantToolsFailed: "{{count}} 项异常",
    assistantToolArguments: "调用参数",
    assistantToolArgumentsUnavailable: "此调用记录未提供参数",
    assistantToolNames: {
      inspect_project: "检查项目",
      inspect_graph: "检查图",
      search_node_catalog: "搜索节点",
      inspect_dataset_schema: "检查数据结构",
      inspect_dataset_profile: "检查数据质量",
      inspect_result: "查看结果",
      apply_graph_edit: "编辑图",
      validate_graph: "校验图",
      execute_graph: "运行图",
      list_graph_results: "查询图结果",
      save_graph: "保存图",
    },
    assistantToolCompleted: "已完成",
    assistantToolFailed: "失败",
    assistantToolCancelled: "已取消",
    assistantToolTimedOut: "已超时",
    assistantToolInterrupted: "已中断",
    assistantToolUnknown: "状态待确认",
    details: "详细信息",
    inspect: "检查",
    logs: "日志",
    output: "输出",
    problems: "问题",
    problemsLocate: "定位问题",
    problemsRelated: "关联位置",
    problemsBlocking: "阻止运行",
    outputNoGraph: "请打开图以查看程序输出",
    outputEmpty: "当前图暂无运行错误",
    outputClear: "清除运行错误",
    problemsNoGraph: "请打开图以查看问题",
    problemsEmpty: "当前图没有问题",
    problemsCount: "{{count}} 个问题",
    problemsLocateNode: "定位问题节点",
  },
  resultSource: {
    readFailed: "无法读取结果",
  },
  runFailure: {
    title: "执行失败",
    unknown: "本次运行未能完成。请根据错误代码和诊断编号检查日志。",
    node: "定位节点：{{name}}",
    run: "运行 #{{id}}",
    code: "错误代码：{{code}}",
    incident: "诊断编号：{{id}}",
    causes: {
      divisionByZero: "除数为 0。请检查除法节点的右侧输入及其上游值。",
      nonFiniteResult: "计算产生了非有限结果（NaN 或无穷大）。请检查输入范围和数值溢出。",
      invalidNumericInput: "数值运算收到不兼容的输入类型。请检查该节点的输入及上游常量。",
      kernelNotFound: "当前运行时尚未提供该节点的计算实现。",
      kernelFailed: "节点计算失败。请检查节点配置，并使用诊断编号查看技术详情。",
      deadlineExceeded: "运行超过了允许的时间。请缩小计算范围后重试。",
      resourceUnavailable: "运行所需的资源不可用。请检查数据源和项目资源。",
      finalizationFailed: "计算结果未能完成提交。请检查项目状态后重试。",
      graph_draft_changed: "图或依赖资源已改变。请在当前图状态下重新运行。",
      graph_not_ready: "请先解决图中的阻断问题再运行。",
      graph_resolution_failed: "无法完成图检查。请使用诊断编号查看技术详情。",
      graph_plan_failed: "无法准备执行计划。请使用诊断编号查看技术详情。",
      stale_project_lifecycle: "项目状态已改变。请在当前项目中重新运行。",
      execution_channel_failed: "运行事件传输失败，无法确认完整结果。请重试并检查日志。",
      ipc_transport_failure: "无法完成与运行后端的通信。请检查应用状态后重试。",
    },
    phases: {
      admission: "阶段：启动运行",
      planValidation: "阶段：执行计划校验",
      resourcePreparation: "阶段：准备资源",
      execution: "阶段：节点计算",
      finalization: "阶段：提交结果",
    },
  },
  nodeCatalog: {
    loadError: "节点目录暂不可用",
    invalidDraft: "当前图草稿无法用于筛选节点",
    invalidSource: "所选 Pin 无法用于创建连接",
  },
  /** 编辑器内右键上下文菜单及由菜单触发的输入框 */
  contextMenu: {
    sidebar: {
      open: "打开",
      rename: "重命名",
      duplicate: "复制",
      delete: "删除",
      newChart: "新建图表",
      importData: "导入数据",
      revealInExplorer: "在资源管理器中打开",
      revealInExplorerFailed: "无法在资源管理器中打开：{{error}}",
    },
    dialog: {
      createSubmit: "创建",
      renameGraphTitle: "重命名",
      renameChartTitle: "重命名图表",
      renameDataTitle: "重命名数据集",
      renameSubmit: "重命名",
    },
    connection: {
      breakLink: "断开连接",
      breakSelectedLinks: "断开所选连接",
      delete: "删除",
    },
    node: {
      copy: "复制",
      cut: "剪切",
      duplicate: "创建副本",
      breakAllLinks: "断开全部连接",
      selectLinkedNodes: "选择已连接节点",
      delete: "删除",
    },
    pin: {
      breakLinks: "断开连接",
      resetValue: "恢复默认值",
      view: "查看",
      viewDisabledNoRun: "请先运行图以生成可查看结果",
      viewDisabledNoUpstream: "无可用上游结果",
    },
  },
  projectPicker: {
    title: "项目管理",
    heading: "选择一个 YssBI 项目",
    description: "从最近项目继续，或新建 / 打开一个 metadata.yssbi 项目。",
    backToEditor: "返回编辑器",
    newProject: "新建项目",
    openProject: "打开项目",
    importProject: "导入项目",
    importing: "正在导入项目…",
    importSuccess: "已添加项目「{{name}}」到列表",
    createSuccess: "已创建项目「{{name}}」并加入列表",
    scanProjects: "扫描项目",
    scanFolderTitle: "选择要扫描的文件夹",
    scanning: "正在扫描项目…",
    scanNoneFound: "所选文件夹中未发现 YssBI 项目",
    scanSuccess: "发现 {{found}} 个项目，新增 {{added}} 个到列表",
    scanAlreadyRegistered: "发现 {{found}} 个项目，均已在列表中",
    scanCancelled: "扫描已取消",
    cleanupProjects: "清理项目",
    cleaningUp: "正在清理项目…",
    cleanupNone: "没有失效的项目",
    cleanupSuccess: "已从列表移除 {{count}} 个失效项目",
    creating: "正在新建...",
    opening: "正在打开...",
    searchPlaceholder: "筛选项目...",
    clearSearch: "清空筛选",
    sortLabel: "排序",
    sortRecent: "最近打开",
    sortName: "名称",
    emptyTitle: "暂无最近项目",
    emptyDescription: "打开现有 metadata.yssbi 项目或新建空项目后，它会出现在这里。",
    noMatchesTitle: "没有匹配的项目",
    noMatchesDescription: "尝试调整筛选关键词。",
    currentBadge: "当前",
    actionsTitle: "项目操作",
    noSelection: "选择左侧项目后可进入或管理",
    enter: "进入项目",
    favorite: "收藏项目",
    unfavorite: "取消收藏项目",
    removeFromList: "从列表移除",
    deleteProjectFiles: "移动到回收站",
    deleteProjectConfirm: {
      title: "将项目移到回收站？",
      description: "「{{name}}」的所有文件和文件夹将移到回收站，之后可从回收站恢复。",
      pathLabel: "项目路径",
      confirm: "移动到回收站",
      deleting: "正在移动…",
      success: "项目已移到回收站",
      failed: "移到回收站失败",
    },
    issues: {
      retry: "重试",
      staleTitle: "项目上下文已变化",
      stale: "该操作属于旧的项目上下文，已被安全忽略。",
      errors: {
        unknown: "无法完成该操作。",
        invalidPath: "所选路径不是有效的 YssBI 项目位置。",
        projectNotFound: "找不到该项目，它可能已被移动或删除。",
        invalidProject: "所选项目元数据无效或已损坏。",
        loadFailed: "无法加载该项目。",
        busy: "另一个项目操作仍在进行，请稍后重试。",
        recoveryRequired: "该项目需要先完成恢复才能继续操作。",
        filesystem: "无法安全地更新项目文件。",
        transport: "无法连接应用后端。",
        unexpectedResponse: "后端返回了无法识别的响应。",
        internal: "发生内部错误，报告问题时请提供事件编号。",
      },
      recovery: {
        registerDestination: "项目已创建，但无法登记新的项目位置。",
        activateDestination: "项目已创建，但无法激活新的项目位置。",
        cleanupRegistry: "项目文件已变更，但项目列表仍需要清理。",
        removeRegistryRecord: "项目文件已移除，但无法删除项目列表中的记录。",
        unknown: "操作只完成了一部分，需要执行恢复。",
      },
    },
    localOnlyHint: "当前列表保存在本机应用数据目录，不会删除磁盘上的项目文件。",
    settingsDescription: "调整项目选择界面的偏好设置。",
    loading: {
      creating: "正在新建项目",
      opening: "正在打开项目",
      scanning: "正在扫描项目",
      scanningFolder: "正在搜索 metadata.yssbi…",
      scanFoundProjects: "找到 {{count}} 个项目",
      scanRegistering: "正在注册项目（{{current}} / {{total}}）…",
      cleanup: "正在清理失效项目",
      cleanupChecking: "正在检查项目（{{current}} / {{total}}）…",
      cleanupRemoving: "已移除 {{removed}} 个失效项目…",
      cleanupCancelled: "清理已取消",
      readingFile: "正在读取项目文件…",
      loadingData: "正在加载项目数据…",
      preparingEditor: "正在准备编辑器…",
      done: "完成",
    },
    newProjectModal: {
      title: "新建项目",
      nameLabel: "项目名",
      pathLabel: "项目文件夹路径",
      browse: "浏览",
      browseTitle: "选择项目所在文件夹",
      create: "创建",
      validating: "正在校验路径...",
      nameRequired: "请填写项目名称。",
      pathRequired: "请填写项目文件夹路径。",
      invalidPath: "项目路径无效。",
      fixErrors: "提交前校验未通过，请查看标红项。",
    },
  },
  language: {
    zhCN: "简体中文",
    enUS: "English",
  },
  menubar: {
    file: "文件",
    edit: "编辑",
    data: "数据",
    window: "窗口",
    view: "视图",
    tools: "工具",
    help: "帮助",
    newEventGraph: "新建事件",
    newFunctionGraph: "新建函数",
    newChart: "新建图表",
    openProject: "打开项目...",
    closeProject: "关闭项目",
    saveProjectAs: "项目另存为...",
    cut: "剪切",
    copy: "复制",
    paste: "粘贴",
    importData: "导入数据",
    databaseEditor: "数据库编辑器",
    schemaViewer: "Schema 查看器",
    splitEditorRight: "向右拆分编辑器",
    splitEditorDown: "向下拆分编辑器",
    openLogsInNewWindow: "在新窗口打开日志",
    resetLayout: "重置布局",
    debugger: "调试器",
    profiler: "性能分析器",
    settings: "设置",
    documentation: "文档",
    architecture: "架构",
    releaseNotes: "发行说明",
    githubRepository: "GitHub 仓库",
    reportIssue: "反馈问题",
    about: "关于",
    switchToDark: "切换为深色主题",
    switchToLight: "切换为浅色主题",
  },
  architectureModal: {
    frontendPage: "前端 React",
    openFrontend: "展开前端架构",
    frontendView: {
      summary: "{{count}} 个前端子系统",
      caption:
        "按 frontend.md 展示能力、状态归属与代表协作。点击节点底部查看职责和实现位置；JSON 页面布局模板尚处于规划阶段，子系统划分不表示目录迁移已完成。",
      businessKind: "业务子系统",
      hostKind: "应用与展示宿主",
      transportKind: "通信适配",
      supportKind: "通用支撑",
      locations: "{{count}} 处实现",
      inspect: "查看{{name}}的职责与代表实现",
      representativeLocations: "代表实现位置",
      workbench: {
        title: "应用与工作台",
        description: "组装窗口、菜单与面板，协调布局、编辑器关闭和项目切换生命周期。",
        parts: {
          composition: "应用组装",
          registry: "面板注册",
          layout: "根 FlexLayout",
          templates: "JSON 模板 · 规划中",
        },
        boundary:
          "app 通过模块 public.ts 组合能力，Application 编排用例并使用 core 投影、domain 规则和 service。根 FlexLayout 独占面板拓扑、位置、顺序、尺寸与折叠状态。Details 等共享宿主使用对应业务的数据和操作，不拥有图草稿、数据或结果生命周期。关闭编辑器继续经过保存、放弃或取消流程。",
        json: {
          template: {
            title: "页面布局模板 · 尚未实现",
            description:
              "目标方案由 JSON 声明页面分区、组件标识和默认布局，宿主校验注册引用并组合组件，再通过既有布局操作安装面板。模板不保存业务状态，也不镜像实时布局；当前默认布局仍由 TypeScript 命令建立。",
          },
          document: {
            title: "业务展示文档 · 已有参考实现",
            description:
              "ActivityPanelDocument 由 Rust 业务 owner 生成，ActivityPanelDocumentView 消费文档、展开状态与有限回调。四个内置 Activity 面板已接入，展示文档与页面布局模板职责不同。",
          },
          snapshot: {
            title: "运行布局快照 · 已有持久化",
            description:
              "从根 FlexLayout 序列化用户调整后的布局；启动仅在空根实例上使用 fromJSON。目标模板方案优先恢复有效快照，无有效快照时建立默认布局，重置由明确操作触发。",
          },
        },
      },
      project: {
        title: "项目与资源交互",
        description: "组织项目打开与切换、资源展示和操作，协调发布结果的检查与采用。",
        parts: {
          lifecycle: "项目生命周期",
          resources: "资源投影",
          operations: "资源操作",
          publication: "发布结果采用",
        },
        boundary:
          "Rust 拥有已提交项目、资源身份与版本；React 保存资源索引和 Activity 文档的显示投影。资源路径是不透明标识。异步结果采用前检查项目身份与发布版本，按各 owner 协调图、数据、图表和工作台生命周期，资源树不直接清空其他子系统内部状态。",
      },
      graph: {
        title: "图编辑与运行交互",
        description: "管理图草稿与画布交互，组织编辑解析、保存、运行并展示语义投影和结果。",
        parts: {
          draft: "草稿与历史",
          projection: "语义投影",
          canvas: "画布交互",
          execution: "校验 / 保存 / 执行",
        },
        boundary:
          "草稿、保存基线与画布临时状态分别管理。Application 协调操作顺序并校验项目、草稿和请求身份；Canvas、Details 与 Problems 消费同一份 Rust GraphSemanticSnapshot。编辑时解析诊断，运行时内部准备计划，保存保持独立且失败后保留草稿。结果与租约由 Rust 管理；Problems、Results 和运行失败各有数据流。",
      },
      data: {
        title: "数据浏览与编辑",
        description: "呈现数据资源、分页和筛选输入，组织编辑、导入导出与查询生命周期。",
        parts: {
          browsing: "资源浏览",
          paging: "按需分页",
          exchange: "编辑与交换",
          lifecycle: "查询生命周期",
        },
        boundary:
          "查询、筛选、聚合、编辑和持久化由 Rust 完成，前端按需获取页面或摘要。当前工作台数据标签只读，没有本地文档草稿，不套用图编辑器的 dirty、Save 与历史模型。工作台和独立窗口复用数据库投影；关闭面板只释放局部查看状态，不清除其他消费者仍使用的投影。",
      },
      chart: {
        title: "图表编辑与展示",
        description: "管理图表文档、配置和数据绑定，将后端数据投影渲染为可视化。",
        parts: {
          document: "图表文档",
          binding: "配置与绑定",
          rendering: "图表渲染",
          lifecycle: "资源生命周期",
        },
        boundary:
          "Rust 管理已提交图表文档，React 分别维护未提交编辑内容、dirty 和显示状态。core/chart、application/chart 与编辑器生命周期协作完成加载、修改、提交和关闭。查询与统计事实来自后端，可复用绘图组件不拥有图表资源或执行结果，也不重新计算领域结果。",
      },
      assistant: {
        title: "Assistant 对话与工具",
        description: "展示会话、输入和审批，消费 Harness 快照与有序事件并衔接业务操作。",
        parts: {
          input: "输入与取消",
          projection: "会话投影",
          approval: "工具与审批",
          coordination: "业务操作衔接",
        },
        boundary:
          "Rust Harness 拥有 session、turn、工具流程、审批和事件顺序。React 保存界面投影与本地输入。Assistant 对图草稿的操作进入已有草稿协调流程，不建立独立 AI 草稿或项目模型。",
      },
      plugins: {
        title: "插件交互与接入",
        description: "提供插件管理入口、状态与任务反馈，将界面贡献交给应用和工作台接入。",
        parts: { management: "安装与启停", projection: "插件状态投影", contributions: "界面贡献" },
        boundary:
          "Rust Plugin Manager 管理安装状态、进程与任务，前端消费相应投影。工作台管理插件面板位置，插件承担专属业务。宿主页面模板与插件自带页面遵守各自渲染边界，目标能力与当前接入范围分别说明。",
      },
      feedback: {
        title: "日志与运行反馈",
        description: "查看有界日志、筛选定位与订阅状态，将稳定错误码转为本地化操作反馈。",
        parts: { logs: "Logs 查看", subscription: "订阅生命周期", localization: "错误本地化" },
        boundary:
          "日志是观察信息，不用于重建业务状态。Graph Problems、Results、Run Output、Assistant 对话与 Logs 保留各自的数据流。订阅按 owner 协调暂停、恢复和释放；具体操作决定错误与反馈的呈现时机。",
      },
      communication: {
        title: "前端通信适配",
        description: "通过 invokeCommand 访问 Rust，解析 DTO 与错误并封装 Event / Channel 订阅。",
        parts: { invoke: "请求适配", parsing: "响应解析", streams: "通知与消息流" },
        boundary:
          "界面操作进入对应 Application 用例，再经 service 调用 Tauri IPC。Service 负责传输与解析，Application 检查项目、草稿或请求身份后采用响应并发布状态。业务 owner 决定订阅生命周期，适配层提供创建与释放。错误保持 { code, details, incidentId }，用户提示由前端本地化。",
      },
      support: {
        title: "客户端设置与通用支撑",
        description: "提供本地偏好、主题、国际化、通用展示组件与桌面平台适配。",
        parts: { preferences: "客户端偏好", theme: "主题", i18n: "国际化", components: "通用组件" },
        boundary:
          "偏好由设置 owner 按用途持久化并跨窗口传播，与项目配置和插件状态分开。通用组件通过输入和回调服务业务，不吸收调用方规则；不依赖 UI/framework 的前端规则归 features/domain，不能反向依赖 React、Tauri 或 service。模板选择和应用仍归应用与工作台。",
      },
      links: {
        shared: "界面与偏好支撑",
        contributions: "界面贡献",
        resourceHost: "资源面板宿主",
        assistantHost: "Assistant 宿主",
        chartResource: "图表资源生命周期",
        openGraph: "打开与切换",
        draftCoordination: "草稿操作协调",
        request: "Application → service",
        adoption: "校验身份并采用",
        dataRequest: "分页与编辑请求",
        logSubscription: "日志订阅",
      },
    },
    breadcrumb: "架构导航",
    backendPage: "后端 Rust",
    openBackend: "展开后端架构",
    communicationPage: "Tauri IPC",
    openCommunication: "查看通信架构",
    communicationView: {
      summary: "Command · Event · Channel · DTO",
      caption:
        "按 communication.md 展示请求、通知与消息流。沿箭头查看交付方向，点击卡片底部查看契约、去重规则和生命周期。",
      frontendKind: "前端",
      commandKind: "Command · 请求响应",
      rustKind: "Rust · 业务权威",
      contractKind: "DTO · 错误契约",
      eventKind: "Event · 变化通知",
      channelKind: "Channel · 有序流",
      details: "契约详情",
      inspect: "查看{{name}}的通信契约",
      service: {
        title: "前端 Service",
        description: "封装请求参数、解析响应，并检查结果所属身份是否仍然有效。",
        parts: { parameters: "参数封装", parsing: "响应解析", identity: "身份校验" },
        boundary:
          "src/services/ 是前端通信入口，通过 invokeCommand 请求后端。结构解析成功后，业务调用方仍需检查响应身份是否匹配当前项目、任务或会话。React 消费 Rust 投影，不拥有第二份已提交业务状态。",
      },
      command: {
        title: "invokeCommand",
        description: "传递一次明确请求并接收处理结果；Channel 可作为请求参数传入。",
        parts: { request: "一次请求", result: "一个结果", channelArgument: "绑定 Channel" },
        boundary:
          "Command 用于查询、编辑、保存或启动任务。前端创建 Channel 后，将它作为 Command 参数传入，为后续消息建立交付通道。一次命令的处理结果与持续消息流分别遵守各自契约。",
      },
      transport: {
        title: "Command 命令适配",
        description: "注册命令、校验输入、转换类型，调用业务用例并映射交付结果。",
        parts: { registry: "命令注册", validation: "输入校验", mapping: "DTO 映射" },
        boundary:
          "yss-application::ipc 拥有唯一命令注册表，依赖 Event、Channel 适配和共享 Contract。命令保持轻薄：解析和校验 wire 输入、转换类型、调用业务用例，再映射 DTO 或 CommandError；按对应业务契约交付 Event 或 Channel 消息。文件事务、科学计算和完整业务流程由业务层负责。",
      },
      business: {
        title: "Rust 业务用例",
        description: "执行查询、编辑、保存与任务用例，拥有业务规则和已提交状态。",
        parts: { useCase: "业务调用", authority: "状态权威", blocking: "耗时任务边界" },
        boundary:
          "应用层与领域层承担业务编排、文件事务及计算，向通信适配层返回类型化结果。耗时工作进入既有 blocking 边界；Tauri 命令不复制这些职责。大表在后端查询、分页或生成摘要，控制传输规模。",
      },
      contract: {
        title: "Contract 共享协议",
        description: "Rust 映射传输结构，前端解析并验证身份；稳定错误码由 React 本地化。",
        parts: { wireTypes: "显式传输类型", identity: "结果身份", paging: "分页与摘要" },
        boundary:
          "yss-ipc-contract 定义共享 DTO 与错误载荷，不依赖 Tauri 或 Application；各适配器转换业务结果，前端解析并校验响应身份。大表通过后端查询、分页和摘要限制规模。Command 错误固定包含以下三个字段；React 根据 code 提供本地化提示，后端不返回用户错误文案。",
        fields: {
          code: "稳定的错误类别，用于前端本地化。",
          details: "安全的结构化信息，无附加信息时为 null。",
          incidentId: "关联内部诊断；无诊断关联时为 null。",
        },
      },
      event: {
        title: "Event 变化通知",
        description: "资源提交等低频通知，通过前端订阅进入已有发布流程。",
        parts: { commit: "提交后通知", subscription: "前端订阅", cleanup: "订阅清理" },
        boundary:
          "yss-ipc-event 使用共享包络发送 Tauri Event，表示业务已经发生变化。订阅方明确通知产生时机、消费者与清理责任。不能假设所有事件必然送达，也不能据此重建已提交状态；需要时通过 Command 查询后端快照。",
      },
      publication: {
        title: "前端发布与去重",
        description: "Command 回执与 Event 统一进入发布流程，去重后更新前端投影。",
        parts: { deduplication: "提交身份去重", revision: "发布版本", snapshot: "按需查快照" },
        boundary:
          "资源修改的 Command 回执和 Event 可能属于同一次提交。前端按提交身份与 publication revision 统一去重，避免重复应用；必要时查询 Rust 快照，再通过已有发布流程更新投影。",
      },
      channel: {
        title: "Channel 有序消息流",
        description: "后端通过绑定通道持续向前端交付进度、执行事件和日志。",
        parts: { progress: "进度", execution: "执行事件", logs: "日志" },
        boundary:
          "yss-ipc-channel 负责中立通道适配；yss-application::ipc 负责执行消息和图草稿交接。前端创建 Channel 并通过 Command 绑定。每条业务流分别定义任务或会话身份、消息顺序、容量约束、丢失和缺口处理、取消及结束语义。恢复策略由对应业务流决定，不能统一假设所有流可重放。",
        lifecycle: { create: "创建", bind: "绑定", receive: "接收", end: "结束", cleanup: "清理" },
      },
      links: {
        invoke: "封装请求",
        dispatch: "调用命令",
        useCase: "业务调用",
        outcome: "类型化结果",
        encode: "映射响应",
        response: "DTO / 错误返回",
        notify: "提交后通知",
        publishEvent: "订阅通知",
        publishReceipt: "资源修改回执",
        stream: "持续交付",
      },
    },
    backendView: {
      inventory: "{{systems}} 个子系统 · {{count}} 个包",
      caption:
        "按 backend.md 展示职责归属与主要协作方向。点击节点底部可查看状态边界与 crate 清单；此图不代表完整 Cargo 依赖或职责迁移已完成。",
      businessKind: "业务子系统",
      transportKind: "通信适配",
      supportKind: "基础支撑",
      crates: "{{count}} 个 crate",
      inspect: "查看{{name}}的边界与 crate",
      application: {
        title: "应用与会话管理",
        description: "组织业务用例，协调跨模块操作、会话切换与结果采用。",
        parts: {
          useCases: "用例入口",
          coordination: "跨模块编排",
          sessions: "会话管理",
          adoption: "提交与结果采用",
        },
        boundary:
          "Application 组合各子系统运行时，重验身份和版本。统计规则归 SCI，结果分析由 Graph Execution 调用 runtime；编辑器投影归 Graph Editor，Application 负责调用与身份校验。",
      },
      project: {
        title: "项目与资源管理",
        description: "拥有已提交项目、资源身份与版本，负责文档事务和文件变化。",
        parts: {
          identity: "模型与身份",
          resources: "资源管理",
          commits: "文档提交",
          discovery: "发现、注册与监听",
        },
        boundary:
          "Project 拥有保存后的文档、资源版本及提交事务。图子系统提供图文档校验，数据子系统拥有表数据；前端草稿不构成第二份已提交状态。项目专属文件系统和监听属于本子系统。",
      },
      node: {
        title: "节点定义、注册与目录",
        description: "声明节点能力、端口和参数，提供注册表与本地化目录。",
        parts: {
          protocol: "能力与接口",
          registry: "注册与校验",
          catalog: "内置定义与创建描述",
          localization: "节点文档与本地化",
        },
        boundary:
          "Node 描述一种节点的能力，不依赖 Graph。节点实例、位置和连线归图文档；连接后的类型、Schema、血缘和诊断由 GraphSemanticSnapshot 管理，图诊断模板归 Graph。",
      },
      graph: {
        title: "图分析与执行",
        description: "从图文档、语义分析到运行准备、执行与结果查询。",
        parts: {
          documents: "文档与编辑",
          semantics: "语义与投影",
          preparation: "运行准备",
          execution: "执行与结果",
        },
        boundary:
          "GraphSemanticSnapshot 是解析后图语义的唯一事实源。Graph Editor 生成编辑器投影，Graph Runtime 解析编辑并在运行前准备匹配计划，Graph Execution 执行计划并管理结果。保存独立于执行；Output 展示运行失败。",
      },
      data: {
        title: "数据管理与查询",
        description: "管理数据契约、数据集目录、查询编辑与数据交换。",
        parts: {
          catalog: "目录与快照",
          runtime: "查询与编辑",
          engine: "DataFusion",
          exchange: "Arrow 与文件交换",
        },
        boundary:
          "SQLite committed catalog 保存元数据，Parquet 保存表数据，DataFusion 查询，Arrow 交换批次；这些不是每次操作必经的固定链路。Project 管资源声明，Database runtime 管会话访问，应用层组织导入导出和提交。",
      },
      science: {
        title: "科学计算",
        description: "提供统计模型、数值算法、输入准备和计算后端。",
        parts: {
          contracts: "中性计算契约",
          runtime: "计算运行时",
          algorithms: "统计模型与算法",
          mathematics: "线性代数与表达式",
        },
        boundary:
          "只有 Execution 和 IPC Command 直接调用 SCI runtime；Application 管理会话和结果有效性。runtime 调用 SCI 算法，SCI 通过 Linalg 使用矩阵；假设检验归 SCI。React 不重算统计事实，Julia/Bayes 插件不依赖 SCI。",
      },
      assistant: {
        title: "Assistant",
        description: "管理模型交互、会话轮次、审批与工具执行记录。",
        parts: {
          turns: "会话与 turn",
          tools: "工具与审批",
          gateway: "能力 Gateway",
          adapters: "模型与存储适配",
        },
        boundary:
          "Harness 拥有对话与工具流程，经 typed capability gateway 调用 Application；Tauri Command 不是内部业务总线。图工具沿用前端草稿流程与 Rust 校验，不另建 AI 草稿或项目模型。",
      },
      plugins: {
        title: "插件系统",
        description: "管理宿主插件安装启用、进程协议、任务与结果接入。",
        parts: {
          installation: "安装与启用",
          processes: "进程与协议",
          tasks: "任务生命周期",
          results: "数据与结果接入",
        },
        boundary:
          "Plugin Manager 管理运行与任务，Application 提供项目绑定的数据和结果能力。Julia/Bayes 独立发布；卸载插件不删除已提交项目结果。独立进程不等同于完整 OS sandbox。这里只统计宿主通用插件 crate。",
      },
      transport: {
        title: "通信适配",
        description: "接收 Tauri 请求，映射 DTO 和错误，交付通知与消息流。",
        parts: {
          commands: "Command 注册与用例调用",
          mapping: "DTO 与错误映射",
          delivery: "Event / Channel",
        },
        boundary:
          "Command、Event、Channel 与共享 Contract 组成桌面 IPC 边界，各自负责命令、事件、通道和 wire 类型，不拥有已提交业务状态。错误统一为 { code, details, incidentId }；业务提交后交付通知，交付失败不撤销已成功的提交。插件进程协议归插件系统。",
      },
      support: {
        title: "基础支撑",
        description: "提供桌面组装、结构化日志、平台操作和独立技术工具。",
        parts: {
          composition: "启动与组装",
          logging: "结构化日志",
          platform: "平台与窗口状态",
          utilities: "哈希、命名与路径",
        },
        boundary:
          "根包 yssbi 构造状态、注入适配器并注册通信入口。支撑能力由业务模块按需使用，不是所有请求必经的额外层；项目监听归 Project，表达式与数值计算归 SCI。基础支撑不接收业务领域规则。",
      },
      links: {
        assembly: "组装接线",
        useCase: "业务用例",
        assistantEntry: "会话入口",
        pluginEntry: "插件入口",
        gateway: "能力 Gateway",
        hostCapabilities: "宿主能力",
        projectUseCase: "项目用例",
        graphUseCase: "图用例",
        nodeDefinitions: "节点能力与目录",
        dataUseCase: "数据用例",
        inputData: "输入数据",
        computation: "计算能力",
      },
    },
    overview: "系统架构总览",
    viewSelector: "架构视图",
    dependenciesPage: "后端 crate 依赖",
    crates: {
      total: "{{count}} 个 workspace crates",
      incoming: "被引用 {{count}}",
      outgoing: "依赖 {{count}}",
      conditional: "条件 / 构建",
      legend:
        "全部 workspace crates · 箭头：引用方 → 被依赖方。点击节点高亮引用关系；虚线为条件或构建依赖。不含 dev 和第三方依赖。",
    },
    controls: "架构图视图控制",
    zoomIn: "放大",
    zoomOut: "缩小",
    fitView: "适应视图",
    minimap: "架构图缩略导航",
    request: "业务请求",
    dispatch: "调用用例",
    result: "结果 · 数据流",
    feedback: "响应 · 通知",
    description:
      "YssBI 桌面架构：React 展示与交互 ↔ Tauri IPC 请求、响应与通知 ↔ Rust 业务、计算与持久化。",
    frontend: {
      title: "前端",
      description: "界面呈现与用户交互",
      presentation: {
        title: "界面展示",
        summary: "工作台、画布、表格与结果",
        description: "工作台、图编辑器、数据表格与计算结果。",
      },
      interaction: {
        title: "用户交互",
        summary: "打开、编辑、保存与执行",
        description: "处理点击、选择、拖动与输入，发起打开、保存和执行请求。",
      },
      state: {
        title: "前端状态",
        summary: "界面状态与未保存草稿",
        description: "管理布局、选择、加载状态、未保存草稿与后端数据的显示投影。",
      },
      feedback: {
        title: "反馈呈现",
        summary: "响应校验与本地化提示",
        description: "校验响应及请求身份，更新对应界面，将稳定错误码转为本地化提示。",
      },
      boundaryTitle: "已提交业务数据以后端为准",
      boundarySummary: "保存成功更新投影，失败保留草稿。",
      boundary: "保存成功采用后端返回状态，失败保留草稿。数据库查询与统计计算由 Rust 处理。",
    },
    communication: {
      title: "通信",
      description: "请求、响应与有序消息流",
      command: {
        title: "Command",
        summary: "请求 / 响应 · 查询、保存与执行",
        description: "查询、提交编辑、保存，以及启动或取消任务。",
      },
      event: {
        title: "Event System",
        summary: "低频通知 · 资源提交与项目变化",
        description: "通知资源提交或项目变化；前端按需查询当前状态。",
      },
      channel: {
        title: "Channel",
        summary: "有序消息 · 进度、执行与日志",
        description: "持续交付任务进度、执行事件与日志；结束时按契约清理订阅。",
      },
      contract: {
        title: "DTO · 数据与错误契约",
        description: "明确参数与返回结构；命令失败统一返回 { code, details, incidentId }。",
      },
      boundaryTitle: "DTO 与统一错误契约",
      boundary:
        "src/services → invokeCommand → yss-application::ipc。命令只做适配，业务交给 Rust 应用层或领域模块。",
    },
    backend: {
      title: "后端",
      description: "业务、计算与持久化的权威",
      orchestration: {
        title: "业务编排",
        summary: "项目、会话与业务协调",
        description: "协调项目、图、数据库与执行，检查项目及会话是否仍然有效。",
      },
      domain: {
        title: "领域逻辑",
        summary: "图校验、执行与科学计算",
        description: "校验业务规则、解析图类型与依赖，准备执行计划并完成科学计算。",
      },
      state: {
        title: "状态管理",
        summary: "已提交项目、版本与结果",
        description: "管理项目编辑状态、资源版本、执行计划及结果，提供可查询投影。",
      },
      persistence: {
        title: "数据持久化",
        summary: "查询、事务与数据存储",
        description: "读写项目文件与表数据，执行查询、筛选、分页、导入导出及事务提交。",
      },
      boundaryTitle: "七个核心子系统",
      subsystems: {
        application: "应用与会话",
        project: "项目与资源",
        graph: "图分析与执行",
        data: "数据",
        science: "科学计算",
        assistant: "Assistant",
        plugins: "插件",
      },
      boundary:
        "应用与会话、项目与资源、图分析与执行、数据、科学计算、Assistant、插件；启动组装、日志诊断与平台适配提供支撑。",
    },
  },
  bayes: {
    title: "贝叶斯参数估计",
    openWindow: "打开贝叶斯参数估计",
    tabs: { model: "模型", results: "结果" },
    actions: {
      run: "运行",
      running: "运行中…",
      cancel: "取消",
      edit: "编辑",
      save: "保存",
      close: "关闭",
    },
    issue: {
      column: "列：{{column}}",
      row: "行：{{row}}",
      parameter: "参数：{{parameter}}",
      path: "字段：{{path}}",
    },
    errors: {
      title: "贝叶斯推断失败",
      fallback: "无法完成当前贝叶斯操作。",
      unknown: "无法完成当前贝叶斯操作（代码：{{code}}）。",
      unexpected: "发生意外的应用错误，无法完成当前贝叶斯操作。",
      bayes_expression_parse_failed: "无法解析模型公式。",
      bayes_inference_failed: "贝叶斯推断失败。",
      bayes_request_failed: "贝叶斯推断请求失败。",
      bayes_validation_request_failed: "模型校验请求失败。",
      bayes_validation_failed: "请先修复模型校验问题，再运行推断。",
      bayes_task_not_found: "推断任务已不可用。",
      bayes_task_active: "推断任务仍在运行，暂时无法清除。",
      bayes_cancel_failed: "无法取消推断任务。",
      bayes_result_not_found: "推断结果不可用。",
      bayes_result_artifact_read_failed: "无法读取结果产物。",
      bayes_artifact_not_found: "请求的结果产物不可用。",
      bayes_artifact_export_unsupported: "此结果产物不支持导出为 CSV。",
      bayes_artifact_export_failed: "无法导出结果产物。",
      bayes_samples_not_found: "当前结果不包含后验样本。",
      bayes_samples_invalid: "后验样本数据无效。",
      bayes_posterior_predictive_not_found: "当前结果不包含后验预测数据。",
      bayes_posterior_predictive_invalid: "后验预测数据无效。",
      bayes_service_lock_poisoned: "贝叶斯服务发生内部错误，报告问题时请提供事件编号。",
      bayes_dataset_source_unsupported: "所选数据源不支持贝叶斯推断。",
      bayes_dataset_load_failed: "无法读取所选数据。",
      bayes_input_empty: "所选数据源没有数据行。",
      bayes_input_response_non_finite: "响应变量包含缺失值或非有限值。",
      bayes_input_predictor_non_finite: "预测变量包含缺失值或非有限值。",
      bayes_input_column_not_numeric: "模型所需列不是数值类型。",
      bayes_input_column_missing: "模型所需列不存在。",
      bayes_input_response_binding_invalid: "响应表达式未准确绑定到一个数据列。",
      bayes_input_response_parameter_forbidden: "响应表达式不能包含模型参数。",
      bayes_input_response_division_by_zero: "响应表达式发生除零。",
      bayes_input_response_ln_domain: "响应表达式对非正数应用了 ln。",
      bayes_input_response_sqrt_domain: "响应表达式对负数应用了 sqrt。",
      bayes_input_response_result_non_finite: "响应表达式产生了非有限值。",
      bayes_input_bernoulli_response_invalid: "Bernoulli 响应只能包含布尔值或 0/1。",
      bayes_input_poisson_response_negative: "Poisson 响应不能包含负计数。",
      bayes_input_poisson_response_not_integer: "Poisson 响应必须包含整数计数。",
      bayes_backend_not_configured: "尚未配置贝叶斯推断后端。",
      julia_bayes_runtime_unavailable: "Julia 不可用，请检查 Julia 安装后重试。",
      julia_bayes_package_unavailable: "Julia 所需的贝叶斯软件包不可用。",
      julia_bayes_model_unsupported: "当前 Julia 后端不支持此模型。",
      julia_bayes_invalid_data: "所选数据不符合 Julia 贝叶斯后端要求。",
      julia_bayes_sampling_failed: "Julia 未能完成后验采样。",
      julia_bayes_result_missing: "Julia 未生成推断结果。",
      julia_bayes_result_invalid: "Julia 生成的推断结果无效。",
      julia_bayes_backend_failed: "Julia 贝叶斯后端运行失败。",
    },
    validation: {
      title: "请检查模型",
      unknown: "模型中存在需要检查的问题（代码：{{code}}）。",
      path: "字段：{{path}}",
      issues: {
        formula_required: "请输入模型公式。",
        formula_not_parsed: "请先解析模型公式。",
        dataset_required: "请选择数据源。",
        dependent_symbol_required: "请配置且仅配置一个因变量符号。",
        response_required: "请选择响应变量列。",
        response_symbol_required: "请将响应表达式绑定到数据符号。",
        response_column_unknown: "所选响应列已不存在。",
        response_binding_required: "请将因变量符号绑定到响应列。",
        data_column_unknown: "所选预测变量列已不存在。",
        data_binding_required: "请将每个自变量符号绑定到数据列。",
        response_expression_required: "请先绑定响应表达式。",
        response_parameter_forbidden: "响应表达式不能引用模型参数。",
        response_data_symbol_count_invalid: "响应表达式必须且只能引用一个数据符号。",
        response_binding_mismatch: "响应表达式与响应列绑定不一致。",
        likelihood_response_transform_unsupported: "当前似然不支持变换后的响应变量。",
        predictor_required: "请先解析并绑定预测表达式。",
        predictor_not_bound: "请确认所有预测符号的角色。",
        predictor_data_symbol_unconfigured: "请配置预测表达式使用的每个数据符号。",
        predictor_parameter_unconfigured: "请配置预测表达式使用的每个参数。",
        expression_number_invalid: "模型表达式包含非法数值。",
        expression_function_arity_invalid: "模型表达式中的函数参数数量不正确。",
        likelihood_response_type_invalid: "响应列类型与所选似然不兼容。",
        likelihood_sigma_constraint_warning: "Normal 的 sigma 参数应使用 positive 约束。",
        likelihood_sigma_parameter_required: "请配置 Normal 似然所需的 sigma 参数。",
        poisson_response_non_negative_unchecked: "Poisson 响应必须非负；运行采样前会检查实际数值。",
        predictor_column_type_invalid: "预测变量列必须为数值类型。",
        no_parameters: "当前模型没有未知参数。",
        parameter_name_required: "每个参数都必须有名称。",
        parameter_name_duplicated: "参数名称必须唯一。",
        parameter_bounds_invalid: "参数下界必须小于上界。",
        parameter_prior_args_invalid: "先验分布参数无效。",
        parameter_prior_constraint_mismatch: "先验分布可能与参数约束不匹配。",
        sampler_chains_invalid: "链数必须大于 0。",
        sampler_samples_invalid: "采样数必须大于 0。",
        sampler_target_accept_invalid: "目标接受率必须在 0 到 1 之间。",
        sampler_max_tree_depth_invalid: "最大树深度必须大于 0。",
        low_sample_count: "采样数较少，诊断结果可能不稳定。",
      },
    },
    common: { none: "无" },
    formula: {
      title: "1. 模型公式",
      responseExpression: "响应表达式",
      distribution: "分布",
      meanPredictor: "均值 / predictor",
      standardDeviationSigma: "标准差 / sigma",
      logit: "logit",
      logRate: "对数率",
      recognizedSymbols: "识别的符号",
    },
    symbols: {
      title: "2. 符号配置",
      symbol: "符号",
      role: "角色",
      data: "数据",
      column: "列",
      prior: "先验",
      bounds: "边界",
      actions: "操作",
      selectDataColumn: "选择数据列",
      configurationTitle: "符号配置",
    },
    roles: { dependent: "因变量", independent: "自变量", parameter: "参数" },
    constraints: {
      parameterConstraint: "参数约束",
      constraint: "约束",
      real: "实数",
      positive: "正数",
      unit: "单位区间",
      bounded: "有界",
      lowerBound: "下界",
      upperBound: "上界",
      includeLower: "包含下界",
      includeUpper: "包含上界",
      true: "是",
      false: "否",
    },
    prior: {
      title: "先验分布",
      distribution: "先验分布",
      selectDistribution: "选择分布",
      recommended: "推荐",
      notSet: "未设置分布",
      args: {
        mean: "均值",
        standardDeviation: "标准差",
        logMean: "对数均值",
        logStandardDeviation: "对数标准差",
        lowerBound: "下界",
        upperBound: "上界",
        alpha: "Alpha",
        beta: "Beta",
        shape: "形状",
        scale: "尺度",
        degreesOfFreedom: "自由度",
        location: "位置",
      },
    },
    dataBinding: {
      title: "数据绑定",
      dataSource: "数据源",
      dataColumn: "数据列",
      selectDataSource: "选择数据源",
      noDataSources: "当前项目没有可用数据源，请先导入数据。",
      noColumns: "当前数据源没有列信息，正在同步或请刷新数据源。",
      noDataSelected: "未选择数据",
      unknownDataSource: "未知数据源",
      unboundColumn: "未绑定列",
    },
    sampler: {
      title: "3. 采样器",
      nutsOnlyDescription: "当前仅支持 NUTS",
      algorithm: "算法",
      chains: "链数",
      samples: "样本数",
      warmup: "预热",
      seed: "随机种子",
      targetAccept: "目标接受率",
      maxTreeDepth: "最大树深度",
    },
    progress: {
      stageElapsed: "本阶段 {{duration}}",
      totalElapsed: "总计 {{duration}}",
      remainingAndTotal: "剩余 {{remaining}} · 总计 {{total}}",
      stages: {
        queued: "等待运行",
        running: "正在启动",
        materializingData: "正在准备数据",
        loadingModel: "正在启动 Julia 任务",
        loadingData: "正在读取模型与数据",
        preparingResponse: "正在准备响应变量",
        loadingKernels: "正在加载生成的计算 Kernel",
        preparingKernels: "正在准备 Predictor Kernel",
        buildingModel: "正在构造先验与 Turing 模型",
        initializingNuts: "正在特化模型并初始化 NUTS",
        warmup: "NUTS 预热",
        sampling: "后验采样",
        summarizing: "正在计算参数摘要",
        writingSamples: "正在写入后验样本",
        posteriorPredictive: "正在计算后验预测",
        finalizing: "正在完成诊断与产物",
        readingResult: "正在读取结果",
        writingArtifacts: "正在计算结果数据",
        renderingResult: "正在计算并渲染结果数据",
        cancelling: "正在取消",
      },
    },
    results: {
      titles: {
        summary: "结果摘要",
        posteriorTrace: "后验轨迹",
        posteriorDensity: "后验密度",
        autocorrelation: "自相关",
        posteriorPredictive: "后验预测",
      },
      actions: { exportCsv: "导出 CSV", openFolder: "打开结果文件夹" },
      messages: { exportSuccess: "CSV 导出成功" },
      errors: {
        exportCsv: "导出 CSV 失败：{{error}}",
        openFolder: "打开结果文件夹失败：{{error}}",
        plot: "图表数据读取失败：{{error}}",
        predictivePlot: "预测区间图读取失败：{{error}}",
      },
      loading: { plot: "正在读取图表数据…" },
      metrics: {
        chains: "链数",
        drawsPerChain: "每链抽样数",
        warmupPerChain: "每链预热数",
        divergences: "发散数",
        treeDepthHits: "最大树深度命中数",
      },
      table: { status: "状态", parameter: "参数", mean: "均值", sd: "标准差" },
      controls: {
        parameter: "参数",
        scale: "尺度",
        originalScale: "原始尺度",
        modelScale: "模型尺度（{{transform}}）",
        chain: "链",
        allChains: "全部链",
        pooled: "合并链",
        chainNumber: "链 {{chain}}",
      },
      chart: {
        value: "值",
        density: "密度",
        autocorrelation: "自相关",
        observation: "观测",
        response: "响应",
        responseModelScale: "响应（模型尺度）",
        drawStride: "抽样，步长 {{stride}}",
        lagMax: "滞后，最大 {{maxLag}}",
      },
      empty: {
        summary: "运行完成后显示参数摘要。",
        plots: "运行完成后显示图表数据。",
        posteriorSamples:
          "当前结果没有保存后验样本，因此无法生成轨迹、密度和自相关图。请在采样器中启用保存样本后重新运行。",
        parameters: "没有可绘制的参数。",
        trace: "没有轨迹数据。",
        density: "没有密度数据。",
        autocorrelation: "没有自相关数据。",
        posteriorPredictive: "运行完成后显示后验预测。",
        posteriorPredictiveData: "当前结果没有保存后验预测数据。",
      },
      ratings: {
        unavailable: "不可用",
        excellent: "极好",
        recommended: "推荐标准",
        concerning: "有点问题",
        untrustworthy: "不建议相信",
        notConverged: "基本没收敛",
        veryGood: "非常好",
        good: "很好",
        acceptable: "可接受",
        low: "偏低",
        unreliable: "不可靠",
      },
      status: { ok: "正常", check_rhat: "检查 R-hat", low_ess: "ESS 偏低", unknown: "未知" },
      severity: { good: "良好", warning: "警告", bad: "较差", unknown: "未知" },
      diagnostics: {
        ratingPrefix: "评级：{{rating}}",
        suggestionPrefix: "建议：{{suggestion}}",
        rhatDescription: "衡量多条 MCMC 链是否收敛到同一后验分布；数值越接近 1，链间混合通常越好。",
        essDescription:
          "左侧为 Bulk ESS，右侧为 Tail ESS。单元格颜色与综合评级采用两者中较低的一项。",
        essDetail: "{{metric}}：{{value}}（{{rating}}）",
        metricValue: "{{metric}}：{{value}}（阈值 {{threshold}}）",
        suggestedNextSteps: "建议的下一步",
        warningsExplained: "诊断警告说明",
        suggestions: {
          checkMetrics: "检查 R-hat、ESS、发散和树深度指标。",
          saveSamples: "启用保存样本以查看轨迹、密度和自相关图。",
          increaseSampling: "必要时增加预热和抽样次数。",
          inspectPlots: "结合轨迹图和后验预测检查模型稳定性。",
        },
        assessment: {
          good: { title: "诊断良好", summary: "当前采样诊断未发现明显问题。" },
          warning: { title: "需要关注", summary: "部分诊断指标需要进一步检查。" },
          bad: { title: "诊断较差", summary: "当前结果可能尚未可靠收敛。" },
          unknown: { title: "诊断不完整", summary: "当前结果缺少部分诊断信息。" },
        },
        warnings: {
          rhat_too_high: {
            title: "{{parameter}} R-hat 偏高",
            explanation: "不同链之间没有充分混合，当前后验摘要可能不稳定。",
            suggestion: "增加预热或抽样次数，检查模型是否过参数化，或收紧过宽的先验。",
          },
          ess_too_low: {
            title: "{{parameter}} 有效样本量不足",
            explanation: "独立有效样本较少，均值、分位数和可信区间可能有较大 Monte Carlo 误差。",
            suggestion: "增加抽样次数，检查自相关；必要时标准化 predictor 或调整模型参数化。",
          },
          unknown: {
            title: "诊断警告（{{code}}）",
            explanation: "后端为当前结果返回了一条结构化诊断警告。",
            suggestion: "使用估计结果前，请检查该指标与结果图表。",
          },
        },
      },
    },
  },
  julia: {
    menu: {
      title: "Julia 计算引擎",
      install: "安装最新版 Julia…",
      refresh: "刷新状态",
      managedRuntime: "系统 Julia",
    },
    status: {
      notInstalled: "未安装",
      installing: "正在安装…",
      ready: "已就绪 · Julia {{version}}",
      invalid: "Runtime 无效或不可启动",
    },
    worker: {
      checking: "Julia 检查中",
      checkingDetail: "正在检查 Julia 计算引擎状态",
      starting: "Julia 加载中",
      startingDetail: "正在准备依赖并启动 Julia 计算引擎",
      ready: "Julia 已就绪",
      readyDetail: "Julia 计算引擎已加载，可直接开始科学计算",
      unavailable: "Julia 不可用",
      unavailableDetail: "Julia 计算引擎未启动",
      statusFailed: "无法读取 Julia 计算引擎状态",
    },
    install: {
      title: "安装 Julia Runtime",
      message:
        "Julia 为 YssBI 提供可选的高级科学计算能力。YssBI 会使用系统中的 Julia；如未安装，将安装最新版 Julia。",
      confirm: "安装最新版",
      preparing: "正在安装 Julia",
      preparingDetail: "正在检查系统 Julia…",
      success: "Julia {{version}} 已准备就绪",
      failed: "Julia 安装未完成",
    },
  },
  aboutModal: {
    title: "关于 {{appName}}",
    version: "版本 {{version}}",
    description: "统计计算与可视化工作台。",
  },
  bottomBar: {
    nodes: "{{count}} 节点",
    links: "{{count}} 连接",
    selected: "{{count}} 选中",
    nodeCount: "节点数量",
    connectionCount: "连接数量",
    selectedNodes: "选中节点",
    executionStatus: "执行状态",
    canvasViewport: "画布视图",
    resetViewport: "重置画布视口",
    ariaLabel: "状态栏",
  },
  workbench: {
    layoutError: {
      notReady: "工作区尚未就绪。",
      invalidPanel: "无法打开此面板。",
      groupUnavailable: "目标编辑器组当前不可用。",
      openFailed: "无法打开面板。",
      restoreFailed: "无法恢复工作区布局。",
    },
  },
  plugins: {
    signerChangeTitle: "插件签名身份已变化",
    signerChangeMessage:
      "原签名指纹：{{previous}}\n新签名指纹：{{next}}\n只有确认此密钥变更来自你信任的发布者后，才继续安装。",
    approveSignerChange: "确认签名者变更",
    maintenance: "管理 {{name}}",
    maintenanceDescription: "查看任务历史和诊断，清理可再生成的缓存。项目正式结果会保留。",
    manageData: "历史、存储与诊断",
    storage: "私有存储",
    storageUsage: "已用 {{used}} / 获准 {{budget}}，其中缓存 {{cache}}",
    softBudget:
      "这是应用层软限额；原生插件仍以当前用户身份运行。清理缓存前请关闭插件页面并结束任务。",
    clearCache: "清理上述缓存目录",
    collectPackages: "清理未引用旧包",
    taskHistory: "已结束的任务",
    clearHistory: "清理历史显示与结果缓存",
    historyRetention:
      "历史和幂等回执保留 30 天。清理显示不会删除保留期内的去重凭据，也不会重复执行旧请求。",
    loadMore: "加载更多",
    diagnostics: "插件进程诊断",
    diagnosticsDescription: "保留最近进程的有限 stderr 输出，并标记进程和当时的任务。",
    noDiagnostics: "暂无诊断输出",
    diagnosticsTruncated: "较早的输出已因容量限制移除。",
    taskStates: {
      admitted: "已接受",
      running: "运行中",
      cancelRequested: "正在取消",
      succeeded: "已完成",
      failed: "已失败",
      cancelled: "已取消",
      outcomeUnknown: "结果未知",
    },
    installPackage: "从安装包添加插件",
    loadFailed: "无法读取插件状态，请重新检查。",
    operationFailed: "插件操作未完成。若有运行中的任务，请先取消或等待完成。",
    missingView: "此插件尚未安装。安装后可在原位置恢复此视图。",
    disabledView: "此插件已禁用，可在插件面板重新启用。",
    viewFailed: "插件视图无法加载，请检查插件状态后重试。",
    viewReleaseFailed: "旧页面会话尚未成功释放。重试会先完成清理，不会重复占用视图名额。",
    viewNavigationBlocked: "插件页面发生了非预期导航，连接已安全撤销。请重新加载页面。",
    viewBusy: "插件正在安装、更新或停止，请稍后重试。",
    viewCapacityReached: "已达到此插件的视图数量上限，请关闭不使用的插件页面后重试。",
    viewProcessUnavailable: "插件进程未能就绪或连接已中断，请重新连接。",
    viewSessionExpired: "页面所属的项目或插件会话已失效，请重新连接。",
    viewConnectionFailed: "无法建立插件页面连接，请根据下方错误代码检查插件状态。",
    disabled: "已禁用",
    enable: "启用",
    disable: "禁用",
    open: "打开视图",
    manage: "管理 {{name}}",
    uninstall: "卸载",
    uninstallTitle: "卸载 {{name}}",
    uninstallMessage:
      "将卸载插件并关闭其所有面板和标签页，未保存的页面状态将丢失。项目、已保存结果和插件私有数据会保留。",
    trustTitle: "安装 {{name}}",
    trustMessage:
      "此插件包含本地可执行程序，将以当前用户权限运行。\n\n发布者：{{publisher}}\n申请能力：{{permissions}}\n签名密钥指纹：{{fingerprint}}\n\n仅安装你信任的来源。",
    available: "可安装",
    availableEmpty: "暂无其他可安装插件",
    install: "安装",
    openJuliaPanel: "打开 Julia 侧栏",
    juliaName: "Julia 计算支持",
    juliaDescription: "提供 Julia 计算运行时与贝叶斯分析支持。",
    builtin: "内置",
    installRuntime: "安装运行时",
    openBayes: "打开贝叶斯分析",
    recheck: "重新检查",
    environmentUnavailable: "Julia 已安装，但计算环境尚未就绪，请查看运行日志后重新检查。",
    installed: "已安装",
    recommended: "推荐",
    recommendedEmpty: "暂无推荐插件",
    catalogUnavailable: "更多插件目录尚未接入，暂不提供在线搜索与推荐。",
  },
  settings: {
    title: "设置",
    close: "关闭设置",
    applyHint: "更改后自动应用",
    noResults: "未找到匹配的设置分类",
    noResultsHint: "试试其他关键词，或清除搜索查看全部设置。",
    clearSearch: "清除搜索",
    sectionDescriptions: {
      ai: "配置统计分析助手的模型与连接。",
      appearance: "调整工作区的显示与交互偏好。",
    },
    sections: {
      ai: "AI",
      appearance: "外观",
    },
    searchPlaceholder: "搜索设置",
    loading: "加载设置中...",
    confirmResetAllTitle: "恢复所有默认设置",
    confirmResetAllMessage: "确定要恢复所有默认设置吗？此操作不可撤销。",
    confirmResetTitle: "恢复默认设置",
    confirmResetMessage: "确定要恢复{{section}}的默认设置吗？",
    restoredAll: "已恢复所有默认设置",
    restoreAllFailed: "恢复默认设置失败: {{error}}",
    restoredSection: "已恢复{{section}}默认设置",
    restoreSectionFailed: "恢复{{section}}默认设置失败: {{error}}",
    labels: {
      openAiModel: "OpenAI 模型",
      openAiApiKey: "OpenAI API 密钥",
      openAiBaseUrl: "OpenAI Base URL",
      colorTheme: "颜色主题",
      language: "语言",

      smoothScroll: "平滑滚动",
      titleBarStyle: "标题栏样式",
    },
    descriptions: {
      openAiModel: "统计分析助手使用的模型。",
      openAiApiKey: "密钥保存在本地应用设置中，仅发送给本机 Harness 运行时。",
      openAiBaseUrl: "统计分析助手使用的 OpenAI 兼容 API 地址。",
      colorTheme: "通过主题统一控制工作区、节点、图表和表格的配色。",
      language: "控制应用界面语言。",

      smoothScroll: "为滚动区域（列表、设置、侧栏等）启用平滑滚动。",
      titleBarStyle: "使用应用内自绘标题栏或操作系统原生窗口边框。",
    },
    options: {
      darkModern: "深色现代（默认）",
      oledBlack: "OLED 黑",
      lightModern: "浅色现代",
      left: "左侧",
      right: "右侧",
      hidden: "隐藏",
      titleBarCustom: "自定义",
      titleBarNative: "原生",
    },
  },
  graphDiagnostics: {
    sidebarTooltip: "该图有 {{count}} 个问题",
  },
  canvas: {
    connection: {
      feedback: {
        append: "连接端口",
        replace: "替换现有连接",
        "same-port": "请选择其他端口",
        "same-node": "同一节点上的端口无法互连",
        "same-direction": "请将输出端口连接到输入端口",
        "type-mismatch": "这些端口的数据类型不兼容",
        orphan: "孤立端口无法连接",
        capacity: "该端口已达到连接数量上限",
      },
      errors: {
        graph_port_not_found: "端口已不存在",
        graph_node_not_found: "节点已不存在",
        graph_connection_not_found: "连接已不存在",
        graph_port_orphan: "孤立端口无法连接",
        graph_connection_direction_mismatch: "请将输出端口连接到输入端口",
        graph_connection_type_mismatch: "这些端口的数据类型不兼容",
        graph_connection_type_unavailable: "端口类型信息不可用",
        graph_connection_limit_reached: "该端口已达到连接数量上限",
        graph_connection_order_required: "该连接需要指定顺序",
        graph_connection_order_forbidden: "该连接不允许指定顺序",
        graph_connection_already_exists: "这些端口已经连接",
        graph_edit_changed: "图已发生变化，请重试。",
        graph_edit_busy: "图正在处理其他编辑，请稍后重试。",
        graph_connection_move_source_empty: "源端口没有可移动的连接",
        graph_connection_move_same_port: "请选择其他目标端口",
        graph_mutation_empty_targets: "请至少选择一个图元素",
        graph_mutation_duplicate_target: "选择中包含重复的图元素",
        graph_managed_node_delete_forbidden: "无法删除受管理的节点",
      },
    },
    pinResultSearch: {
      open: "搜索 Pin 结果",
      close: "关闭 Pin 结果搜索",
      empty: "暂无 Pin 结果，请先运行图",
      searchPlaceholder: "按节点、Pin 或结果标题筛选…",
      noMatches: "没有匹配的 Pin 结果",
      unknownNode: "未知节点",
      unknownPin: "未知 Pin",
    },
    graphState: {
      new: "尚无匹配结果",
      unexecuted: "尚未运行",
      running: "参与本次运行",
      error: "存在阻断问题或运行失败",
      valid: "结果可用",
      stale: "结果过期",
      partial: "部分结果可用",
    },
    resultCount: "{{valid}} / {{total}} 个输出可用",
    cancelExecution: "中断执行",
    clearExecutionArtifacts: "清除运行提示",
    clearExecutionArtifactsDisabled: "暂无可清除的运行提示",
    executionArtifactsCleared: "已清除运行提示",
    executionArtifactsClearFailed: "清除运行提示失败：{{message}}",
    executionCancelled: "执行已中断",
    executing: "执行中...",
    functionRunUnavailable: "函数图通过其调用方运行",
    problemsBlockExecution: "请先解决图中的阻断问题再运行",
    executeCurrentGraph: "运行当前图",
    newEventGraph: "新建事件",
    newFunctionGraph: "新建函数",
    coreLogic: "核心逻辑",
    reusableRoutine: "可复用流程",
    openFile: "打开文件",
    showAllCommands: "显示所有命令",
    nodePalette: {
      unavailable: "暂不可用",
      searchPlaceholder: "搜索节点…",
      collapseAll: "折叠所有内容",
      expandAll: "展开所有内容",
      noMatches: "未找到匹配项",
    },
  },
  nodeDocumentationModal: {
    title: "节点文档",
    description: "搜索当前语言的节点标题和别名；再次点击当前节点可收起文档。",
    searchPlaceholder: "搜索节点标题和别名…",
    noMatches: "未找到匹配的节点文档",
    noDocumentation: "此节点暂无详细文档。",
    selectNode: "选择节点以查看其文档。",
    nodeId: "节点 ID",
    ports: "端口",
    noPorts: "无端口",
    parameters: "参数",
    noParameters: "无参数",
    resourcePath: "资源路径",
    resourceRevision: "资源修订版本",
    close: "关闭节点文档",
  },
  editorDropPreview: {
    openResource: "打开 {{name}}",
    mergeIntoGroup: "移动到该分组",
    splitLeft: "向左拆分",
    splitRight: "向右拆分",
    splitTop: "向上拆分",
    splitBottom: "向下拆分",
  },
  tabBar: {
    contextMenu: {
      close: "关闭",
      hideContent: "隐藏内容",
      expandContent: "展开内容",
      closeOthers: "关闭其他",
      closeSaved: "关闭已保存",
      closeAll: "全部关闭",
    },
  },
  activityBar: {
    plugins: "插件",
    project: "项目",
    nodes: "节点",
    commands: "命令",
  },
  chartsSidebar: {
    charts: "图表",
    noCharts: "暂无图表",
    configuration: "配置",
    openChartHint: "打开或新建图表以配置图表。",
    dataset: "数据集",
    chartType: "图表类型",
    encodingX: "X 轴",
    encodingY: "Y 轴 / 列",
    columns: "列",
    noColumns: "无可用列",
    chartTypes: {
      histogram: "直方图",
      scatter: "散点图",
      line: "折线图",
    },
  },
  chart: {
    created: "图表已创建",
    createFailed: "创建图表失败",
    saved: "图表已保存",
    noActiveChart: "未选择图表",
    previewEmpty: "请选择列以预览图表",
    previewEmptyHint: "在右侧 Detail 面板中选择数据集、图表类型与编码",
    previewLoadFailed: "无法加载图表预览",
    previewColumnNotFound: "列“{{column}}”已不可用",
    refresh: "刷新",
    refreshing: "刷新中…",
  },
  did: {
    fakeGroup: {
      methodology:
        "实体级随机分配共执行 {{nPerm}} 次置换，每次在 {{nEntities}} 个实体中随机指定 {{nTreatedEntities}} 个虚构处理实体；其中 {{nPermValid}} 次拟合成功。双侧随机化推断 p 值在相同双向固定效应设定下比较置换系数与观测系数的绝对值。",
      unavailable: {
        no_treated_entities: "该检验至少需要一个处理组实体。",
        all_entities_treated: "该检验至少需要一个未处理实体。",
        insufficient_valid_permutations:
          "{{nPerm}} 次置换中仅有 {{nPermValid}} 次有效拟合，至少需要 {{minValidPermutations}} 次。",
      },
      errors: {
        internal_error: "随机化检验发生内部错误。",
        ipc_transport_failure: "无法连接随机化检验服务。",
        ipc_malformed_error: "随机化检验返回了无效的错误响应。",
        did_fake_group_request_failed: "随机化检验请求失败。",
        did_fake_group_invalid_response: "随机化检验返回了无效结果。",
        did_fake_group_invalid_initial_result: "已保存的随机化检验结果无效。",
        unknown: "随机化检验失败（代码：{{code}}）。",
      },
    },
  },
  sidebar: {
    open: "打开",
    viewInDatabaseEditor: "在数据库编辑器中查看",

    projectTree: {
      empty: "暂无项目资源",
      categories: {
        events: "事件",
        functions: "函数",
        charts: "图表",
      },
    },

    noNodes: "暂无可用节点",
    noActiveGraph: "未打开活动图",
    noActiveGraphDescription: "打开一个 Event 或 Function 后可查看命令状态",
    noData: "暂无数据",
    noEvents: "暂无 Event",
    noFunctions: "暂无 Function",
    dataLoadFailed: "数据加载失败",
    noCommandHistory: "无命令历史",
    noBuiltinNodes: "无系统内置节点",
    deleteDataTitle: "删除数据集",
    deleteDataMessage: "确定删除数据集「{{name}}」？此操作无法撤销。",
    sections: {
      event: "事件",
      function: "函数",
      local: "局部",
      global: "全局",
      data: "数据",
    },
    commands: {
      MoveNodes: "移动节点",
      SetPinValue: "设置引脚值",
      ConnectPins: "连接引脚",

      CreateNode: "创建节点",
      DeleteNodes: "删除节点",
      Composite: "组合操作",
    },
  },
  databaseEditor: {
    title: "数据库编辑器",
    noDataFrame: "无 DataFrame",
    loadingProjectData: "正在加载项目数据...",
    noDataFrameSelected: "未选择 DataFrame",
    cellPreviewPlaceholder: "选中单元格后将在此显示内容",
    columns: "列",
    rows: "行",
    editCell: "编辑单元格",
    insertRowAbove: "在上方插入行",
    insertRowBelow: "在下方插入行",
    deleteRows: "删除 {{count}} 行",
    deleteRow: "删除行",
    renameColumn: "重命名列",
    addColumn: "添加列",
    deleteColumn: '删除列 "{{name}}"',
  },
  resultState: {
    pending: "结果处理中（{{completed}}/{{total}}）",
    executionFailed: "结果计算失败",
    upstreamFailed: "上游结果计算失败",
    cancelled: "结果计算已取消",
  },
  sourceInspector: {
    title: "结果查看",
    loading: "加载中…",
    missingResultId: "URL 中缺少结果 ID",
    noSource: "未找到结果",
    loadFailed: "加载结果失败",
  },
  plot: {
    title: "图表",
    failedInitialize: "窗口初始化失败",
    noData: "此窗口没有可用数据",
    invalidData: "图表数据格式无效",
    readyTitle: "图表窗口已就绪",
    readyHint:
      "从 Scatter、Line、ECDF、KDE、Histogram 或 Correlation Plot 节点执行后将在此显示图表",
  },
  info: {
    regressionResults: "回归结果",
    missingDataKey: "URL 中缺少数据 key",
    noData: "此窗口没有可用数据",
    failedInitialize: "窗口初始化失败",
  },
  log: {
    streamTruncated: "部分日志可能已丢失",
    streamDisconnected: "日志连接已断开",
    title: "日志",
    showCount: "显示 {{filtered}} / {{total}} 条",
    refresh: "刷新日志",
    autoScrollEnabled: "自动滚动已启用",
    autoScrollDisabled: "自动滚动已禁用",
    filter: "过滤器",
    searchPlaceholder: "搜索日志内容...",
    level: "级别",
    domains: {
      all: "全部",
      application: "应用",
      execution: "执行",
      system: "系统",
      graph: "图",
      data: "数据",
      ui: "界面",
    },
    clear: "清空日志",
    loadingLogs: "加载日志中...",
    noLogs: "暂无日志",
    noMatches: "没有匹配的日志",
    runGraphHint: "执行图后将显示日志",
    adjustFilterHint: "尝试调整过滤条件",
  },
  importModal: {
    title: "导入数据",
    subtitle: "选择数据来源，导入为项目中的数据集",
    close: "关闭",
    comingSoon: "功能开发中",
    developing: "开发中",
    categories: {
      file: "文件",
      sql: "SQL 数据库",
      samples: "示例数据",
      other: "其他",
    },
    types: {
      csv: { label: "CSV", description: "逗号分隔文本，适合表格型数据" },
      xlsx: { label: "Excel", description: "工作簿或单 Sheet 表格" },
      sqlite: { label: "SQLite", description: "本地 .db / .sqlite 文件" },
      postgres: { label: "PostgreSQL", description: "远程 PostgreSQL 服务" },
      mysql: { label: "MySQL", description: "远程 MySQL 数据库" },
      mariadb: { label: "MariaDB", description: "远程 MariaDB 数据库" },
      api: { label: "REST API", description: "从远程接口拉取数据" },
    },
    selectSheet: "选择 Sheet",
    samples: {
      subtitle: "选择内置示例，导入到当前项目后即可自由编辑和分析。所有示例均可离线使用。",
      loading: "正在加载示例…",
      empty: "暂无可用的示例数据。",
      import: "导入到当前项目",
      importing: "正在导入示例…",
      retry: "重新加载",
      failed: "暂时无法完成此操作，请稍后重试。",
      dimensions: "{{rows}} 行 · {{columns}} 列",
      defaultDescription: "用于浏览、编辑和分析的内置示例数据。",
      errors: {
        sample_not_found: "未找到该示例，请重新打开导入窗口。",
        sample_version_mismatch: "示例版本已变化，请重新打开导入窗口。",
        sample_integrity_failed: "示例文件校验失败，请修复应用资源后重试。",
        sample_catalog_invalid: "示例目录无法读取，请修复应用资源后重试。",
        sample_resource_unavailable: "示例文件不可用，请修复应用资源后重试。",
        stale_project_lifecycle: "当前项目已变化，请重新打开导入窗口。",
        project_lifecycle_admission_closed: "项目正在切换，请稍后重试。",
        project_recovery_required: "项目需要恢复，请重新打开项目后继续。",
      },
      datasets: {
        iris: {
          name: "鸢尾花 · Iris",
          description: "花萼、花瓣尺寸与品种，适合描述统计、分组比较和散点图。",
        },
        diamonds: {
          name: "钻石 · Diamonds",
          description: "钻石的重量、切工、颜色与价格，适合分布分析和回归建模。",
        },
        flights: {
          name: "航班 · Flights",
          description: "航班时刻、延误与航线，适合缺失值处理、分组汇总和时间分析。",
        },
        "data-car": {
          name: "汽车保险 · dataCar",
          description: "车辆、投保暴露与理赔记录，适合频数分析和理赔成本建模。",
        },
        "vic-elec": {
          name: "用电需求 · vic_elec",
          description: "半小时用电需求、气温与节假日，适合时间序列和季节性分析。",
        },
      },
    },
    sheetHint: "选择要导入的 Sheet",
    selectTable: "选择表",
    tableHint: "选择要导入的表",
    connect: "连接",
    connectTo: "连接 {{name}}",
    formConfig: "表单配置",
    connectionString: "连接字符串",
    host: "主机",
    port: "端口",
    username: "用户名",
    password: "密码",
    database: "数据库",
    connectionRequired: "请填写连接信息",
    connectionFieldsInvalid: "一个或多个连接字段包含无效文本",
  },
  dataOperation: {
    importing: "正在导入数据…",
    reading: "正在读取…",
    importingCsv: "解析 CSV 并写入项目数据库",
    importingExcel: "导入 Sheet「{{sheet}}」",
    importingSqlite: "导入 SQLite 表「{{table}}」",
    importingRemote: "从 {{label}} 导入表「{{table}}」",
    readingSqlite: "读取 SQLite 表列表",
    readingExcel: "读取 Excel Sheet 列表",
    readingRemote: "连接 {{label}} 并读取表列表",
    importFailed: "导入失败：{{error}}",
    deleteFailed: "删除失败：{{error}}",
    renameFailed: "重命名失败：{{error}}",
    connectFailed: "{{label}} 连接失败：{{error}}",
    noSqliteTables: "该数据库中没有用户表",
    noRemoteTables: "该数据库中未找到用户表",
    noExcelSheets: "该 Excel 文件中没有 Sheet",
    comingSoon: "{{type}} 导入功能开发中…",
  },
  detail: {
    data: {
      applyPhysical: "应用 Physical 转换",
      applySemantic: "应用 Semantic",
      categoryHint:
        "填写原始取值到内部语义标签的映射。数据视图保留原始值，编码大小不表示大小关系。",
      ordinalHint: "按从低到高的顺序列出等级；使用箭头明确调整顺序。",
      binaryHint: "必须配置两个不同的允许值；缺失值不计为类别。",
      value: "第 {{index}} 项的存储值",
      label: "第 {{index}} 项的标签",
      labelPlaceholder: "语义标签",
      removeValue: "删除第 {{index}} 项",
      addValue: "添加取值",
      positive: "事件 / 正类",
      noPositive: "未指定",
      integer: "仅允许整数",
      minimum: "最小值",
      maximum: "最大值",
      updateFailed:
        "无法应用设置。请检查值域、精度、允许取值及当前 Semantic 与 Physical 的兼容性。",
    },
    inspect: {
      emptyTitle: "未选择节点",
      empty: "选择一个 Canvas 节点以编辑其配置",
      multipleTitle: "已选择 {{count}} 个节点",
      multiple: "请选择一个节点以编辑其配置",
      noParameters: "该节点没有可配置属性",
    },
    result: {
      readFailedTitle: "结果不可用",
      expandPlot: "在独立窗口中展开",
      expandFailedTitle: "无法打开图表窗口",
      expandFailed: "图表预览仍可在当前结果页签中查看。",
      invalidPlot: "该结果不包含有效的图表数据。",
    },
    noSelection: "未选择",
    noSelectionHint: "从侧栏或画布中选择一项",
    nodeNotFound: "图中未找到该节点。",
    parameters: "参数",
    sections: {
      capabilities: "能力",
      diagnostics: "问题",
    },
    parameterEditor: {
      apply: "应用",
      column: "列",
      operator: "运算符",
      valueType: "值类型",
      selectColumn: "选择{{column}}",
      moveColumnUp: "上移{{column}}",
      moveColumnDown: "下移{{column}}",
      predicateColumn: "筛选条件列",
      predicateOperator: "筛选运算符",
      predicateValueType: "筛选值类型",
      predicateValue: "筛选值",
    },
    fields: {
      name: "名称",
      type: "类型",
      elementType: "元素类型",
      category: "分类",
      graph: "图",
      value: "值",
      columns: "列",
      rows: "行",
      source: "来源",
      time: "时间",
      level: "级别",
      message: "消息",
      column: "列名",
    },
    itemTypes: {
      event: "事件",
      function: "函数",
      data: "数据集",
      chart: "图表",
    },
    typeLabels: {
      function: "函数",
    },
    callSites: {
      title: "被引用 ({{count}})",
      loading: "正在加载…",
      empty: "暂无其他图引用此函数",
      references: "{{count}} 处",
    },
    callFunction: {
      openTarget: "打开目标函数",
      missingTarget: "目标函数缺失：{{path}}",
      target: "目标函数",
      changeTarget: "更换目标",
    },
    counts: {
      columns: "{{count}} 列",
      rows: "{{count}} 行",
    },
    constants: {
      title: "常量",
      defaultName: "常量",
      add: "添加常量",
      updateFailed: "常量更新失败，请重试。",
      insertReference: "插入引用",
      remove: "删除常量 {{name}}",
      empty: "在此添加供当前图使用的常量。",
      choose: "选择常量",
    },
    constantValue: {
      edit: "编辑",
      empty: "（空）",
      clear: "清空",
      jsonLabel: "JSON",
      title: {
        Array: "编辑 Array 值",
        Object: "编辑 Object 值",
        DataFrame: "编辑 DataFrame 值",
        DataSeries: "编辑 DataSeries 值",
      },
      description: {
        Array: "以 JSON 数组编辑元素值，元素类型由常量类型决定。",
        Object: "以 JSON 对象编辑键值。",
        DataFrame:
          '以 JSON 填写表格内容：{"col_1":[1,2],"col_2":[3,4]}，各列长度须一致。数据随当前图保存。',
        DataSeries: '单列 JSON：{"列名":[1,2,3]}。数据随当前图保存。',
      },
      errors: {
        invalidJson: "JSON 格式无效",
        notArray: "请输入 JSON 数组",
        notObject: "请输入 JSON 对象",
        notDataFrameContent: "DataFrame 需为列名 → 值数组的对象，且各列长度一致",
        notDataSeriesContent: 'DataSeries 需为仅含一列的列式对象 {"列名":[...]}',
      },
    },
    delete: {
      title: "删除{{itemType}}",
      message: "确定要删除{{itemType}}「{{name}}」吗？",
      button: "删除{{itemType}}",
    },
    pinEditor: {
      inputs: "输入",
      outputs: "输出",
      noInputs: "无输入",
      noOutputs: "无输出",
      newPin: "新 Pin",
      containerTooltip: "容器：{{container}}（点击切换）",
      containerNone: "无",
    },
    log: {
      types: {
        application: "APP",
        execution: "EXEC",
        system: "SYS",
        graph: "GRAPH",
        data: "DATA",
        ui: "UI",
      },
    },
    configuration: {
      updateFailed: "无法更新配置，请检查字段值。",
    },
    nodeDoc: {
      inputs: "输入",
      outputs: "输出",
      noInputs: "无输入",
      noOutputs: "无输出",
      unconnected: "未连接",
      selectInput: "选择输入",
      addConnection: "添加连接",
      removeConnection: "移除连接",
      connectionFailed: "连接更新失败",
      addPort: "新增 {{name}} Pin",
      removePort: "移除 {{name}} Pin",
      portInstanceFailed: "Pin 更新失败",
      documentation: "文档",
      unnamed: "（未命名）",
      view: "查看",
    },
  },
};
