...
import { ProjectData } from '@/shared/types/domain';
...
export async function initProjectSync(): Promise<ProjectData | null> {
  ...
  if (status === LoadStatus.Ready) {
    const { variables, graphs, databases } = useProjectStore.getState();
    return {
      variables,
      graphs,
      databases,
...
export function getProjectData(): ProjectData {
  const { status, variables, graphs, databases } = useProjectStore.getState();
