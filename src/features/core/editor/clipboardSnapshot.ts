...
export function buildClipboardSnapshot(nodeIds: string[], graphId: string): ClipboardSnapshot | null {
  if (nodeIds.length === 0 || !graphId) return null;
  ...
  for (const nodeId of nodeIds) {
    const node = dataStore.getGraphNode(graphId, nodeId);
    ...
    const pinIds = dataStore.getGraphNodePins(graphId, nodeId);
    const pins: ClipboardPinEntry[] = [];

    for (const pinId of pinIds) {
      const pin = dataStore.getGraphPin(graphId, pinId);
      if (!pin) continue;
  ...
  for (const pinId of allSelectedPinIds) {
    const connIds = dataStore.getGraphPinConnections(graphId, pinId);
    for (const connId of connIds) {
      if (seenConnIds.has(connId)) continue;
      seenConnIds.add(connId);
      const conn = dataStore.getGraphConnection(graphId, connId);
      if (!conn) continue;
