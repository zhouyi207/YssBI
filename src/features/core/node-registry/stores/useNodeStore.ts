...
interface NodeStore {
  ...
  // Lifecycle
  initTab: (tabId: string, nodes: Node[], variables: Record<string, Variable>) => void;
  clearTabs: () => void;

  // Nodes
  setNodes: (tabId: string, nodes: Node[]) => void;
  updateNode: (tabId: string, nodeId: string, updater: (prev: Node) => Node) => void;
  ...
  // Getters
  getNodes: (tabId: string) => Node[];
}

const createTabState = (nodes: Node[] = [], variables: Record<string, Variable> = {}): TabState => ({
  nodes,
  variables,
  history: { past: [], future: [] }
});
