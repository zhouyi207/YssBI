...
export function fitWorldBounds(
  ...
  const padding = Number.isFinite(options.padding) ? Math.max(0, options.padding ?? DEFAULT_PADDING) : DEFAULT_PADDING;
  const minScale = positiveFinite(options.minScale ?? EDITOR_VIEWPORT_SCALE_LIMITS.min, EDITOR_VIEWPORT_SCALE_LIMITS.min);
  const maxScale = Math.max(
    minScale,
    positiveFinite(options.maxScale ?? EDITOR_VIEWPORT_SCALE_LIMITS.max, EDITOR_VIEWPORT_SCALE_LIMITS.max),
  );
