...
const sourcePin = {
  id: 'source', nodeId: 'node-a', name: 'Source', type: 'object', direction: 'output',
...
describe('connectPreview scoped projection', () => {
  it('reads only the exact graphPath and groupId with concurrent interactions', () => {
    ...
    expect(getConnectPreview({ graphPath: 'events/one', groupId: 'group-a' })).toMatchObject({
      active: true, worldX: 10, groupId: 'group-a', highlightedConnectionIds: ['connection-a'],
    });
    expect(getConnectPreview({ graphPath: 'events/two', groupId: 'group-b' })).toMatchObject({
      active: true, worldX: 20, groupId: 'group-b', highlightedConnectionIds: ['connection-b'],
    });
    expect(getConnectPreview({ graphPath: 'events/one', groupId: 'group-b' })).toMatchObject({ active: false });
  });
  ...
  it('replaces the visual owner when another pane starts interaction for the same graph', () => {
    ...
    expect(getConnectPreview({ graphPath: 'events/shared', groupId: 'group-a' }).active).toBe(false);
