...
type PinPointerModifiers = Pick<MouseEvent, "button" | "altKey" | "ctrlKey" | "metaKey">;

export function resolvePinPointerAction(
  event: Partial<PinPointerModifiers> & Pick<PinPointerModifiers, "button">,
  capability: Pick<
    NonNullable<PinData["connections"]>,
    "current" | "canAppend" | "canReplace" | "canMove"
  >,
): PinPointerAction {
...
export function useCanvasInteraction({
  ...
  const onNodePointerDown = useCallback(
    (nodeId: string, event: React.PointerEvent, groupId?: string) => {
  ...
  const onPinPointerDown = useCallback(
    async (pin: Pin, event: React.PointerEvent, groupId?: string) => {
      ...
      const world = getCanvasWorldPoint(
        gid,
        graphPath,
        event.clientX,
        event.clientY,
