...
export function subscribeDragPreview(scope: CanvasPreviewScope, listener: () => void): () => void {
  const initialState = useGraphInteractionStore.getState();
  let previousInteraction = getCanvasInteraction(initialState, scope.graphPath, scope.groupId);
  ...
  return useGraphInteractionStore.subscribe((state) => {
    ...
    const settledOverridesChanged =
      nextInteraction.type !== "draggingNodes" && nextOverrides !== previousOverrides;
