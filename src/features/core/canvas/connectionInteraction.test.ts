...
describe('connectionInteraction', () => {
  ...
  it.each([
    ...
    ['replace', source, pin({ ...target, connections: { ...capability, current: 1, canAppend: false, canReplace: true } }), { kind: 'replace', displacedConnectionIds: ['old'] }],
    ['full bounded', source, pin({ ...target, connections: { ...capability, current: 1, canAppend: false, canReplace: false } }), { kind: 'invalid', reason: 'capacity' }],
    ['orphan', source, pin({ ...target, orphan: true }), { kind: 'invalid', reason: 'orphan' }],
    ['same port', source, source, { kind: 'invalid', reason: 'same-port' }],
    ['same node', source, pin({ ...target, nodeId: source.nodeId }), { kind: 'invalid', reason: 'same-node' }],
    ['direction', source, pin({ ...target, direction: 'output' }), { kind: 'invalid', reason: 'same-direction' }],
    ['kind', source, pin({ ...target, type: 'exec', kind: 'control', dataType: undefined }), { kind: 'invalid', reason: 'kind-mismatch' }],
    ['type', source, pin({ ...target, dataType: { kind: 'String' } }), { kind: 'invalid', reason: 'type-mismatch' }],
    ['control', pin({ ...source, type: 'exec', kind: 'control', dataType: undefined }), pin({ ...target, type: 'exec', kind: 'control', dataType: undefined }), { kind: 'append' }],
    ['effect', pin({ ...source, type: 'exec', kind: 'effect', dataType: undefined }), pin({ ...target, type: 'exec', kind: 'effect', dataType: undefined }), { kind: 'append' }],
  ] as const)('maps %s compatibility into interaction feedback', (_label, candidateSource, candidateTarget, expected) => {
  ...
  it('merges, sorts, and deduplicates both occupied endpoint incumbents for visuals', () => {
    ...
    expect(resolveConnectionFeedback(occupiedSource, occupiedTarget, {
  ...
  it('snaps to the nearest valid candidate with stable id tie-breaking', () => {
    const result = resolveConnectionTarget({
      ...
      sourceConnectionIds: [],
      candidates: [
        { pin: pin({ id: 'z', nodeId: 'z-node', direction: 'input' }), center: { x: 12, y: 10 }, connectionIds: [] },
        { pin: pin({ id: 'a', nodeId: 'a-node', direction: 'input' }), center: { x: 12, y: 10 }, connectionIds: [] },
        { pin: pin({ id: 'invalid', nodeId: 'a', direction: 'input' }), center: { x: 10, y: 10 }, connectionIds: [] },
      ],
    });
  ...
  it('returns no hover or feedback when every candidate is outside the explicit radius', () => {
    const result = resolveConnectionTarget({
      ...
      candidates: [
        ...
        { pin: pin({ id: 'invalid', nodeId: 'a', direction: 'input' }), center: { x: 70, y: 70 }, connectionIds: [] },
      ],
    });
