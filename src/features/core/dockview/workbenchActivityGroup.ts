...
import { WORKBENCH_ACTIVITY_GROUP_ID, WORKBENCH_EDGE_GROUP_IDS } from "./workbenchDockviewDefaults";
...
export function isWorkbenchActivityPanel(panel: ActivityPanelLike | undefined): boolean {
...
function sourceGroupContainsPersistentView(
  ...
  return (
    sourceGroup?.panels.some((panel) => persistentSidebarMetadata(panel) !== undefined) === true
  );
}

export function shouldAllowWorkbenchActivityDrop(event: WorkbenchActivityDropEvent): boolean {
  const transfer = event.getData();
  ...
  const sourceIsActivity =
    isWorkbenchActivityPanel(source) || transfer?.groupId === WORKBENCH_ACTIVITY_GROUP_ID;
  const targetIsActivity =
    targetGroupId(event) === WORKBENCH_ACTIVITY_GROUP_ID || isWorkbenchActivityPanel(event.panel);
  if (sourceIsPersistent) {
    return (
      targetPosition(event) === "right" || targetGroupId(event) === WORKBENCH_EDGE_GROUP_IDS.right
    );
  }
  if (!sourceIsActivity && !targetIsActivity) return true;

  const allowedReorder =
    transfer !== undefined &&
    transfer.viewId === event.api.id &&
    transfer.panelId !== null &&
    transfer.tabGroupId === undefined &&
...
export function vetoInvalidWorkbenchActivityDrop(event: WorkbenchActivityDropEvent): void {
