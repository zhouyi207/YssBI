...
function validateSerializedPanel(
  ...
  if (
    !isOptionalFiniteNumber(value.minimumWidth) ||
  ...
  if (
    ...
    domains.has(value.params.domain)
  )
    return false;
...
export function isValidLogsDockviewLayout(candidate: unknown): candidate is SerializedDockview {
  ...
  for (const [panelId, panel] of Object.entries(candidate.panels)) {
    if (!isNonEmptyString(panelId) || !validateSerializedPanel(panelId, panel, domains))
      return false;
  }
  if (
    domains.size !== LOG_DOMAIN_ORDER.length ||
    LOG_DOMAIN_ORDER.some((domain) => !domains.has(domain))
  )
    return false;
