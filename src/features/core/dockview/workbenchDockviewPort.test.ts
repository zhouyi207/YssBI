...
import { createWorkbenchDockviewPort } from './workbenchDockviewInternal';
import { WorkbenchLayoutError } from './workbenchDockviewPort';
...
class FakeWorkbenchDockview {
  ...
  readonly getEdgeGroup = vi.fn((position: EdgeGroupPosition) =>
    this.edges.get(position)?.group.api);
  ...
  constructor() {
    ...
    this.api = {
      ...
      get activePanel() { return self.activePanelRecord?.panel; },
      get activeGroup() { return self.activeGroupRecord?.dockviewGroup; },
      ...
      addGroup: (options?: AddGroupOptions) => self.addGridGroup(
  ...
  layout(): SerializedDockview {
    const gridGroups = [...this.groupRecords.values()]
    ...
    const root = leaves.length === 1
      ? leaves[0]
      : { type: 'branch' as const, data: leaves, size: 100 };
    const edgeGroups = Object.fromEntries([...this.edges.entries()].map(([position, edge]) => [
      position,
      {
        size: position === 'left' || position === 'right'
          ? edge.group.width
          : edge.group.height,
    ...
    return {
      ...
      panels: Object.fromEntries([...this.panelRecords.entries()].map(([id, record]) => [
        id,
        {
          id,
  ...
  private createGroup(id: string, location: WorkbenchLocation): FakeGroup {
    ...
    const groupApi = {
      ...
      get isActive() { return fake.activeGroupRecord === group; },
    ...
    group = {
      get activePanel() { return state.activePanel; },
      set activePanel(panel) { state.activePanel = panel; },
      get collapsed() { return state.collapsed; },
      set collapsed(value) { state.collapsed = value; },
      get headerPosition() { return state.headerPosition; },
      set headerPosition(value) { state.headerPosition = value; },
      get height() { return state.height; },
      set height(value) { state.height = value; },
      get width() { return state.width; },
      set width(value) { state.width = value; },
      id,
      location,
      panels: state.panels,
  ...
  private addPanel(options: AddPanelOptions<Record<string, unknown>>): IDockviewPanel {
    ...
    const group = (referenceId ? this.groupRecords.get(referenceId) : undefined)
    ...
    const panel = {
      id: options.id,
      get group() { return record.group.dockviewGroup; },
      get params() { return record.params; },
      get title() { return record.title; },
      get isPinned() { return record.pinned; },
      api: panelApi,
  ...
  private setActiveGroup(group: FakeGroup | undefined): void {
    ...
    const selected = group?.activePanel
      ? this.panelRecords.get(group.activePanel.id)
      : undefined;
  ...
  private movePanel(
    ...
    request: FakeMoveRequest,
  ): void {
    const reference = request.group
      ? this.groupRecords.get(request.group.id)
      : record.group;
    ...
    record.group = target;
    const index = Math.min(Math.max(request.index ?? target.panels.length, 0), target.panels.length);
...
describe('workbench Dockview port', () => {
  ...
  it('publishes native layout, active, and collapse events and rebinds edge listeners', async () => {
    ...
    let notifications = 0;
    port.subscribe(() => { notifications += 1; });
    ...
    expect(port.listGroupPanels(logs.groupId).map((panel) => panel.panelInstanceId))
      .toEqual([output.panelInstanceId, logs.panelInstanceId]);
    ...
    const collapseSubscriptions = fake.edge('bottom').group.collapsedChange.subscribe.mock.calls.length;
    fake.layoutFromJson.emit();
    expect(fake.edge('bottom').group.collapsedChange.subscribe.mock.calls.length)
      .toBeGreaterThan(collapseSubscriptions);
  ...
  it('keeps duplicate editor identity, canonical remaps, pinning, and role filters separate', async () => {
    ...
    expect(port.findEditorPanelsByResource(editorRequest.resourceRef)
      .map((panel) => panel.panelInstanceId)).toEqual([
      first.panelInstanceId,
      second.panelInstanceId,
    ]);
    expect(port.getActiveEditorPanel()).toBeUndefined();
    expect(port.listGroupPanels(first.groupId).map((panel) => panel.metadata.role))
      .toEqual(['editor', 'editor', 'result']);

    expect(await port.remapResource(
      editorRequest.resourceRef,
      'events/renamed.yssbi-event',
    )).toBe(2);
  ...
  it('revalidates commit tokens inside the FIFO and resolves whenIdle after queued work', async () => {
    ...
    let idle = false;
    const idlePromise = internal.whenIdle().then(() => { idle = true; });
  ...
  it('rejects old-bound queued work immediately when its root unbinds', async () => {
    ...
    const queuedOutcome = port.openEditor({
      ...editorRequest,
  ...
  it('authorizes committed removal inside the FIFO and treats denial or throw as stale', async () => {
    ...
    expect(await internal.commitRemove([{
  ...
  it('maps Dockview close failures to a stable error with the panel ID', async () => {
    ...
    const error = await internal.commitRemove([{
      panelInstanceId: panel.panelInstanceId,
      groupId: panel.groupId,
      metadata: panel.metadata,
    }]).then(
  ...
  it('rejects a suspended publication invalidated before live apply', async () => {
    ...
    const started = new Promise<void>((resolve) => { markStarted = resolve; });
    const publication = internal.runPublicationTransaction(async (transaction) => {
      transaction.remapResource(
        editorRequest.resourceRef,
        'events/stale-operation.yssbi-event',
      );
      markStarted();
      await gate;
    });
  ...
  it('installs hydration defaults before draining queued application work', async () => {
    ...
    port.subscribe(() => { observedHydrationStates.push(port.isHydrated); });
    let externalSettled = false;
    const externalOpen = port.openEditor({
      ...editorRequest,
  ...
  it('keeps shadow and serialized active selection coherent through reveal, activate, and remove', async () => {
    ...
    await internal.runLayoutTransaction((transaction) => {
      ...
      expect(transaction.listGroups().find((group) => group.groupId === second.groupId))
        .toMatchObject({
          active: true,
  ...
  it('reconciles an active empty central group after removing the active sole panel', async () => {
    ...
    const shadow = await internal.runLayoutTransaction((transaction) => {
      ...
      const centralGroup = transaction.listGroups().find(
        (group) => group.groupId === centralGroupId,
      );
  ...
  it('replays non-last removals before dependent moves and defers last-panel closes', async () => {
    ...
    await internal.runLayoutTransaction((transaction) => {
      transaction.removePanels([
        first.panelInstanceId,
        soleSecondary.panelInstanceId,
      ]);
      expect(transaction.move({
        panelInstanceId: second.panelInstanceId,
        groupId: first.groupId,
        index: 1,
        activate: false,
      })).toBe(true);
      expect(transaction.listGroupPanels(first.groupId)
        .map((panel) => panel.panelInstanceId)).toEqual([
    ...
    expect(port.listGroupPanels(first.groupId)
      .map((panel) => panel.panelInstanceId)).toEqual([
      third.panelInstanceId,
      second.panelInstanceId,
    ]);
  ...
  it('defers subscriber callbacks until buffered live commands reach the final state', async () => {
    ...
    const unsubscribe = port.subscribe(() => {
      observedOrders.push(port.listGroupPanels(first.groupId)
  ...
  it('rejects a missing publication removal before later business work runs', async () => {
    const fake = createFakeWorkbenchDockview();
    const { internal } = createWorkbenchDockviewPort();
    internal.bind(fake.api);
    internal.completeHydration();
    let businessCommitted = false;

    await expect(internal.runPublicationTransaction(async (transaction) => {
  ...
  it('maps Dockview snapshot and validation serialization failures to stable errors', async () => {
    ...
    const snapshotPort = createWorkbenchDockviewPort();
    ...
    const validationPort = createWorkbenchDockviewPort();
  ...
  it('applies no live commands when a transaction callback or shadow validation fails', async () => {
    ...
    await expect(internal.runLayoutTransaction((transaction) => {
      transaction.move({
        panelInstanceId: first.panelInstanceId,
        groupId: first.groupId,
        index: -1,
  ...
  it('keeps lifecycle, hydration install, and restore capabilities off public interfaces', () => {
    ...
    expect(publicDockview).not.toHaveProperty('createWorkbenchDockviewPort');
