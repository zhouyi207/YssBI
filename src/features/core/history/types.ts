...
export interface CommandHandler<TArgs = unknown, TContext = unknown> {
  ...
  execute(graphId: string, args: TArgs): Promise<TContext> | TContext;
  /** Reverse the operation using saved context */
  undo(graphId: string, context: TContext): Promise<void>;
  /** Re-apply after undo (may differ from initial execute) */
  redo(graphId: string, context: TContext): Promise<void>;
}
