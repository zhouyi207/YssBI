...
it("adopts compilation without saving and saves only for the explicit save action", async () => {
  const document = structuredClone(current().document);
  document.nodes[nodeId].parameters.unsaved = true;
  useGraphDraftStore
    .getState()
    .applyTransform(graphPath, { changed: true, document, projection: projection() });
  vi.spyOn(HarnessGraphToolsService, "prepare").mockImplementation(async (r, doc) => {
    if (r.capabilityId === "compile_graph")
      return {
        type: "compilation",
        update: {
          type: "ready",
    ...
    return {
      ...
      update: {
        ...
        resourceRevision: 2,
        document: doc,
        projectionReplacement: { graphPath, projection: projection() },
      },
    };
  });
  await applyAssistantGraphTool(request("compile", "compile_graph"), () => true);
  expect(current().compiledArtifactId).toBe("a".repeat(64));
  expect(current().saveDirty).toBe(true);
  expect(current().undoStack).toHaveLength(1);
  await applyAssistantGraphTool(request("save", "save_graph"), () => true);
  expect(current().saveDirty).toBe(false);
  expect(current().saving).toBe(false);
  expect(current().savedDocument.nodes[nodeId].parameters).toEqual({ unsaved: true });
});
