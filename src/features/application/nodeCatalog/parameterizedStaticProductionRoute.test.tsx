...
const fixture = JSON.parse(
  readFileSync(resolve("src/tests/fixtures/parameterized-static-production-route.json"), "utf8"),
...
describe("ParameterizedStatic production route", () => {
  it("uses one Rust-authoritative fixture from Catalog descriptor through ordered editor submit", async () => {
    ...
    const projectItem = catalog.items.find((item) => item.nodeTypeId === "yssbi.dataframe.project");
    ...
    let mutationCalls = vi
      .mocked(invoke)
      .mock.calls.filter(([command]) => command === "mutate_graph_document");
    ...
    const projectNode = useGraphDataStore.getState().getGraphNode(fixture.graphPath, createdNodeId);
    ...
    const updated = useGraphDataStore.getState().getGraphNode(fixture.graphPath, createdNodeId);
    expect(
      updated?.parameterEditors?.find((editor) => editor.key === "columns")?.configuration,
