...
export type GraphMutationRejectionCode = Exclude<GraphMutationErrorCode, "graph_revision_conflict">;

type GraphMutationErrorMessageKey = `canvas.connection.errors.${GraphMutationErrorCode}`;

const ERROR_MESSAGE_KEYS: Record<GraphMutationErrorCode, GraphMutationErrorMessageKey> =
  Object.fromEntries(
    GRAPH_MUTATION_ERROR_CODES.map((code) => [code, `canvas.connection.errors.${code}`]),
...
export function graphMutationErrorCode(error: unknown): GraphMutationErrorCode | null {
  ...
  return error.code in ERROR_MESSAGE_KEYS ? (error.code as GraphMutationErrorCode) : null;
}

export function graphMutationErrorMessageKey(code: string): GraphMutationErrorMessageKey | null {
  return code in ERROR_MESSAGE_KEYS ? ERROR_MESSAGE_KEYS[code as GraphMutationErrorCode] : null;
}
