...
export function useBayesArtifacts({
  ...
  const read = useCallback(
    async <T>(fallbackCode: string, operation: () => Promise<T>): Promise<BayesReadOutcome<T>> => {
      const request = begin();
  ...
  const readDensity = useCallback(() => {
    ...
    return read("bayes_density_read_failed", () =>
      readBayesDensityPlotData(taskId, parameter, 256),
    );
  }, [parameter, read, result, taskId]);
  ...
  const readAutocorrelation = useCallback(() => {
    ...
    return read("bayes_autocorrelation_read_failed", () =>
      readBayesAutocorrelationData(taskId, parameter, 50),
    );
  }, [parameter, read, result, taskId]);
  ...
  const readPosteriorPredictive = useCallback(() => {
    ...
    return read("bayes_posterior_predictive_read_failed", () =>
...
function revisionGeneration(revisions: Readonly<Record<string, number>>): string {
  return JSON.stringify(
    Object.entries(revisions).sort(([left], [right]) => left.localeCompare(right)),
  );
}
