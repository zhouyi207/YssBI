...
export function useBayesModelDraft(initialDraft: BayesModelDraftDTO = createDefaultBayesDraft()) {
  ...
  const updateModelEquation = async (
    formulaText: string,
    ...
    try {
      ...
      setDraft((current) =>
        rebuildDraft(applyParsedFormula({ ...current, likelihood }, response.formula)),
      );
      return true;
    } catch (caught) {
      if (generation !== formulaRequestGeneration.current) return false;
  ...
  const rebuildDraft = (next: BayesModelDraftDTO): BayesModelDraftDTO => {
    ...
    const symbols = createSymbolDrafts(rawSymbols, next.symbols, datasetColumns).map((symbol) => {
      ...
      return symbol.role === "dependent" ? { ...symbol, role: "independent" as const } : symbol;
    });
    ...
    const responseBinding =
      next.responseBinding && dependentSymbols.has(next.responseBinding.symbol)
  ...
  const next: BayesModelDraftDTO = {
    ...
    responseBinding:
      role === "dependent"
        ...
        : draft.responseBinding?.symbol === name
          ? null
...
function applySymbolDataBinding(
  draft: BayesModelDraftDTO,
  name: string,
  column: string,
): BayesModelDraftDTO {
...
function applyDatasetDefaults(draft: BayesModelDraftDTO): BayesModelDraftDTO {
  ...
  const responseColumn = dependentSymbol
    ...
    : null;
  const responseBinding =
    dependentSymbol && responseColumn
  ...
  for (const symbol of draft.symbols.filter((symbol) => symbol.role === "independent")) {
    if (
      dataBindings[symbol.name] &&
      dataset.columns.some((column) => column.name === dataBindings[symbol.name])
    ) {
      continue;
    }
...
function preferredColumn(
  ...
  const exact = dataset.columns.find(
    (column) => column.name === symbolName && allowedTypes.includes(column.dtype),
  );
