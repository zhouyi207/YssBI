...
function setup(): {
  ...
  const dependencies: DatabaseMetadataCoordinatorDependencies = {
    project: {
      ...
      isCurrent: (identity) => identity.projectInstanceId === currentProject.projectInstanceId
...
describe('DatabaseMetadataCoordinator', () => {
  it('publishes neither stale success nor stale failure after project replacement', async () => {
    ...
    await expect(completion).resolves.toEqual({ status: 'stale' } satisfies DatabaseMetadataOutcome);
  ...
  it('only publishes the newest request for one database while another key remains independent', async () => {
    ...
    fixture.reader.read = async (_projectId, id) => requests.get(id)?.shift()?.promise
      ?? database(id, 'unexpected');
    ...
    await expect(newCompletion).resolves.toEqual({ status: 'published' } satisfies DatabaseMetadataOutcome);
    await expect(inventoryCompletion).resolves.toEqual({ status: 'published' } satisfies DatabaseMetadataOutcome);
    await expect(oldCompletion).resolves.toEqual({ status: 'stale' } satisfies DatabaseMetadataOutcome);
