...
function entry(resultId: string, state: PinResultEntry['state'], createdAtMs: string): PinResultEntry {
  return {
    resultId,
    runId: `run-${resultId}`,
...
describe('PinHistoryMenu', () => {
  ...
  afterEach(() => {
    act(() => root.unmount());
    container.remove();
    document.querySelectorAll('[data-slot="dropdown-menu-content"]').forEach((node) => node.remove());
  });
  ...
  it('opens the selected historical exact result ID', async () => {
    act(() => root.render(
      <PinHistoryMenu graphPath={graphPath} outputs={[output]} label="View" />,
    ));
    await act(async () => {
      const trigger = container.querySelector<HTMLButtonElement>('button');
      trigger?.dispatchEvent(new PointerEvent('pointerdown', { bubbles: true, button: 0 }));
