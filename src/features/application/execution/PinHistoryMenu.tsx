...
export function PinHistoryMenu({ graphPath, outputs, label, className }: PinHistoryMenuProps) {
  ...
  const loadHistory = useCallback(async () => {
    ...
    try {
      const histories = await Promise.all(
        outputs.map(async (output): Promise<PinHistoryProjection> => {
  ...
  const openEntry = useCallback(
    (projection: PinHistoryProjection, resultId: string) => {
                ...
                <span className="flex items-center gap-1">
                  ...
                  {latest ? (
                    <span className="text-[10px] text-primary">
                      {t("contextMenu.pin.historyLatest")}
                    </span>
                  ) : null}
                </span>
