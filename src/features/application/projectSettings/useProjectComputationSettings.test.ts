...
vi.mock("@/features/application/project/projectIOStore", () => ({
  useProjectIOStore: <T>(selector: (state: { projectInstanceId: string | null }) => T): T =>
    selector({
...
vi.mock("@/features/core/projectLifecycle/projectLifecycleAuthority", () => ({
  ...
  isCurrentProjectIdentity: vi.fn(
    (identity: { projectInstanceId: string; epoch: number }) =>
...
function snapshot(
  overrides: Partial<ComputationSettingsSnapshotDto> = {},
): ComputationSettingsSnapshotDto {
  return {
...
describe("useProjectComputationSettings", () => {
  ...
  it("applies one correlated revisioned mutation and reconciles its event echo once", async () => {
    ...
    vi.mocked(ProjectService.updateProjectComputationSettings).mockImplementation(
      async (request) => {
    ...
    expect(ProjectService.updateProjectComputationSettings).toHaveBeenCalledWith(
  ...
  it("ignores mismatched operation receipts and older event revisions", async () => {
    ...
    await act(async () => {
      await expect(current?.apply()).rejects.toThrow(/correlation/i);
    });
    expect(current?.confirmed?.settingsRevision).toBe(3);
  ...
  it("rejects a correlated direct receipt whose settings revision does not advance", async () => {
    ...
    await act(async () => {
      await expect(current?.apply()).rejects.toThrow(/revision/i);
    });
    expect(current?.confirmed?.settingsRevision).toBe(3);
