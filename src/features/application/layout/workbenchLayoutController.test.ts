...
function createFakeRoot(
  initialLayout: SerializedDockview | undefined,
  order: string[],
) {
...
function createFakePort(
  ...
  const move = vi.fn(async (request: { panelInstanceId: string; groupId: string; index?: number }) => {
  ...
  const layoutMove = vi.fn((request: {
  ...
  const runLayoutTransaction = vi.fn(async (
    operation: (transaction: WorkbenchLayoutTransaction) => unknown,
  ) => {
  ...
  const internal = {
    ...
    unbind: vi.fn(() => { order.push('internal.unbind'); }),
  ...
  return {
    ...
    setIdle(next: Promise<void>): void { idle = next; },
    setSerialized(next: SerializedDockview): void { serialized = structuredClone(next); },
  };
}
...
describe('WorkbenchLayoutController hydration', () => {
  ...
  it('installs startup defaults behind hydration before queued application work', async () => {
    ...
    let hydrated = false;
    const hydration = harness.controller.whenHydrated().then(() => { hydrated = true; });
  ...
  it('restores a valid root while replacing only invalid Logs with defaults', async () => {
    ...
    expect(harness.root.fromJSON).toHaveBeenCalledWith(
      savedRoot,
...
describe('WorkbenchLayoutController persistence', () => {
  ...
  it('waits for current hydration and FIFO idle before a direct close-time write', async () => {
    ...
    vi.spyOn(harness.logsController, 'captureBoundSnapshot')
      .mockImplementation(() => { harness.order.push('logs.capture'); });
    harness.controller.bind(harness.root.api, 'main');
