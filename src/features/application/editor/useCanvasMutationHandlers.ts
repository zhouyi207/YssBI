...
function toCanvasMutationOutcome(
  outcome: Awaited<ReturnType<typeof executeSafeGraphMutationOutcome>>,
...
export function createCanvasMutationHandlers(): CanvasInteractionHandlers {
  ...
  return {
    async submitConnection({ graphPath, intent, sourcePinId, targetPinId }) {
      const outcome =
        intent === "connect"
          ? await executeSafeGraphMutationOutcome(graphPath, "Canvas connect", "ConnectPins", {
              pinA: sourcePinId,
              pinB: targetPinId,
            })
    ...
    async disconnectPort(graphPath, pinId) {
      return toCanvasMutationOutcome(
        await executeSafeGraphMutationOutcome(graphPath, "Alt disconnect port", "DisconnectPort", {
          pinId,
        }),
      );
    },
