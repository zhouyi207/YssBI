...
vi.mock("@/features/core/projectLifecycle/projectLifecycleAuthority", () => ({
  ...
  isCurrentProjectIdentity: (identity: Readonly<{ projectInstanceId: string; epoch: number }>) =>
...
describe("editor command focus", () => {
  ...
  it.each([
    [
      "panel",
      () => {
        mocks.activePanel = editorPanel({ panelInstanceId: "editor-b" });
      },
    ],
    [
      "group",
      () => {
        mocks.activePanel = editorPanel({ groupId: "group-b" });
      },
    ],
    [
      "resource path",
