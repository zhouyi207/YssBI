  ...
  switch (expression.type) {
    ...
    case 'binary': {
      ...
      switch (expression.op) {
        ...
        case 'div': return `\\frac{${formatRawExpressionLatex(expression.left)}}{${formatRawExpressionLatex(expression.right)}}`;
        case 'pow': return `${left}^{${formatRawExpressionLatex(expression.right)}}`;
      }
    }
    case 'call': {
      ...
      const name = expression.function === 'min' || expression.function === 'max'
