...
describe("searchLocalizedCatalogItems", () => {
  it.each([
    ...
    [
      "technical term full pinyin",
      item({ nodeTypeId: "technical-pinyin", technicalTerms: ["技术术语"] }),
      "ji shu shu yu",
    ],
    [
      "technical term pinyin initials",
      item({ nodeTypeId: "technical-initials", technicalTerms: ["技术术语"] }),
      "jssy",
    ],
    ["stable node ID", item({ nodeTypeId: "yssbi.hidden.stable-id" }), "hidden.stable-id"],
    [
      "backend search text",
      item({ nodeTypeId: "backend", backendSearchText: ["backend-only-secret"] }),
      "backend-only-secret",
    ],
    [
      "resource name",
      item({ nodeTypeId: "resource", resourceNames: ["季度销售数据"] }),
      "季度销售",
    ],
    ...
    [
      ...
      item({ nodeTypeId: "mixed", title: "整数 AddParser" }),
      "zs addparser",
    ],
