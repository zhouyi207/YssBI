...
describe('buildLocalizedCatalogTree', () => {
  ...
  it('uses the stable item identity as a deterministic title tie-breaker', () => {
    ...
    expect(tree.find((node) => node.category.categoryId === 'output')?.items
      .map(catalogItemKey)).toEqual([
        catalogItemKey(first),
        catalogItemKey(second),
      ]);
  });
  ...
  it('flattens only expanded categories in the same item-before-child order as Canvas', () => {
    ...
    expect(rows.map((row) => `${row.kind}:${row.kind === 'category'
      ? row.category.categoryId
      : row.item.nodeTypeId}`)).toEqual([
      'category:statistics',
  ...
  it('returns all populated ancestors for search-driven expansion', () => {
    ...
    expect(collectLocalizedCatalogCategoryIds(tree)).toEqual(new Set([
      'statistics',
      'statistics.regression',
    ]));
  });
