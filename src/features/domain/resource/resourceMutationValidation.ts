...
function graphPathFromDelta(delta: ResourceDeltaDto): string | undefined {
  if (delta.payload.kind === 'resource_move'
    || delta.payload.kind === 'resource_lifecycle') return undefined;
...
export function validateResourceMutationResult(
  ...
  if (!Array.isArray(result.moves) || result.moves.some((move) => {
    if (!isRecord(move)
      ...
      || move.name.trim().length === 0) return true;
  ...
  if (new Set(result.deltas.map((delta) => `${delta.resource.kind}:${delta.resource.key}`)).size
    !== result.deltas.length) {
    return 'resource deltas contain duplicate targets';
  }
  if (result.deltas.some((delta) =>
    delta.causedBy !== null && delta.causedBy !== result.operationId)) {
