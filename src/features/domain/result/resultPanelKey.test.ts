...
describe("resultPanelKey", () => {
  it("reuses one logical key for newer payloads from the same output", () => {
    expect(resultPanelKey(descriptor("result-1", outputRef))).toBe(
      resultPanelKey(descriptor("result-2", outputRef)),
    );
  });

  it("keeps unrelated and output-less results distinct", () => {
    expect(resultPanelKey(descriptor("result-1", outputRef))).not.toBe(
      resultPanelKey(descriptor("result-1", otherOutputRef)),
    );
    expect(resultPanelKey(descriptor("a", null))).not.toBe(resultPanelKey(descriptor("b", null)));
