...
export const NodeContainer = React.memo<NodeContainerProps>(
  ({ node, graphPath: _graphPath, groupId, selected, dimmed, onPointerDown, children }) => {
    ...
    const hasLinks =
      graphBucket?.nodePins[node.id]?.some(
    ...
    return (
      ...
      >
        ...
        {hasError && <div className="absolute -top-1 -right-1 w-3 h-3 bg-red-500 rounded-full" />}
        {!hasError && callIssue && (
          <Tooltip>
