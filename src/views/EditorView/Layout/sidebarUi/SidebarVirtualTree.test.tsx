...
describe('SidebarVirtualTree', () => {
  ...
  function renderTree(renderRow: (row: (typeof rows)[number]) => React.ReactNode = (row) => (
    <span>{row.label}</span>
  )): void {
    act(() => root.render(
      <SidebarVirtualTree
  ...
  it('provides one treeitem tab stop and moves focus with tree navigation keys', () => {
    ...
    act(() => items[1].dispatchEvent(new KeyboardEvent('keydown', {
      key: 'ArrowUp',
      bubbles: true,
    })));
    ...
    act(() => items[2].dispatchEvent(new KeyboardEvent('keydown', {
      key: 'Home',
      bubbles: true,
    })));
    expect(document.activeElement).toBe(items[0]);
  ...
  it('does not intercept navigation keys from nested interactive controls', () => {
    ...
    firstButton!.focus();
    act(() => firstButton!.dispatchEvent(new KeyboardEvent('keydown', {
      key: 'ArrowDown',
      bubbles: true,
    })));
