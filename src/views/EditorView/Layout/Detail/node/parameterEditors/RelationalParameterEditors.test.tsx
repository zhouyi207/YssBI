...
describe("ProjectColumnsEditor", () => {
  it("submits an ordered multi-selection in the user selection order", () => {
    ...
    act(() =>
      root.render(<ProjectColumnsEditor editor={editor} errors={[]} onCommit={onCommit} />),
    );
...
describe("FilterPredicateEditor", () => {
  it.each([
    ...
    act(() =>
      root.render(<FilterPredicateEditor editor={editor} errors={[]} onCommit={onCommit} />),
    );
  ...
  it("keeps operator and Boolean defaults consistent when switching to and from Unknown", () => {
    ...
    expect(container.querySelector('[aria-label="Predicate operator"]')?.textContent).toContain(
      "equal",
    );
    expect(container.querySelector('[aria-label="Predicate value"]')?.textContent).toContain(
      "false",
    );
