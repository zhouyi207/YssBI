...
describe('NodePinInterfacePanel', () => {
  it('renders pin selectors with compatible graph options', async () => {
    ...
    expect(Array.from(container.querySelectorAll('[data-slot="collapsible-trigger"]'))
      .map((trigger) => trigger.textContent?.replace(/\s+/g, ' ').trim()))
      .toEqual(['Inputs', 'Outputs']);

    const sections = Array.from(container.querySelectorAll<HTMLElement>('[data-slot="collapsible"]'));
    expect(sections[0]?.getAttribute('data-state')).toBe('closed');
    expect(sections[1]?.getAttribute('data-state')).toBe('closed');

    await act(async () => {
      container.querySelectorAll<HTMLElement>('[data-slot="collapsible-trigger"]')[0]
    ...
    expect(Array.from(document.querySelectorAll('[data-slot="select-item"]'))
    ...
    const addButton = container.querySelector<HTMLButtonElement>('[data-testid="add-output-connection-output-result"]');
    await act(async () => addButton?.click());
    expect(container.querySelectorAll('[data-slot="select-trigger"]')).toHaveLength(3);
    expect(container.querySelectorAll('[data-testid^="remove-output-connection-output-result"]'))
      .toHaveLength(2);

    await act(async () => {
      container.querySelectorAll<HTMLElement>('[data-testid^="remove-output-connection-output-result"]')[1]
        ?.click();
    });
    expect(container.querySelectorAll('[data-slot="select-trigger"]')).toHaveLength(2);
  ...
  it('uses the output slots to add and remove exact graph connections', async () => {
    ...
    const outputItems = Array.from(document.querySelectorAll<HTMLElement>('[data-slot="select-item"]'));
    const targetItem = outputItems.find((item) => item.textContent?.trim() === 'Target node · Value');
    await act(async () => {
    ...
    expect(container.querySelector('[data-testid="remove-output-connection-output-result-0"]'))
      .not.toBeNull();
    await act(async () => {
      container.querySelector<HTMLElement>('[data-testid="remove-output-connection-output-result-0"]')
        ?.click();
    });
  ...
  it('uses the input selector to connect or clear its upstream output', async () => {
    ...
    await act(async () => {
      container.querySelector<HTMLElement>('[data-slot="collapsible-trigger"]')
        ?.dispatchEvent(new MouseEvent('click', { bubbles: true }));
    });
    await act(async () => {
      container.querySelector<HTMLElement>('[data-slot="select-trigger"]')
    ...
    const inputItems = Array.from(document.querySelectorAll<HTMLElement>('[data-slot="select-item"]'));
    const sourceItem = inputItems.find((item) => item.textContent?.trim() === 'Source node · Result');
    await act(async () => {
    ...
    const emptyItem = Array.from(document.querySelectorAll<HTMLElement>('[data-slot="select-item"]'))
