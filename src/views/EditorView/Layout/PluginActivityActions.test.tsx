...
describe('PluginActivityActions', () => {
  ...
  it('routes the manager install action to the Julia installer', async () => {
    ...
    const managerButton = host.querySelector<HTMLButtonElement>(
      '[data-workbench-plugin-manager]',
    );
    act(() => managerButton?.click());

    const installButton = [...document.body.querySelectorAll('button')].find((button) => (
      button.textContent === 'plugins.manager.install'
    ));
