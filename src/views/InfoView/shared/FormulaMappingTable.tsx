...
function formatCoef(coef: number | undefined, formatNum: (v: number, d?: number) => string): string {
    ...
    <Table className="w-full text-xs">
      <TableHeader>
        <TableRow className="border-0 hover:bg-transparent">
          <TableHead className="h-auto w-20 px-3 py-1.5 text-left font-medium text-muted-foreground">Symbol</TableHead>
          <TableHead className="h-auto px-3 py-1.5 text-left font-medium text-muted-foreground">Variable</TableHead>
          {hasCat && (
            <TableHead className="h-auto px-3 py-1.5 text-left font-medium text-muted-foreground">Category</TableHead>
          )}
          {showCoef && (
            <TableHead className="h-auto w-28 px-3 py-1.5 text-right font-medium text-muted-foreground">{coefHeader}</TableHead>
          )}
        </TableRow>
      </TableHeader>
      <TableBody>
        {mappings.map((m, idx) => (
          <TableRow key={`${m.symbol}-${m.variable}-${idx}`} className={`border-t border-border ${idx % 2 === 0 ? 'bg-muted/50' : ''}`}>
