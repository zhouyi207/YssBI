    ...
    <ReportLayout
      ...
      subtitle={
        <p className="text-xs leading-relaxed text-muted-foreground">
          Variables: {var_names.join(", ")} · Trend: {trendLabel} · Number of obs ={" "}
          {num_observation} · Lags = {n_lags}
          <br />
          Trace @5% / 1% selected rank: {selected_rank_trace_95} / {selected_rank_trace_99} · Max
          eigen @5% / 1%: {selected_rank_max_95} / {selected_rank_max_99}
        </p>
      }
    >
      <ReportSection title="Johansen tests" icon="chart">
        <InfoStatsTable
          ...
          tableClassName="font-mono text-xs text-foreground"
        >
          <TableHeader>
            <TableRow className="border-b border-border text-muted-foreground hover:bg-transparent">
              ...
              {show_max_eigen && (
                <>
                  <TableHead className="h-auto border-l border-border px-2 py-2 text-right">
                    λ_max
                  </TableHead>
                  <TableHead className="h-auto px-2 py-2 text-right">cv 10%</TableHead>
                  <TableHead className="h-auto px-2 py-2 text-right">cv 5%</TableHead>
          ...
          <TableBody>
            {rows.map((r) => {
              ...
              const starMax =
                show_max_eigen &&
                r.rank === selected_rank_max_95 &&
                r.max_eigenvalue_statistic != null;
              return (
                <TableRow key={r.rank} className="border-b border-border hover:bg-muted/40">
                  ...
                  <TableCell className="px-2 py-1.5 text-right text-muted-foreground">
                    {fmt(r.trace_crit_10pct)}
                  </TableCell>
                  <TableCell className="px-2 py-1.5 text-right text-muted-foreground">
                    {fmt(r.trace_crit_5pct)}
                  </TableCell>
                  <TableCell className="px-2 py-1.5 text-right text-muted-foreground">
                    {fmt(r.trace_crit_1pct)}
                  </TableCell>
                  {show_max_eigen && (
                    <>
                      ...
                      <TableCell className="px-2 py-1.5 text-right text-muted-foreground">
                        {fmt(r.max_eigen_crit_10pct)}
                      </TableCell>
                      <TableCell className="px-2 py-1.5 text-right text-muted-foreground">
                        {fmt(r.max_eigen_crit_5pct)}
                      </TableCell>
                      <TableCell className="px-2 py-1.5 text-right text-muted-foreground">
                        {fmt(r.max_eigen_crit_1pct)}
                      </TableCell>
                    </>
                  )}
                </TableRow>
      ...
      <p className="mt-2 text-[11px] text-muted-foreground">
        * next to trace (or max eigen) marks the row selected at 5% by Johansen’s sequential rule
        (Stata vecrank).
      </p>
    </ReportLayout>
  );
