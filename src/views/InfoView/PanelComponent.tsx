...
import type {
  ...
  PanelModelType as ModelType,
} from "./PanelFormulaBlock";
...
function getDefaultSelections(data: PanelSummaryResult): {
  model: ModelType;
  effect: EffectType;
  method: TabKey;
} {
...
export const PanelComponent: React.FC<{ data: PanelSummaryResult }> = ({ data }) => {
  ...
  const methods = useMemo(() => METHOD_MAP[modelType][effectType], [modelType, effectType]);
  ...
  const handleModelChange = (m: ModelType) => {
    ...
    const newEffect: EffectType =
      m === "mixed" ? "none" : effectType === "none" ? "entity" : effectType;
      ...
      {currentData && !currentError && (
        <>
          <ReportSection title="Model Summary" icon="modelSummary">
            {(currentMethod === "re_fgls" ||
              currentMethod === "re_mle" ||
              currentMethod === "re_fgls_time" ||
              currentMethod === "re_mle_time" ||
              currentMethod === "re_fgls_twoway" ||
              currentMethod === "re_mle_twoway") &&
            currentData?.diagnostic_info?.panel_fe_info ? (
              ...
              />
            ) : (currentMethod === "re_be" || currentMethod === "re_be_time") &&
              currentData?.diagnostic_info?.panel_fe_info ? (
              <PanelBESummaryGrid
                info={currentData.model_basic_info}
                panelFe={currentData.diagnostic_info.panel_fe_info}
