  ...
  return (
    <Button
      ...
      className={cn(
        ...
        primary && 'bg-[var(--accent-color)] text-primary-foreground hover:bg-[var(--accent-color-hover)]',
        !primary && !danger && 'border-transparent bg-transparent text-foreground hover:bg-[var(--interactive-hover)]',
    ...
    <aside className="flex w-[284px] shrink-0 flex-col border-l border-[var(--strong-border)] bg-[var(--sidebar-bg)] max-[920px]:w-[244px] max-[760px]:hidden">
          ...
          <div className="flex flex-col gap-4 p-4">
            ...
            <div className="flex flex-col gap-1">
              ...
              <ActionButton disabled={isBusy} onClick={() => onToggleFavorite(selected.id)}>
                ...
                <span>{selected.isFavorite ? t('projectPicker.unfavorite') : t('projectPicker.favorite')}</span>
              </ActionButton>
            </div>
      ...
      <div className="shrink-0 border-t border-[var(--strong-border)] p-3">
        <ActionButton disabled={isBusy} onClick={onCleanupProjects}>
          <VscClearAll size={14} />
          <span>{cleaningUp ? t('projectPicker.cleaningUp') : t('projectPicker.cleanupProjects')}</span>
        </ActionButton>
      </div>
    </aside>
  );
