...
export function useDatabaseGridSelectionAdapter({
  ...
  const handleRowSelect = useCallback(
    (rowIndex: number, modifiers: DatabaseGridSelectionModifiers) => {
  ...
  const handleColumnSelect = useCallback(
    (columnIndex: number, modifiers: DatabaseGridSelectionModifiers) => {
      const current = selectionRef.current;
      ...
      onSelectionChange(nextColumns.length > 0 ? { type: "columns", columns: nextColumns } : null);
    },
    [onSelectionChange],
  );
  ...
  const handleSelectionChanged = useCallback(
    (event: SelectionChangedEvent<DatabaseGridRow>) => {
      if (
        syncingRowSelectionRef.current ||
        event.source === "api" ||
  ...
  const handleCellMouseDown = useCallback(
    (event: CellMouseDownEvent<DatabaseGridRow>) => {
      ...
      const current = selectionRef.current;
      const retainedRanges = modifiers.additive && current?.type === "cells" ? current.ranges : [];
      const anchor = modifiers.extend
        ? (cellSelectionAnchorRef.current ??
          (current?.type === "cells" ? current.activeCell : target))
        : target;
  ...
  const handleCellMouseOver = useCallback(
    (event: CellMouseOverEvent<DatabaseGridRow>) => {
  ...
  const handleCellFocused = useCallback(
    (event: CellFocusedEvent<DatabaseGridRow>) => {
      ...
      if (columnIndex === null || event.rowIndex < 0 || event.rowIndex >= rowCountRef.current)
        return;
