...
export const RuntimeViewWindow: React.FC = () => {
  ...
  usePersistedWindow('runtimeView');

  useEffect(() => {
    if (!sourceId) {
      setSourceError(t('runtimeView.missingSourceId'));
      return;
    }
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        if (!descriptor) {
          setSourceError(t('runtimeView.noSource'));
          return;
        }
        setSourceDescriptor(descriptor);
        const windowTitle = descriptor.title || t('runtimeView.title');
  ...
  const chromeTitle = sourceDescriptor?.title ?? t('runtimeView.title');
    ...
    <div className="flex h-screen w-full flex-col overflow-hidden bg-[var(--workbench-bg)] font-sans text-[var(--workbench-fg)]">
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
        {sourceError ? (
          ...
          <UnifiedDataView payload={sourceDescriptor} layout="window" />
        ) : (
          <div className="flex flex-1 items-center justify-center text-sm text-muted-foreground">
            {t('runtimeView.loading')}
          </div>
        )}
      </div>
    </div>
