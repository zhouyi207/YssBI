...
describe('Bayes issue presentation inventory', () => {
  it('maps every audited production code in both locales', () => {
    for (const locale of [enUS, zhCN]) {
      ...
      for (const code of VALIDATION_CODES) expect(locale.bayes.validation.issues).toHaveProperty(code);
  ...
  it.each([
    ['en-US', enUS],
    ['zh-CN', zhCN],
  ] as const)('uses localized generic text plus code for unknown %s issues', async (language, locale) => {
  ...
  it('never presents raw IpcError details or raw Error text', async () => {
    ...
    expect(bayesActionErrorMessage(new Error('private transport failure'), t))
