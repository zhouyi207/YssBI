import type { BayesDatasetSelectionDTO, ParameterConstraintDTO, PriorSpecDTO } from '@/shared/types/bayes';

export function numericColumns(columns: BayesDatasetSelectionDTO['columns']): BayesDatasetSelectionDTO['columns'] {
  return columns.filter(column => column.dtype === 'number' || column.dtype === 'integer');
}

export function isPriorCompatibleWithConstraint(distribution: PriorSpecDTO['distribution'], constraint: ParameterConstraintDTO): boolean {
...
export function priorFromParts(distribution: PriorSpecDTO['distribution'], args: string[]): PriorSpecDTO {
