...
describe('staged result inspection run-event parser', () => {
  it('parses the exact inspection wire and preserves opaque outer and source identities', () => {
    expect(parseStagedResultInspectionRunEvent(stagedResultInspectionWire))
      .toEqual(stagedResultInspectionWire);
  ...
  it.each([
    ['missing run key', (event: Record<string, unknown>) => { delete event.run; }],
    ['extra outer key', (event: Record<string, unknown>) => { event.extra = true; }],
    ['extra source key', (event: Record<string, unknown>) => {
      ((event.kind as Record<string, unknown>).source as Record<string, unknown>).extra = true;
    }],
    ['missing source key', (event: Record<string, unknown>) => {
  ...
  it.each([
    ...
    1,
    null,
  ])('rejects invalid positive decimal runId %j', (runId) => {
