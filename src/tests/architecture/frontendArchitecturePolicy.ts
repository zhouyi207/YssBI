...
const VIEW_CORE_CAPABILITIES = [
  ...
  viewCoreCapability("src/features/core/statusBar/statusBarItemTypes.ts", [
...
const VIEW_DOMAIN_CAPABILITIES = [
  ...
  viewCoreCapability("src/features/domain/nodeCatalog/creationDescriptor.ts", [
    "NodeCreationDescriptor",
  ]),
...
function literalSets(membership: FrontendLiteralPolicyMembership): MutableMembershipSets {
  return new Map(
    FRONTEND_LAYERS.map((layer) => [layer, new Set(membership[layer].map(normalizeSourcePath))]),
  );
}
...
export function classifyFrontendSources(
  ...
  const normalizedSources = [
    ...new Set(sources.map(({ path }) => normalizeSourcePath(path))),
  ].sort();
