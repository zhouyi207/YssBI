...
describe("frontend state authority audit", () => {
  ...
  it("detects delegated action cycles and unresolved delegates", () => {
    const cycleA = field({
      member: "saveA",
      memberKind: "action",
      delegatesTo: "src/features/core/example/privateStore.ts::action::saveB",
    });
    const cycleB = field({
      member: "saveB",
      memberKind: "action",
      delegatesTo: "src/features/core/example/privateStore.ts::action::saveA",
    });
  ...
  it("discovers manifest members from the project snapshot and fails closed when one is absent", () => {
    ...
    withIsolatedTypeScriptProject(
      new Map(sources.map(({ path, source }) => [path, source])),
      (context) => {
