...
export const FRONTEND_EXTERNAL_DEPENDENCY_POLICY: ExternalDependencyPolicy = {
  ...
  uses: [
    ...
    allowance("pure-shared", "type-only", "clsx", [null]),
    allowance("pure-shared", "runtime", "react", [null]),
    allowance("pure-shared", "type-only", "react", [null]),
    allowance("pure-shared", "runtime", "tailwind-merge", [null]),
    allowance("services", "runtime", "@tauri-apps/api", [
      "core",
      "event",
      "webviewWindow",
      "window",
    ]),
    allowance("services", "type-only", "@tauri-apps/api", ["core"]),
    allowance("services", "type-only", "react", [null]),
    ...
    allowance("views", "type-only", "react-icons", [null, "fi", "vsc"]),
  ],
};
...
function validatePolicy(
  ...
  for (const packageName of duplicateRuntime) {
    errors.push({
      ...
      packageName,
      reason: "duplicate-runtime-declaration",
    });
  }
  const invalidDeclaredRuntime = [
    ...setDifference(expectedRuntime, declaredRuntime).map(
  ...
  for (const reason of invalidDeclaredRuntime) {
    errors.push({
      kind: "invalid-external-policy-row",
      packageName: reason.split(":").slice(1).join(":"),
      reason,
    });
  }
  ...
  for (const packageName of duplicateBuild) {
    errors.push({
      kind: "invalid-external-policy-row",
      packageName,
      reason: "duplicate-build-declaration",
    });
  }
  for (const packageName of [
    ...
    errors.push({
      kind: "invalid-external-policy-row",
      packageName,
      reason: "invalid-build-declaration",
    });
  }
  ...
  for (const row of policy.uses) {
    ...
    if (seenRows.has(key)) reason = "duplicate-row";
      ...
      row.canonicalSubpaths.length === 0 ||
      row.canonicalSubpaths.some((subpath) => !canonicalSubpath(subpath))
    )
      reason = "invalid-subpath";
    else if (duplicates(row.canonicalSubpaths.map((subpath) => subpath ?? "<root>")).length > 0) {
      reason = "duplicate-subpath";
    } else if (
      (row.mode === "runtime" || row.mode === "type-only") &&
      ...
      (row.resourceKind !== "stylesheet" ||
        ...
        !stylesheetLayers.has(row.consumerSourceFile))
    )
      reason = "invalid-build-style-consumer";
    else if (row.mode !== "build-style" && row.consumerSourceFile !== null)
      reason = "unexpected-consumer";
    if (reason !== null) {
...
function findingRule(
  ...
  const packageModeRows = policy.uses.filter(
    (row) => row.packageName === origin.packageName && row.mode === dependency.mode,
  );
  ...
  const subpathRows = resourceRows.filter((row) =>
    row.canonicalSubpaths.includes(origin.canonicalSubpath),
  );
...
export function auditFrontendExternalDependencies(
  ...
  for (const dependency of externalDependencies(dependencies)) {
    ...
    const consumerSourceFile =
      dependency.mode === "build-style" ? dependency.repositoryRelativeSourceFile : null;
