...
const matrix = labels.map((_, row) => labels.map((__, column) => {
  if (row === 0 && column === 1) return 0.5;
  if (row === labels.length - 1 && column === labels.length - 1) return 1;
  return null;
}));
const pMatrix = labels.map((_, row) => labels.map((__, column) => {
...
describe('CorrelationMatrixChart', () => {
  it('renders only available cells accessibly within a narrow chart box', () => {
    ...
    const legend = host.querySelector<SVGRectElement>(
      '[data-chart-legend="correlation-scale"]',
    );
    expect.soft(legend).not.toBeNull();
    if (legend) {
      expect.soft(translatedRightEdge(legend)).toBeLessThanOrEqual(
        Math.min(...cells.map(translatedLeftEdge)),
      );
    }
