...
import {
  ...
  hasLjungBoxStats,
} from '@/shared/types/report/correlogram';
...
export function CorrelogramChart({
  ...
  useEffect(() => {
    ...
    if (!container || data.length === 0 || !box) {
      ...
      layers.grid
        .selectAll<SVGRectElement, ConfidenceRegion>(
          'rect.correlogram-confidence-region',
        )
        .data([])
        .join('rect');
      layers.grid
        .selectAll<SVGLineElement, ConfidenceReference>(
          'line.correlogram-confidence-reference',
        )
        .data([])
        .join('line');
      layers.marks
    ...
    layers.root
      ...
      .attr(
        'transform',
        `translate(${CORRELOGRAM_MARGIN.left},${CORRELOGRAM_MARGIN.top})`,
      );
    ...
    const detachTooltip = attachMarkTooltip(
      bars as D3Onable<SVGRectElement, CorrelogramBarDTO>,
      {
        tooltip,
        position: 'anchor',
        getHtml: bar =>
          ...
          + tooltipMutedLine(
            `${valueLabel}: ${bar.value.toFixed(4)}`,
            chartTheme,
            11,
          )
          + (hasLjungBoxStats(bar)
            ? tooltipMutedLine(
                `Q(${bar.lag}): ${bar.qStat.toFixed(4)}`,
                chartTheme,
                11,
              )
              + tooltipMutedLine(
                `p-value: ${formatPValueDisplay(bar.pValue)}`,
                chartTheme,
                11,
              )
            : ''),
