...
export function VarStabilityChart({
  ...
  useEffect(() => {
    ...
    const tooltip = new PlotTooltipController(tooltipRef.current, containerRef.current);
    ...
    if (!containerRef.current || indexedData.length === 0 || !box) {
      ...
      layers.marks
        .selectAll<SVGCircleElement, IndexedVarStabilityPoint>("circle.var-eigenvalue-point")
    ...
    layers.root
      .attr("display", null)
      .attr("transform", `translate(${VAR_STABILITY_MARGIN.left},${VAR_STABILITY_MARGIN.top})`);
    ...
    layers.grid
      ...
      .attr("data-chart-reference", (radius) => (radius === 1 ? "unit-circle" : "modulus-ring"))
      .attr("data-chart-value", (radius) => radius)
      .attr("cx", centerX)
      .attr("cy", centerY)
      .attr("r", (radius) => radius * radiusScale)
      .attr("fill", "none")
      .attr("stroke", (radius) => (radius === 1 ? chartTheme.zeroLine : chartTheme.grid))
    ...
    const points = layers.marks
      ...
      .attr("r", 5)
      .attr("fill", (datum) =>
        datum.point.status === "unstable" ? seriesColors.negative : seriesColors.primary,
      )
      .attr("stroke", (datum) =>
        datum.point.status === "unstable" ? seriesColors.negative : seriesColors.primary,
      )
    ...
    const detachTooltip = attachMarkTooltip(
      points as D3Onable<SVGCircleElement, IndexedVarStabilityPoint>,
      {
        ...
        getHtml: ({ point, index }) => {
          ...
          const complexValue =
            point.im >= 0 ? `${real} + ${imaginary}i` : `${real} - ${imaginary}i`;
          return (
            tooltipStrongLine(getPointLabel(index, point), chartTheme, { size: 11 }) +
            ...
            (point.status === "unstable"
              ? tooltipStrongLine(unstableTooltipLabel, chartTheme, {
                  size: 10,
                  color: seriesColors.negative,
                })
              : "")
          );
        },
        getAriaLabel: ({ point, index }) => getPointAriaLabel(point, index),
        onEnter: (element) => select(element).attr("r", 6).attr("stroke-width", 2),
        onLeave: (element) => select(element).attr("r", 5).attr("stroke-width", 1.5),
      },
    );
