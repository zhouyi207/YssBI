...
export function CorrelationMatrixChart({
  ...
  useEffect(() => {
    ...
    const matrixIsSquare = matrix.length === labels.length
      && matrix.every(row => row.length === labels.length);
    ...
    const offsetX = CORRELATION_LEGEND_GUTTER_WIDTH
      + (matrixAreaWidth - availableSize) / 2;
    ...
    const detachTooltip = attachMarkTooltip(
      cellRects as D3Onable<SVGRectElement, CorrelationCell>,
      {
        ...
        getHtml: cell =>
          ...
          tooltipStrongLine(
            `Coefficient: ${cell.value.toFixed(3)}`,
            chartTheme,
          ) +
    ...
    layers.xAxis
      ...
      .attr('font-size', '10px')
      .text(item => item.label.length > 12
