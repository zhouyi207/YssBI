...
describe('shared statistical info renderers', () => {
  it('renders computed DID coefficients, intervals, and reference lines', () => {
    ...
    const points = [...host.querySelectorAll<SVGCircleElement>(
      '[data-chart-mark="did-coefficient"]',
    )];
    expect.soft(points).toHaveLength(3);
    expect.soft(points.map(point => Number(point.getAttribute('data-rel-time'))))
      .toEqual([-1, 0, 1]);
    expect.soft(points.map(point => Number(point.getAttribute('data-coefficient'))))
      .toEqual([-0.2, 0, 0.3]);

    const intervals = [...host.querySelectorAll<SVGLineElement>(
      '[data-chart-mark="did-confidence-interval"]',
    )];
    expect.soft(intervals).toHaveLength(2);
    expect.soft(intervals.map(interval => Number(interval.getAttribute('data-ci-low'))))
      .toEqual([-0.4, 0.1]);
    expect.soft(intervals.map(interval => Number(interval.getAttribute('data-ci-high'))))
      .toEqual([0, 0.5]);
    ...
    expect.soft(host.querySelectorAll('[data-chart-mark="did-coefficient-trend"]'))
      .toHaveLength(1);
  });
  ...
  it('renders a unit circle and one keyboard-accessible point per VAR eigenvalue', () => {
    ...
    renderChart(
      <VarStabilityChart
        ...
        yLabel="虚部"
        ariaLabel="特征根稳定性图"
        getPointLabel={index => index === 0
          ? '<img src=x onerror=alert(1)>'
          : `特征根 ${index + 1}`}
        getPointAriaLabel={(point, index) =>
          `特征根 ${index + 1}，状态 ${point.status}`}
        modulusLabel="模"
    ...
    const unitCircle = host.querySelector<SVGCircleElement>(
      '[data-chart-reference="unit-circle"]',
    );
    expect.soft(unitCircle).not.toBeNull();
    expect.soft(Number(unitCircle?.getAttribute('data-chart-value'))).toBe(1);

    const points = [...host.querySelectorAll<SVGCircleElement>(
      '[data-chart-mark="var-eigenvalue"]',
    )];
    expect.soft(points).toHaveLength(data.length);
    expect.soft(points.map(point => point.getAttribute('tabindex'))).toEqual(['0', '0']);
    expect.soft(points.map(point => point.getAttribute('aria-label'))).toEqual([
      '特征根 1，状态 stable',
      '特征根 2，状态 unstable',
    ]);
    expect.soft(points.map(point => point.getAttribute('data-status'))).toEqual([
      'stable',
      'unstable',
    ]);
    expect.soft(points.map(point => point.getAttribute('fill'))).toEqual([
