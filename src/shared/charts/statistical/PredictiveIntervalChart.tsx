...
export function PredictiveIntervalChart({
  ...
  useEffect(() => {
    ...
    const observedMarks = observedLayer
      ...
      .attr('r', 2.4)
      .attr('fill', point => point.observed < point.lower || point.observed > point.upper
    ...
    observedMarks
      ...
      .data(point => [point])
      .join('title')
      .text(point => `observation ${point.observation}\nobserved: ${point.observed}\nmean: ${point.mean}\n95% interval: [${point.lower}, ${point.upper}]`);
  ...
  return (
    <div className={cn('space-y-2', className)}>
      ...
      <div className="flex flex-wrap gap-4 text-xs text-muted-foreground" aria-label="Chart legend">
        ...
        <span className="inline-flex items-center gap-1">
          <span className="size-2 rounded-full" style={{ backgroundColor: seriesColors.secondary }} />
          Observed
        </span>
        <span className="inline-flex items-center gap-1">
          <span className="size-2 rounded-full" style={{ backgroundColor: seriesColors.highlight }} />
          Outside interval
        </span>
      </div>
    </div>
