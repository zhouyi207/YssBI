...
export function DidEventStudyChart({ points, xLabel, yLabel, ariaLabel }: DidEventStudyChartProps) {
  ...
  useEffect(() => {
    ...
    const box = resolveChartBox(size.width, size.height, DID_EVENT_STUDY_MARGIN);
    ...
    layers.root
      .attr("display", null)
      .attr("transform", `translate(${DID_EVENT_STUDY_MARGIN.left},${DID_EVENT_STUDY_MARGIN.top})`);

    const xValues = sorted.map((point) => point.rel_time);
    const xMinimum = Math.min(...xValues);
    const xMaximum = Math.max(...xValues);
    const xPadding = xMaximum === xMinimum ? 0.75 : Math.max(0.5, (xMaximum - xMinimum) * 0.08);
    const xDomain: [number, number] = [xMinimum - xPadding, xMaximum + xPadding];
    ...
    const yDomain: [number, number] = [yMinimum - yPadding, yMaximum + yPadding];
    ...
    const intervalPoints = sorted.filter(
      (point) => !point.is_reference && point.std_err > 0 && point.ci_high >= point.ci_low,
    );
    intervalLayer
  ...
  }, [ariaLabel, chartTheme, points, seriesColors.primary, size, xLabel, yLabel]);
