...
export function KdeChart({
  ...
  useEffect(() => {
    ...
    const paddedXDomain = paddedNumericDomain(
      data.map((point) => point.x),
      0.06,
      1,
    );
    const paddedYDomain = paddedNumericDomain(
      data.map((point) => point.y),
      0.06,
      0.01,
    );
    const domainMin = xMin == null ? paddedXDomain[0] : Math.max(xMin, paddedXDomain[0]);
    const yMaximum = Math.max(0, paddedYDomain[1]);
    const xScale = scaleLinear().domain([domainMin, paddedXDomain[1]]).range([0, box.plotWidth]);
    const yScale = scaleLinear().domain([0, yMaximum]).range([box.plotHeight, 0]);
