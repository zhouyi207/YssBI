...
export function MultiLineChart({
  ...
  useEffect(() => {
    ...
    const resolvedXDomain = xDomain
      ?? paddedNumericDomain(points.map(point => point.x), 0.04, 1);
    const resolvedYDomain = yDomain
      ?? paddedNumericDomain(points.map(point => point.y), 0.04, 1);
  ...
  }, [chartTheme, colors, height, margin, size.width, visibleSeries, xDomain, xLabel, yDomain, yLabel]);
