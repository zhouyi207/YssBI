...
describe('HistogramChart', () => {
  it('keeps all-zero bins at zero height', () => {
    act(() => {
      root.render(
        <ChartThemeContextProvider value={chartTheme}>
          <HistogramChart
            data={[{ label: 'A', count: 0 }, { label: 'B', count: 0 }]}
            height={200}
          />
        </ChartThemeContextProvider>,
      );
    ...
    const heights = [...host.querySelectorAll<SVGRectElement>('rect.bar')]
  ...
  it('uses group semantics only for keyboard-interactive compact bars', () => {
    const data = [{ label: 'A', count: 2 }, { label: 'B', count: 3 }];
    ...
    expect.soft(bars.map(bar => bar.getAttribute('aria-label'))).toEqual([
      'Histogram bin A, Count 2',
      'Histogram bin B, Count 3',
    ]);
  });
});
