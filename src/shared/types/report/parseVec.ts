...
import { assignPresentKeys, isFiniteNumber, isNonNegativeInteger, isRecord, isString } from './guards';
...
export function parseVecSummaryResultData(raw: unknown): VECSummaryResultData | null {
  ...
  if (!var_names || !isFiniteNumber(raw.num_observation) || !isNonNegativeInteger(raw.rank)) return null;
  if (!isNonNegativeInteger(raw.lags) || !isString(raw.trend_spec)) return null;
  if (!Array.isArray(raw.equations) || !Array.isArray(raw.coefficients) || !Array.isArray(raw.beta)) {
    return null;
  }
  ...
  const veclmar =
    raw.veclmar === undefined ? undefined : parseObjectArray(raw.veclmar, parseVarLmar) ?? undefined;
...
export function parseVecRankResultData(raw: unknown): VecRankResultData | null {
  ...
  const var_names = parseStringArray(raw.var_names);
  if (!var_names || !isFiniteNumber(raw.num_observation) || !isNonNegativeInteger(raw.n_lags)) return null;
  if (typeof raw.show_max_eigen !== 'boolean' || !Array.isArray(raw.rows) || !isString(raw.note)) return null;
  const rankFields = [
