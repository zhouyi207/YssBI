...
function isLocalizedParameter(value: unknown): value is LocalizedParameterDto {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) return false;
  const candidate = value as Record<string, unknown>;
  return hasExactKeys(candidate, ['key', 'title', 'description'])
    && typeof candidate.key === 'string'
    && typeof candidate.title === 'string'
...
export function isLocalizedCatalogDto(value: unknown): value is LocalizedCatalogDto {
  ...
  return hasExactKeys(candidate, [
    'projectInstanceId', 'registryFingerprint', 'resourcePublicationRevision',
    'locale', 'categories', 'items',
  ])
    ...
    && Array.isArray(candidate.categories) && candidate.categories.every(isLocalizedCategory)
    && Array.isArray(candidate.items) && candidate.items.every(isLocalizedCatalogItemDto);
}
