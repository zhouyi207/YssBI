...
function isNodeCreation(value: unknown): boolean {
  ...
  return value.kind === 'resourceBound'
...
function isNode(value: unknown): boolean {
  return isRecord(value)
    && hasExactKeys(
      value,
      ['localId', 'creation', 'parameters', 'userLabel', 'relativePosition'],
    )
...
function isDynamicPortBinding(value: unknown): boolean {
  ...
  return (value.kind === 'resolved' || value.kind === 'orphan')
...
function isInputState(value: unknown): boolean {
  return isRecord(value)
    ...
    && isJsonValue(value.state.literalOverride);
}
...
export function parseClipboardSubgraphDto(value: unknown): ClipboardSubgraphDto {
  if (!isRecord(value)
    || !hasExactKeys(
      value,
      ['schemaVersion', 'nodes', 'portBindings', 'inputStates', 'connections'],
    )
