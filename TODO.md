...
- [x] **Detail 状态推导式重构**：新增 `src/features/core/editor/detail/`（`resolveDetailTarget` + `useDetailTarget`）；`editorStore` 仅存 `sidebarDetailFocus`（variable/data），Tab 型 event/function/worksheet 只从 `layoutStore.activeTabId` 推导，Log 用 `logStore.selectedLog`；`Detail.tsx` / Sidebar 高亮改读推导结果；删除 `syncDetailFromEditorTab`、`syncDetailFromNodeSelection` 及 Tab/节点/Worksheet/Workspace/Log 等处重复 `setSelectedInfo`。推导优先级：① 画布单选节点 → ② Sidebar variable/data → ③ Log → ④ activeTab → 空；取消节点选中后回退 Sidebar 或 Tab（不再变空）。Sidebar event/function 单击不再切 Detail（double-click 开 Tab）；worksheet 单击对齐 `openWorksheet`。
- [x] **ResultSource Presentation 迁移**：后端 `SourceDescriptor.presentation`（`Inspector` / `Plot{chart}` / `Report{report}`）替代 `renderer` + 临时 `window_type` / `SourcePresentation`；节点 API 改为 `publish_plot` / `publish_report` / `open_registered_source`；`OpenSourceWindow` 事件携带 `presentation` + `windowTitle`；View 节点直接 `open_registered_source`；前端 `presentationRoute` / `plotTypeFromPresentation` 统一 Pin「查看」与执行自动开窗；删除 `presentationRouteForDescriptor` 对 `renderer` 的分支推断。
