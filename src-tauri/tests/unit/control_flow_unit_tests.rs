...
fn test_all_control_nodes_registered() {
    ...
    // 验证所有控制流节点都已注册
    assert!(
        registry.get_prototype("if_else").is_some(),
        "IfElse should be registered"
    );
    assert!(
        registry.get_prototype("sequence").is_some(),
        "Sequence should be registered"
    );
    assert!(
        registry.get_prototype("sequence5").is_some(),
        "Sequence5 should be registered"
    );
    assert!(
        registry.get_prototype("while_loop").is_some(),
        "WhileLoop should be registered"
    );
    assert!(
        registry.get_prototype("for_loop").is_some(),
        "ForLoop should be registered"
    );
}
...
fn test_if_else_execution_model() {
    let registry = get_registry();
    let if_else = registry
        .get_prototype("if_else")
        .expect("IfElse should be registered");
...
fn test_if_else_pin_structure() {
    ...
    assert_eq!(
        input_order.len(),
        2,
        "IfElse should have 2 inputs (1 exec + 1 data)"
    );
    assert_eq!(
        output_order.len(),
        2,
        "IfElse should have 2 outputs (True + False)"
    );
...
fn test_if_else_pin_names() {
    ...
    assert_eq!(
        input_info[1].1, "Condition",
        "Second input should be 'Condition'"
    );
...
fn test_sequence5_execution_model() {
    let registry = get_registry();
    let sequence5 = registry
        .get_prototype("sequence5")
        .expect("Sequence5 should be registered");
...
fn test_while_loop_execution_model() {
    let registry = get_registry();
    let while_loop = registry
        .get_prototype("while_loop")
        .expect("WhileLoop should be registered");
...
fn test_for_loop_execution_model() {
    let registry = get_registry();
    let for_loop = registry
        .get_prototype("for_loop")
        .expect("ForLoop should be registered");
...
fn test_execution_models_summary() {
    ...
    for node_type in control_flow_nodes {
        let proto = registry
            .get_prototype(node_type)
            .expect(&format!("{} should be registered", node_type));
        assert_eq!(
