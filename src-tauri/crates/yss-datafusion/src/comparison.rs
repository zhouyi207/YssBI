...
fn compare_arrays(
    ...
    if left.data_type().is_temporal() && right.data_type() == &DataType::Utf8 {
        right = yss_tabular_arrow::lossless_cast(right.as_ref(), left.data_type(), false)
            .map_err(|_| failure())?;
    } else if right.data_type().is_temporal() && left.data_type() == &DataType::Utf8 {
        left = yss_tabular_arrow::lossless_cast(left.as_ref(), right.data_type(), false)
            .map_err(|_| failure())?;
    }
        ...
        && (left.data_type().is_decimal() || right.data_type().is_decimal())
    {
        ...
        left = yss_tabular_arrow::lossless_cast(left.as_ref(), &dtype, false)
            .map_err(|_| failure())?;
        right = yss_tabular_arrow::lossless_cast(right.as_ref(), &dtype, false)
            .map_err(|_| failure())?;
    }
    if left.data_type() == right.data_type() {
