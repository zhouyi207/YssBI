...
impl DatasetStore {
    pub(crate) fn prepare_generation(
        ...
        yss_database_arrow::validate_storage_schema(&metadata.schema)
    ...
    pub fn prepare_import(
        ...
        let source_schema = Arc::new(yss_database_arrow::timezone_free_schema(&input_schema));
        ...
        for (index, field) in source_schema.fields().iter().enumerate() {
            if matches!(field.data_type(), DataType::Dictionary(..))
                && yss_database_arrow::CategoryDomain::from_field(field)
                    .map_err(|_| DatasetStoreError::InvalidSchema)?
                    .is_none()
            {
        ...
        for batch in batches {
            ...
            for (field, array) in batch.schema().fields().iter().zip(batch.columns()) {
                yss_database_arrow::validate_semantic_array(field, array.as_ref())
        ...
        if !domains.is_empty() {
            ...
            for (index, inferred) in domains {
                ...
                let id = yss_database_arrow::column_identity(field)
                    .map_err(|_| DatasetStoreError::InvalidSchema)?;
                let domain = yss_database_arrow::CategoryDomain {
...
fn import_schema(source: &Schema) -> Result<SchemaRef, DatasetStoreError> {
    ...
    let mut fields = source
        ...
        .map(|field| {
            ...
            let categories = yss_database_arrow::CategoryDomain::from_field(field)
                .map_err(|_| DatasetStoreError::InvalidSchema)?;
            yss_database_arrow::with_column_metadata(
                field
                    .as_ref()
    ...
    for field in [
        ...
        fields.push(
            yss_database_arrow::with_column_metadata(field, &Uuid::new_v4().to_string(), None)
                .map_err(|_| DatasetStoreError::InvalidSchema)?,
        );
    }
    let schema = yss_database_arrow::with_row_columns(
        Schema::new_with_metadata(fields, source.metadata().clone()),
        &row_id,
        &order,
    )
    .map_err(|_| DatasetStoreError::InvalidSchema)?;
    yss_database_arrow::validate_storage_schema(&schema)
        .map_err(|_| DatasetStoreError::InvalidSchema)?;
    Ok(Arc::new(schema))
}
