...
fn semantic_edits_persist_restore_and_guard_numeric_execution() {
    ...
    let original = store
        ...
        .unwrap()
        .snapshot;
    let engine = yss_database_engine::DataFusionRuntime::new(64 * 1024 * 1024, 8192).unwrap();
...
fn datetime_column_cast_retains_clock_values_and_forced_nulls_in_persisted_data() {
    ...
    let batch = RecordBatch::try_new(
        Arc::new(Schema::new(vec![
            yss_database_arrow::with_column_semantic(
                ...
                &yss_database_arrow::ColumnSemantic::new(
    ...
    let data = yss_database_io::read_parquet_batches(&reopened.file_paths()[0], 10, None)
        .unwrap()
        .next()
        .unwrap()
        .unwrap();
...
fn editable_fixture() -> (
    ...
    Arc<DatasetStore>,
    Arc<DatasetSnapshot>,
    Arc<yss_database_engine::DataFusionRuntime>,
) {
...
fn delta_overflow_compacts_the_requested_edit_in_one_atomic_commit() {
    ...
    let directory = Directory::new();
    let store = DatasetStore::create(directory.path()).unwrap();
    let engine = yss_database_engine::DataFusionRuntime::new(192 * 1024 * 1024, 2).unwrap();
...
fn native_profiles_follow_the_snapshot_and_preserve_finite_mode_and_empty_table_semantics() {
    ...
    let tiny_engine = yss_database_engine::DataFusionRuntime::new(1, 2).unwrap();
    assert!(matches!(
        original
...
fn sparse_edits_merge_before_filters_and_nulls_survive_column_rename_and_reopen() {
    ...
    let renamed = store
        ...
        .snapshot;
    assert_eq!(
        yss_database_arrow::column_identity(renamed.metadata().schema.field(0)).unwrap(),
        column_id
    );
    ...
    let projected = renamed
        ...
        .unwrap();
    assert_eq!(
        yss_database_arrow::column_identity(projected.schema().field(0)).unwrap(),
        column_id
    );
...
fn filtered_edit_branches_preserve_multicolumn_null_insert_delete_and_snapshot_semantics() {
    ...
    let directory = Directory::new();
    let store = DatasetStore::create(directory.path()).unwrap();
    let engine = yss_database_engine::DataFusionRuntime::new(128 * 1024 * 1024, 2).unwrap();
    let schema = Arc::new(Schema::new(vec![
    ...
    let row_name = yss_database_arrow::dataset_row_columns(&compacted.metadata().schema)
        .unwrap()
        .unwrap()
        .row_id;
    let page = compacted
...
fn inserted_rows_keep_display_position_and_restore_does_not_reuse_row_ids() {
    ...
    let ids = |snapshot: &Arc<DatasetSnapshot>| {
        let name = yss_database_arrow::dataset_row_columns(&snapshot.metadata().schema)
            .unwrap()
            .unwrap()
            .row_id;
        snapshot
...
fn committed_catalog_reopens_exact_data_and_recovers_pending_publication() {
    ...
    let reader = yss_database_io::read_parquet_batches(&snapshot.file_paths()[0], 1, None).unwrap();
...
fn imported_dictionary_domains_survive_reordered_parquet_batches() {
    ...
    yss_database_io::write_parquet_batches(&file, schema, batches.into_iter().map(Ok)).unwrap();
    let reader = yss_database_io::read_parquet_batches(&file, 2, None).unwrap();
    let store = DatasetStore::create(directory.path()).unwrap();
    let snapshot = store
        .commit(
            store
...
fn multipart_import_keeps_row_identity_continuous_across_batch_and_file_boundaries() {
    ...
    let batch = yss_database_io::read_parquet_batches(&snapshot.file_paths()[1], 1, None)
        .unwrap()
        .next()
        .unwrap()
        .unwrap();
    ...
    let engine = yss_database_engine::DataFusionRuntime::new(128 * 1024 * 1024, 8192).unwrap();
