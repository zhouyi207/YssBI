...
impl DatasetStore {
    pub fn prepare_column_semantic(
        ...
        operation: &str,
        name: &str,
        semantic: &yss_database_arrow::ColumnSemantic,
        control: &RelationControl,
    ) -> Result<PreparedDataset, DatasetStoreError> {
        let field =
            yss_database_arrow::with_column_semantic(user_column(before, name)?.clone(), semantic)
                .map_err(|_| DatasetStoreError::InvalidValue)?;
        ...
        let result = query.visit_batches(control, &mut |batch| {
            ...
            yss_database_arrow::validate_semantic_array(&field, values.as_ref()).map_err(|_| {
                invalid = true;
    ...
    pub fn prepare_cast_column(
        ...
        let data_type = yss_database_arrow::timezone_free_data_type(&data_type);
        ...
        let semantic = yss_database_arrow::cast_column_semantic(field, &target)
            .map_err(|_| DatasetStoreError::InvalidValue)?;
        let field = yss_database_arrow::with_column_semantic(target, &semantic)
            .map_err(|_| DatasetStoreError::InvalidValue)?;
    ...
    pub fn prepare_cell_edit(
        ...
        let value = yss_database_arrow::json_to_array(field, &[value])
            .map_err(|_| DatasetStoreError::InvalidValue)?;
    ...
    pub fn prepare_add_row(
        ...
        let rows = yss_database_arrow::dataset_row_columns(&before.metadata.schema)
            .map_err(|_| DatasetStoreError::InvalidSchema)?
            .ok_or(DatasetStoreError::InvalidSchema)?;
        let left = if index == 0 {
            None
