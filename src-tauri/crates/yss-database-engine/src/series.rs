...
struct SemanticConversionFunction {
    ...
    conversion: Arc<yss_database_arrow::PreparedConversion>,
}
...
pub(crate) fn convert(
    ...
    let conversion = Arc::new(
        yss_database_arrow::PreparedConversion::new(source, &conversion)
    ...
    let function = create_udf(
        ...
        Arc::new(move |arguments| {
            ...
            let arrays = arrays
                .iter()
                .map(|array| {
                    yss_database_arrow::lossless_cast(array.as_ref(), &result_type, false)
                        .map_err(|_| failure(RelationError::InvalidInput))
                })
                .collect::<Result<Vec<_>, _>>()?;
