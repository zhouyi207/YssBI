    ...
    let expressions = target
        ...
        .map(|field| {
            ...
            let source = source
                .fields()
                .iter()
                .find(|field| yss_database_arrow::column_identity(field).ok() == Some(id));
...
impl DataFusionRuntime {
    pub fn dataset_query(
        ...
        yss_database_arrow::validate_storage_schema(&input.schema)
            .map_err(|_| RelationError::InvalidInput)?;
        let rows = yss_database_arrow::dataset_row_columns(&input.schema)
            .map_err(|_| RelationError::InvalidInput)?
            .ok_or(RelationError::InvalidInput)?;
        let base_rows = yss_database_arrow::dataset_row_columns(&input.base_schema)
            .map_err(|_| RelationError::InvalidInput)?
            .ok_or(RelationError::InvalidInput)?;
...
impl DatasetQuery {
    pub fn cast_column(
        ...
        let target = schema
            ...
            .data_type()
            .clone();
        if target != yss_database_arrow::timezone_free_data_type(&target) {
            return Err(RelationError::InvalidInput);
        }
        let expression = {
