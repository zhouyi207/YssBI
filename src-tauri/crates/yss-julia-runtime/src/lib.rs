...
pub enum JuliaRuntimeError {
    #[error("Julia was not found on the system PATH")]
    Missing,
    #[error("{message}")]
...
pub fn system_julia_executable() -> Result<PathBuf, JuliaRuntimeError> {
    match inspect_system_julia() {
        RuntimeProbe::Ready { executable, .. } => Ok(executable),
        RuntimeProbe::Missing => Err(JuliaRuntimeError::Missing),
        RuntimeProbe::Invalid { message, .. } => Err(JuliaRuntimeError::Invalid { message }),
    }
}
