...
// 值域：[0, 4]
// 仅适用于时序性数据
pub fn durbin_waston(resid: &Col<f64>) -> f64 {
    assert!(
        resid.nrows() > 1,
        "Durbin-Watson requires at least two residuals"
    );
    let diff = (1..resid.nrows())
...
// 小样本表现更好，统计上更“讲究”
// 统计严谨 / 学术分析
pub fn omnibus(resid: &Col<f64>) -> (f64, f64) {
    let n = resid.nrows() as f64;
...
// 经济计量 / 金融工程
pub fn jarque_bera(resid: &Col<f64>) -> (f64, f64) {
    let n = resid.nrows() as f64;
    let (s, k) = skewness_kurtosis(resid);
