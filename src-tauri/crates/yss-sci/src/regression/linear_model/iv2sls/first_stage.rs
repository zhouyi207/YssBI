...
pub(crate) fn compute_first_stage_summary(
    ...
    let mx1_y = yss_sci_linalg::MatRef::from_row_major_slice(&(mx1_y_data), n, k_endog).to_owned();
    ...
    let mx1_x2 = yss_sci_linalg::MatRef::from_row_major_slice(&(mx1_x2_data), n, k_iv).to_owned();
    ...
    let mz_y = yss_sci_linalg::MatRef::from_row_major_slice(&(mz_y_data), n, k_endog).to_owned();

    // Σ_VV = (1/(N-k_z)) Y' M_Z Y
    let sigma_vv = (mz_y.transpose() * mz_y.as_ref()) / yss_sci_linalg::Scale(df_z.max(1) as f64);
    ...
    let y_mx1 = yss_sci_linalg::MatRef::from_row_major_slice(&(y_mx1_data), n, k_endog).to_owned();
