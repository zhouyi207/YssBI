...
mod tests {
    use super::*;
    #[test]
    fn handles_bidirectional_requests_without_blocking_the_original_reply() {
        let listener = std::net::TcpListener::bind("127.0.0.1:0").unwrap();
        let client = std::net::TcpStream::connect(listener.local_addr().unwrap()).unwrap();
        let (server, _) = listener.accept().unwrap();
        let host = Peer::connect(
            client.try_clone().unwrap(),
            client.try_clone().unwrap(),
            ResourceBudget::default(),
            Arc::new(|_, request| Ok(request.params)),
        );
        let plugin = Peer::connect(
            server.try_clone().unwrap(),
            server.try_clone().unwrap(),
            ResourceBudget::default(),
            Arc::new(|peer, request| {
                if request.method == "roundtrip" {
                    peer.call("host.echo", request.params, Duration::from_secs(2))
                } else {
                    Ok(Value::Null)
                }
            }),
