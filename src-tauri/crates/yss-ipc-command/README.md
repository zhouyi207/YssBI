# `yss-ipc-command` transport contract

> Status: Current
> Scope: Tauri command、event、channel、wire DTO 和 transport error seam
> Canonical owners: the four `yss-ipc-*` crates own executable wire facts; this README owns the shared transport contract
> Update when: public transport shape、registry ownership、delivery semantics 或 frontend invoke seam 改变时

YssBI's desktop IPC boundary consists of four crates:

| Crate                                               | Responsibility                                                                                                                   |
| --------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `yss-ipc-command`                                   | Tauri commands, the single `invoke_handler()` registry, application-to-response mapping and diagnosed command errors             |
| [`yss-ipc-event`](../yss-ipc-event/README.md)       | Tauri Event delivery after authoritative commits                                                                                 |
| [`yss-ipc-channel`](../yss-ipc-channel/README.md)   | Harness subscriptions and graph-client handoff, execution/Run Output encoding, project progress and diagnostics channel adapters |
| [`yss-ipc-contract`](../yss-ipc-contract/README.md) | Shared wire DTOs, event envelopes and the `{ code, details, incidentId }` error payload                                          |

Command depends on Event, Channel and Contract; Event and Channel depend on Contract. Contract has no Tauri, Application or runtime dependency. It reuses neutral document/identity contracts and existing projection value types. Business workflows and committed state remain in their application/domain owners. Event and Channel never depend on Command or register commands.

## Public surface

The crate keeps handlers, command-specific schemas and diagnosed error construction private. Its production surface is intentionally narrow:
