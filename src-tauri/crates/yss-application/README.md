...
| 数据：11 个          | `yss-data-contract`、`yss-database-contract`、`yss-database-runtime`、`yss-database-schema`、`yss-dataset-profile`、`yss-database-store`、`yss-relational-contract`、`yss-database-source`、`yss-database-arrow`、`yss-data-contract`、`yss-database-io`             | 数据导入导出、编辑、查询、快照和项目资源发布               |
...
直接依赖同时包含实现调用和类型引用。`yss-graph-execution` 的[运行准备](../yss-graph-execution/src/graph_preparation.rs) 从已解析图直接构造执行计划、参数和输出契约；Application 只提供经过重验的语义与资源依据。`yss-function-editor-projection` 用于项目函数签名的展示与类型转换，通用图编辑器投影由 `yss-graph-editor::projection` 提供。
