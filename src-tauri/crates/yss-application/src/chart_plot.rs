...
pub(crate) fn query_chart_plot_in_session(
    ...
    let result = chart_plot_from_pair(NumericPairView::from(&pair), query.max_points)?;
...
fn chart_plot_from_pair(
    ...
    let data = pair
        ...
        .collect::<Vec<_>>();
    if data.is_empty() {
        return Err(ChartPlotApplicationError::PlotDataEmpty);
    }
...
fn subsample_points<T>(data: Vec<T>, max_points: usize) -> Vec<T> {
    ...
    let stride = data.len() / max_points + usize::from(!data.len().is_multiple_of(max_points));
...
mod tests {
    ...
    #[test]
    fn query_chart_plot_filters_caps_and_preserves_formats_from_same_session() {
        ...
        let result = chart_plot_from_pair(
            NumericPairView {
                x: &x,
                y: &y,
