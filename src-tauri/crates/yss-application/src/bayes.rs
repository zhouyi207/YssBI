...
mod tests {
    ...
    #[test]
    fn failed_task_emits_safe_diagnostic_and_returns_safe_error() {
        ...
        let captured_events = captured_events.lock().expect("captured event lock");
        let record = captured_events
            .iter()
            .find(|entry| {
                entry.fields.get("diagnostic_event").map(String::as_str)
                    == Some("bayesInferenceFailed")
            })
        ...
        assert!(!record.fields.contains_key("backend_message"));
        assert!(!record.fields.contains_key("backend_detail"));
    }
