use std::fmt;
use yss_ipc_contract::error::CommandErrorDto;
...
#[serde(transparent)]
pub struct CommandError(CommandErrorDto);

impl CommandError {
    pub fn expected(code: &'static str) -> Self {
        assert_valid_code(code);
        Self(CommandErrorDto {
            code,
            details: Value::Null,
            incident_id: None,
        })
    ...
    pub(crate) fn details(&self) -> Option<&serde_json::Map<String, Value>> {
        self.0.details.as_object()
    }

    pub fn incident_id(&self) -> Option<&str> {
        self.0.incident_id.as_deref()
    }
