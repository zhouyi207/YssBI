...
fn chart_application_command_error(error: &ChartApplicationError) -> CommandError {
    match error {
        ...
        ChartApplicationError::Project(error) => {
            crate::ipc::commands::project_failure::application_project_command_error(error.clone())
        }
...
pub fn get_plot_column_pair(
    ...
    let x_column = yss_data_contract::TabularColumnName::try_from(x_col.as_str())
        .map_err(database_computation_error)?;
    let y_column = yss_data_contract::TabularColumnName::try_from(y_col.as_str())
        .map_err(database_computation_error)?;
    let result = state
