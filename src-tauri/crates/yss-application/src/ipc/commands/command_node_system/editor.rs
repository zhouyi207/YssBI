...
use yss_ipc_contract::graph_draft::CompileGraphDraftDto;
use yss_ipc_contract::graph_draft::GraphDraftTransformDto;
...
pub fn resolve_graph_draft(
    ...
    locale: String,
) -> Result<yss_ipc_contract::editor_projection::EditorGraphProjectionDto, CommandError> {
    application
        .resolve_graph_draft(
            project_instance_id,
        ...
        .map(|projection| crate::ipc::schema::editor_projection::map_editor_projection(&projection))
...
pub fn transform_graph_draft(
    ...
    document: yss_graph_document::GraphDocument,
    mutation: serde_json::Value,
) -> Result<GraphDraftTransformDto, CommandError> {
    let mutation = parse_editor_mutation(mutation)?;
    let graph_path = parse_graph_path(graph_path)?;
    let result = application
        .transform_graph_draft(project_instance_id, graph_path, locale, document, mutation)
        .map_err(map_editor_resource_error)?;
    Ok(crate::ipc::schema::graph_draft::graph_draft_transform_to_transport(&result))
}
