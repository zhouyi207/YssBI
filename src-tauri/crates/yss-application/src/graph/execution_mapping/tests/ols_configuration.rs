...
fn node_owned_ols_configuration_compiles_and_changes_computed_results() {
    ...
    let execute = |document: &GraphDocument| {
        let compiled = runtime
            .compile_draft(document, graph.clone(), &catalog, &basis, &|id| {
        ...
        let package =
            crate::graph::execution_mapping::execution_package_from_graph(package, execution_basis)
                .unwrap();
    ...
    assert!(
        runtime
            .compile_draft(&document, graph, &catalog, &basis, &|id| {
                yss_graph_execution::kernels::KernelRegistry::default().supports(id)
            })
            .unwrap()
            .artifact_id()
