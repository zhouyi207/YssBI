...
use yss_graph_execution::plan::{
    CanonicalDecimal, CompiledExecutionPackage, CompiledParameterBundleBuilder,
    CompiledParameterHandle, ExecutionPlan, KernelId, PlanGraphId, PlanInputBinding,
    PlanInputCoercion, PlanInputCoercionKind, PlanInputSource, PlanKernelSpecialization,
    PlanNodeId, PlanObservationIntent, PlanOperation, PlanOutputBinding, PlanOutputRef,
    PlanParameterFieldId, PlanParameterPayload, PlanParameterScalar, PlanParameterSchemaId,
    PlanParameterValue, PlanPortAddress, PlanProvenance, PlanSourceIdentity, PlanTypeBinding,
    ValueRef,
};
#[derive(Debug, Error)]
pub enum GraphPackageMappingError {
    #[error("graph package kernel capabilities differ from the execution basis")]
...
pub fn execution_package_from_graph(
    ...
    if package.kernel_fingerprint() != &basis.kernel_fingerprint().as_bytes() {
        return Err(GraphPackageMappingError::KernelCapabilitiesMismatch);
    }
