...
use yss_graph_runtime::{GraphRuntimeEpoch, GraphRuntimeState};
...
pub struct DatabaseSessionCandidateError {
    #[source]
    source: super::database::DatabaseSessionApplicationError,
}

impl From<super::database::DatabaseSessionApplicationError> for DatabaseSessionCandidateError {
    fn from(source: super::database::DatabaseSessionApplicationError) -> Self {
        Self { source }
    }
}
...
impl ApplicationState {
    pub fn initialize() -> Result<Self, ApplicationInitializationError> {
        Self::initialize_with_nodes(super::NodeComponents::builtins()?)
    }

    pub fn initialize_with_nodes(
        nodes: super::NodeComponents,
    ) -> Result<Self, ApplicationInitializationError> {
        ...
        let application = Self::new(Arc::new(ApplicationSessionSlot::new(nodes)));
        application
...
pub fn build_current_project_candidate(
        ...
        .map(|declaration| {
            reusable_instances
                ...
                .unwrap_or_else(|| match &dataset_store {
                    ...
                    _ => DatabaseInstance::unavailable(declaration.clone()),
                })
        })
        .collect::<Vec<_>>();
    ...
    let runtime_generation = RuntimeGeneration::from_existing(generation.get());
    ...
    fn candidate_build_binds_one_session_identity_and_rejects_mismatch() {
        assert!(matches!(
            build_current_project_candidate(
                ApplicationSessionEpoch::from_existing(u64::MAX),
                Arc::new(ProjectState::new()),
                [],
                &super::super::NodeComponents::builtins().unwrap(),
            ),
            Err(ProjectSessionCandidateError::GenerationExhausted)
        ));
