...
pub(crate) fn install_variable_effect_snapshots(
...
pub(crate) fn variable_scope_graph_path(
...
pub(crate) fn variable_history_scope(
    data: &ProjectData,
    transaction: &ProjectHistoryTransaction,
...
pub(crate) fn variable_effect_filesystem_mutations(
    data: &ProjectData,
    ...
    for graph_path in local_graph_paths {
        ...
        mutations.push(StagedFilesystemMutation::Write {
            ...
            contents: crate::project_io::serialize_graph_resource_document(graph, local_variables)
                .map_err(|error| error.to_string())?,
        });
    }
    Ok(mutations)
...
pub(crate) fn validate_variable_effect_document(
    path: &std::path::Path,
    contents: &[u8],
) -> Result<(), String> {
    if path == std::path::Path::new(yss_project_layout::GLOBAL_VARIABLES_FILE) {
        serde_json::from_slice::<crate::project_io::GlobalVariablesDocument>(contents)
            .map(|_| ())
            .map_err(|error| error.to_string())
    } else {
