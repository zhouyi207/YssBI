...
impl ProjectState {
    ...
    pub fn add_variable(
        ...
        revisions.insert(
            id,
            crate::project_state::VariableRevisionEntry::present(
...
mod tests {
    ...
    fn authority_snapshot(
        ...
        std::collections::HashMap<VariableId, crate::project_state::VariableRevisionEntry>,
        u64,
    ) {
        (
    ...
    fn remove_variable_retains_next_revision_tombstone() {
        ...
        assert!(
            !state
                .variable_revision_snapshot_for_test(&variable.id)
                .unwrap()
                .present
        );
    }
