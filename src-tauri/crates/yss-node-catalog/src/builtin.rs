...
fn register_builtin_nominal_validators(
    ...
    builder
        .register_nominal_validator(
            parse_type_id(yss_node_protocol::dataframe::PROJECT_COLUMNS_TYPE_ID)?,
            parse_type_id(yss_node_protocol::dataframe::PROJECT_COLUMNS_VALIDATOR_ID)?,
            yss_node_protocol::dataframe::DATAFRAME_NOMINAL_CODEC_VERSION,
            |value| yss_node_protocol::dataframe::prepare_project_columns_json(value).map(drop),
        )
        .map_err(BuiltinAssemblyError::Registration)?;
    builder
        .register_nominal_validator(
            parse_type_id(yss_node_protocol::dataframe::FILTER_PREDICATE_TYPE_ID)?,
            parse_type_id(yss_node_protocol::dataframe::FILTER_PREDICATE_VALIDATOR_ID)?,
            yss_node_protocol::dataframe::DATAFRAME_NOMINAL_CODEC_VERSION,
            |value| yss_node_protocol::dataframe::prepare_filter_predicate_json(value).map(drop),
        )
        .map_err(BuiltinAssemblyError::Registration)?;
    Ok(())
}
