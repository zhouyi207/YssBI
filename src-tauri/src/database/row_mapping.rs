...
pub(crate) fn polars_value_to_json(v: AnyValue<'_>) -> serde_json::Value {
    match v {
        ...
        AnyValue::Time(ns) => {
            ...
            let m = (secs % 3600) / 60;
            let s = secs % 60;
            serde_json::Value::String(format!("{h:02}:{m:02}:{s:02}"))
        }
...
pub(crate) fn dataframe_to_row_matrix(df: &DataFrame) -> Vec<Vec<serde_json::Value>> {
    (0..df.height())
        .map(|row_idx| {
            df.columns()
                .iter()
                .map(|series| match series.get(row_idx) {
                    Ok(value) => polars_value_to_json(value),
