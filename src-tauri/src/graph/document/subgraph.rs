...
fn rewrite_address(
...
) -> Result<ClipboardPortAddress, MutationConflict> {
    let node_id = nodes
        .get(&address.node_id)
        .cloned()
        .ok_or_else(|| invalid_export("subgraph address references an unselected node"))?;
    let port = match &address.port {
        PortRef::Declared { key } => ClipboardPortRef::Declared { key: key.clone() },
        PortRef::Instance {
            template,
            instance_id,
    ...
    Ok(ClipboardPortAddress { node_id, port })
}
