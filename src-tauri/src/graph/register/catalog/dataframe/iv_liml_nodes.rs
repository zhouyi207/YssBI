...
use super::info_nodes::{
    ...
    Iv2slsStockYogoBiasRow, Iv2slsStockYogoCv, Iv2slsStockYogoSizeRow, IvLimlOveridTest,
...
> {
        ...
        other => {
            let got = match other {
                ...
                DataValue::Struct { type_key, .. } => {
                    return Err(format!(
                        "IV: Y input is not a DataSeries (got Struct<{}>).",
                        type_key
                    ));
                }
                _ => "other type",
    ...
    if inst_df.height() != n_raw {
        return Err(format!(
            "IV: x_instruments has {} rows, expected {}",
            inst_df.height(),
            n_raw
        ));
    }
    ...
    if k_iv < k_endog {
        return Err(format!(
            "IV: underidentified — {} instruments < {} endogenous.",
            k_iv, k_endog
        ));
    }
