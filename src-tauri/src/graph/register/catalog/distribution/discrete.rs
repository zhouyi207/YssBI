...
fn register_bernoulli(registry: &NodeRegistry) {
    let def = NodeDefinition::new("Bernoulli", vec!["Distribution".to_string(), "Random".to_string()])
        .with_ui_style("value")
        .with_description("Sample from Bernoulli(p) distribution, outputs 0 or 1")
        .with_pin_slots(vec![
            PinSlot::fixed(PinDefinition::data_input("P", DataRole::Inputs(0), float_type())),
...
fn register_binomial(registry: &NodeRegistry) {
    let def = NodeDefinition::new("Binomial", vec!["Distribution".to_string(), "Random".to_string()])
        .with_ui_style("value")
        .with_description("Sample from Binomial(n_trials, p) distribution")
        .with_pin_slots(vec![
            PinSlot::fixed(PinDefinition::data_input("N Trials", DataRole::Inputs(0), int_type())),
            PinSlot::fixed(PinDefinition::data_input("P", DataRole::Inputs(1), float_type())),
            PinSlot::fixed(PinDefinition::data_input("N Samples", DataRole::Inputs(2), int_type())),
            PinSlot::fixed(PinDefinition::data_output(
                "Samples",
                DataRole::Output,
                PinDataTypeDefinition::concrete(DataType::DataSeries(Box::new(DataType::Int64))),
        ...
        .with_data_evaluator(Arc::new(|ctx| {
            ...
            let mut rng = StdRng::from_entropy();
            let values: Vec<i64> = (0..n_samples).map(|_| dist.sample(&mut rng) as i64).collect();
...
fn register_poisson(registry: &NodeRegistry) {
    let def = NodeDefinition::new("Poisson", vec!["Distribution".to_string(), "Random".to_string()])
        .with_ui_style("value")
        .with_description("Sample from Poisson(lambda) distribution")
        .with_pin_slots(vec![
            PinSlot::fixed(PinDefinition::data_input("Lambda", DataRole::Inputs(0), float_type())),
            PinSlot::fixed(PinDefinition::data_input("N", DataRole::Inputs(1), int_type())),
            PinSlot::fixed(PinDefinition::data_output(
                "Samples",
                DataRole::Output,
...
fn register_geometric(registry: &NodeRegistry) {
    let def = NodeDefinition::new("Geometric", vec!["Distribution".to_string(), "Random".to_string()])
        .with_ui_style("value")
...
fn register_negative_binomial(registry: &NodeRegistry) {
    let def = NodeDefinition::new("NegativeBinomial", vec!["Distribution".to_string(), "Random".to_string()])
        .with_ui_style("value")
        .with_description("Sample from NegativeBinomial(r, p) distribution")
        .with_pin_slots(vec![
            PinSlot::fixed(PinDefinition::data_input("R", DataRole::Inputs(0), float_type())),
            PinSlot::fixed(PinDefinition::data_input("P", DataRole::Inputs(1), float_type())),
...
fn register_discrete_uniform(registry: &NodeRegistry) {
    let def = NodeDefinition::new("DiscreteUniform", vec!["Distribution".to_string(), "Random".to_string()])
        .with_ui_style("value")
        .with_description("Sample from DiscreteUniform(low, high) distribution, inclusive")
        .with_pin_slots(vec![
            PinSlot::fixed(PinDefinition::data_input("Low", DataRole::Inputs(0), int_type())),
            PinSlot::fixed(PinDefinition::data_input("High", DataRole::Inputs(1), int_type())),
            PinSlot::fixed(PinDefinition::data_input("N", DataRole::Inputs(2), int_type())),
            PinSlot::fixed(PinDefinition::data_output(
                "Samples",
                DataRole::Output,
...
fn register_hypergeometric(registry: &NodeRegistry) {
    let def = NodeDefinition::new("Hypergeometric", vec!["Distribution".to_string(), "Random".to_string()])
        .with_ui_style("value")
        .with_description("Sample from Hypergeometric(N, K, n) distribution")
        .with_pin_slots(vec![
            PinSlot::fixed(PinDefinition::data_input("N", DataRole::Inputs(0), int_type())),
            PinSlot::fixed(PinDefinition::data_input("K", DataRole::Inputs(1), int_type())),
            PinSlot::fixed(PinDefinition::data_input("n", DataRole::Inputs(2), int_type())),
            PinSlot::fixed(PinDefinition::data_input("N Samples", DataRole::Inputs(3), int_type())),
            PinSlot::fixed(PinDefinition::data_output(
                "Samples",
                DataRole::Output,
                PinDataTypeDefinition::concrete(DataType::DataSeries(Box::new(DataType::Int64))),
        ...
        .with_data_evaluator(Arc::new(|ctx| {
            ...
            let dist = statrs::distribution::Hypergeometric::new(
                n_pop as u64,
                k as u64,
                n_draw as u64,
            )
