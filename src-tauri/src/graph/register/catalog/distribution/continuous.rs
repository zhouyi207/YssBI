...
fn float_input(ctx: &dyn crate::execution::NodeExecutionContextTrait, role: DataRole) -> Result<f64, String> {
...
fn register_normal(registry: &NodeRegistry) {
    let def = NodeDefinition::new("Normal", vec!["Distribution".to_string(), "Random".to_string()])
        .with_ui_style("value")
        .with_description("Sample from Normal(mean, std) distribution")
        .with_pin_slots(vec![
            PinSlot::fixed(PinDefinition::data_input("Mean", DataRole::Inputs(0), float_type())),
            PinSlot::fixed(PinDefinition::data_input("Std", DataRole::Inputs(1), float_type())),
            PinSlot::fixed(PinDefinition::data_input("N", DataRole::Inputs(2), int_type())),
...
fn register_uniform(registry: &NodeRegistry) {
    let def = NodeDefinition::new("Uniform", vec!["Distribution".to_string(), "Random".to_string()])
        .with_ui_style("value")
        .with_description("Sample from Uniform(low, high) distribution")
        .with_pin_slots(vec![
            PinSlot::fixed(PinDefinition::data_input("Low", DataRole::Inputs(0), float_type())),
            PinSlot::fixed(PinDefinition::data_input("High", DataRole::Inputs(1), float_type())),
            PinSlot::fixed(PinDefinition::data_input("N", DataRole::Inputs(2), int_type())),
            PinSlot::fixed(PinDefinition::data_output(
                "Samples",
                DataRole::Output,
...
fn register_exp(registry: &NodeRegistry) {
    let def = NodeDefinition::new("Exponential", vec!["Distribution".to_string(), "Random".to_string()])
        .with_ui_style("value")
        .with_description("Sample from Exp(rate) distribution")
        .with_pin_slots(vec![
            PinSlot::fixed(PinDefinition::data_input("Rate", DataRole::Inputs(0), float_type())),
            PinSlot::fixed(PinDefinition::data_input("N", DataRole::Inputs(1), int_type())),
            PinSlot::fixed(PinDefinition::data_output(
                "Samples",
                DataRole::Output,
                PinDataTypeDefinition::concrete(DataType::DataSeries(Box::new(DataType::Float64))),
        ...
        .with_data_evaluator(Arc::new(|ctx| {
            ...
            let dist = statrs::distribution::Exp::new(rate).map_err(|e| format!("Exponential: invalid params: {}", e))?;
            let mut rng = StdRng::from_entropy();
            let values: Vec<f64> = (0..n).map(|_| dist.sample(&mut rng)).collect();
...
fn register_gamma(registry: &NodeRegistry) {
    let def = NodeDefinition::new("Gamma", vec!["Distribution".to_string(), "Random".to_string()])
        .with_ui_style("value")
        .with_description("Sample from Gamma(shape, rate) distribution")
        .with_pin_slots(vec![
            PinSlot::fixed(PinDefinition::data_input("Shape", DataRole::Inputs(0), float_type())),
            PinSlot::fixed(PinDefinition::data_input("Rate", DataRole::Inputs(1), float_type())),
            PinSlot::fixed(PinDefinition::data_input("N", DataRole::Inputs(2), int_type())),
            PinSlot::fixed(PinDefinition::data_output(
                "Samples",
                DataRole::Output,
...
fn register_beta(registry: &NodeRegistry) {
    let def = NodeDefinition::new("Beta", vec!["Distribution".to_string(), "Random".to_string()])
        .with_ui_style("value")
        .with_description("Sample from Beta(alpha, beta) distribution")
        .with_pin_slots(vec![
            PinSlot::fixed(PinDefinition::data_input("Alpha", DataRole::Inputs(0), float_type())),
            PinSlot::fixed(PinDefinition::data_input("Beta", DataRole::Inputs(1), float_type())),
            PinSlot::fixed(PinDefinition::data_input("N", DataRole::Inputs(2), int_type())),
            PinSlot::fixed(PinDefinition::data_output(
                "Samples",
                DataRole::Output,
...
fn register_students_t(registry: &NodeRegistry) {
    let def = NodeDefinition::new("StudentsT", vec!["Distribution".to_string(), "Random".to_string()])
        .with_ui_style("value")
        .with_description("Sample from Student's t(df) distribution")
        .with_pin_slots(vec![
            PinSlot::fixed(PinDefinition::data_input("DF", DataRole::Inputs(0), float_type())),
            PinSlot::fixed(PinDefinition::data_input("N", DataRole::Inputs(1), int_type())),
            PinSlot::fixed(PinDefinition::data_output(
                "Samples",
                DataRole::Output,
...
fn register_cauchy(registry: &NodeRegistry) {
    let def = NodeDefinition::new("Cauchy", vec!["Distribution".to_string(), "Random".to_string()])
        .with_ui_style("value")
        .with_description("Sample from Cauchy(location, scale) distribution")
        .with_pin_slots(vec![
            PinSlot::fixed(PinDefinition::data_input("Location", DataRole::Inputs(0), float_type())),
            PinSlot::fixed(PinDefinition::data_input("Scale", DataRole::Inputs(1), float_type())),
            PinSlot::fixed(PinDefinition::data_input("N", DataRole::Inputs(2), int_type())),
            PinSlot::fixed(PinDefinition::data_output(
                "Samples",
                DataRole::Output,
...
fn register_chi_squared(registry: &NodeRegistry) {
    let def = NodeDefinition::new("ChiSquared", vec!["Distribution".to_string(), "Random".to_string()])
        .with_ui_style("value")
...
fn register_log_normal(registry: &NodeRegistry) {
    let def = NodeDefinition::new("LogNormal", vec!["Distribution".to_string(), "Random".to_string()])
        .with_ui_style("value")
        .with_description("Sample from LogNormal(mu, sigma) distribution")
        .with_pin_slots(vec![
            PinSlot::fixed(PinDefinition::data_input("Mu", DataRole::Inputs(0), float_type())),
            PinSlot::fixed(PinDefinition::data_input("Sigma", DataRole::Inputs(1), float_type())),
            PinSlot::fixed(PinDefinition::data_input("N", DataRole::Inputs(2), int_type())),
            PinSlot::fixed(PinDefinition::data_output(
                "Samples",
                DataRole::Output,
...
fn register_weibull(registry: &NodeRegistry) {
    let def = NodeDefinition::new("Weibull", vec!["Distribution".to_string(), "Random".to_string()])
        .with_ui_style("value")
...
fn register_laplace(registry: &NodeRegistry) {
    let def = NodeDefinition::new("Laplace", vec!["Distribution".to_string(), "Random".to_string()])
        .with_ui_style("value")
...
fn register_pareto(registry: &NodeRegistry) {
    let def = NodeDefinition::new("Pareto", vec!["Distribution".to_string(), "Random".to_string()])
        .with_ui_style("value")
...
fn register_inverse_gamma(registry: &NodeRegistry) {
    let def = NodeDefinition::new("InverseGamma", vec!["Distribution".to_string(), "Random".to_string()])
        .with_ui_style("value")
...
fn register_triangular(registry: &NodeRegistry) {
    let def = NodeDefinition::new("Triangular", vec!["Distribution".to_string(), "Random".to_string()])
        .with_ui_style("value")
        .with_description("Sample from Triangular(a, b, c) distribution")
        .with_pin_slots(vec![
            PinSlot::fixed(PinDefinition::data_input("A", DataRole::Inputs(0), float_type())),
            PinSlot::fixed(PinDefinition::data_input("B", DataRole::Inputs(1), float_type())),
            PinSlot::fixed(PinDefinition::data_input("C", DataRole::Inputs(2), float_type())),
            PinSlot::fixed(PinDefinition::data_input("N", DataRole::Inputs(3), int_type())),
            PinSlot::fixed(PinDefinition::data_output(
                "Samples",
                DataRole::Output,
                PinDataTypeDefinition::concrete(DataType::DataSeries(Box::new(DataType::Float64))),
...
fn register_fisher_snedecor(registry: &NodeRegistry) {
    let def = NodeDefinition::new("FisherSnedecor", vec!["Distribution".to_string(), "Random".to_string()])
        .with_ui_style("value")
        .with_description("Sample from F(d1, d2) distribution")
        .with_pin_slots(vec![
            PinSlot::fixed(PinDefinition::data_input("D1", DataRole::Inputs(0), float_type())),
            PinSlot::fixed(PinDefinition::data_input("D2", DataRole::Inputs(1), float_type())),
            PinSlot::fixed(PinDefinition::data_input("N", DataRole::Inputs(2), int_type())),
            PinSlot::fixed(PinDefinition::data_output(
                "Samples",
                DataRole::Output,
...
fn register_erlang(registry: &NodeRegistry) {
    let def = NodeDefinition::new("Erlang", vec!["Distribution".to_string(), "Random".to_string()])
        .with_ui_style("value")
        .with_description("Sample from Erlang(k, rate) distribution, k is shape (integer)")
        .with_pin_slots(vec![
            PinSlot::fixed(PinDefinition::data_input("K", DataRole::Inputs(0), int_type())),
            PinSlot::fixed(PinDefinition::data_input("Rate", DataRole::Inputs(1), float_type())),
            PinSlot::fixed(PinDefinition::data_input("N", DataRole::Inputs(2), int_type())),
            PinSlot::fixed(PinDefinition::data_output(
                "Samples",
