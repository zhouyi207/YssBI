...
fn register_get_variable(registry: &NodeRegistry) {
    let definition = NodeDefinition::new("Get Variable", vec!["Variables".to_string()])
        ...
        .with_pin_slots(vec![
            PinSlot::fixed(PinDefinition::data_output("Value", DataRole::Output, PinDataTypeDefinition::concrete(DataType::Any))),
        ])
        .with_data_evaluator(Arc::new(|ctx| {
            let params = ctx.get_instance_params();
            let variable_id = params.variable_id().ok_or("Get Variable: variable_id not set")?;
...
fn register_set_variable(registry: &NodeRegistry) {
    let definition = NodeDefinition::new("Set Variable", vec!["Variables".to_string()])
        ...
        .with_pin_slots(vec![
            ...
            PinSlot::fixed(PinDefinition::data_input("Value", DataRole::Input, PinDataTypeDefinition::concrete(DataType::Any))),
            PinSlot::fixed(PinDefinition::data_output("Value", DataRole::Output, PinDataTypeDefinition::concrete(DataType::Any))),
        ])
        ...
        .with_data_evaluator(Arc::new(|ctx| {
            let params = ctx.get_instance_params();
            let variable_id = params.variable_id().ok_or("Set Variable: variable_id not set")?;
