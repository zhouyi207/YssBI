...
pub(crate) fn bind_nominal_handles(
    _provider: &mut ProviderRegistration,
    _handles: DataframeNominalHandles,
) {
    // The production Graph compiler consumes protocol/catalog facts only.  A
    // concrete nominal lowering implementation belongs to the backend adapter
    // and is deliberately not installed in the Graph catalog.
}
...
pub(crate) fn bind_nominal_handles(
    ...
    // Test builds keep the catalog assembly shape without reviving the removed
    // compiler lowerer owner.
}
...
mod legacy_lowerers {
    ...
    use crate::graph::compiler::engine::{
        FragmentMetadata, FragmentResult, LoweredKernel, LoweredNode, LoweringContext,
            ...
            operator => {
                ...
                Ok(match operator {
                    ...
                    FilterOperator::GreaterThan => {
                        RelationalExpression::GreaterThan(column, literal)
                    }
        ...
        Ok(LoweredNode {
            kernel: LoweredKernel::Relational(RelationalNodeFragment {
                backend: RelationalBackendId::new("relational.default")
                    .map_err(|_| LoweringError::internal(LoweringInvariant::InvalidStaticHandle))?,
                fragment: crate::graph::compiler::engine::relational::RelationalFragment {
                    id: RelationalFragmentId::new(format!("node.{}", context.node_id)).map_err(
                        |_| LoweringError::internal(LoweringInvariant::InvalidStaticHandle),
                    )?,
