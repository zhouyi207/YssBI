...
mod tests {
    ...
    fn assert_triggers_output(effect: &ExecutionEffect, expected_role: ExecRole) {
        match effect {
            ExecutionEffect::TriggerOutput(role) => {
                assert_eq!(role, &expected_role, "Expected to trigger {:?}", expected_role);
            }
    ...
    fn assert_is_sequence(effect: &ExecutionEffect, expected_roles: Vec<ExecRole>) {
        match effect {
            ExecutionEffect::TriggerAndContinue { current, remaining } => {
                ...
                assert_eq!(all_roles, expected_roles, "Expected sequence {:?}", expected_roles);
            }
            ExecutionEffect::TriggerOutput(role) if expected_roles.len() == 1 => {
                assert_eq!(role, &expected_roles[0], "Expected single trigger {:?}", expected_roles[0]);
            }
    ...
    fn test_branch_node_true_path() {
        ...
        let mut ctx =
            crate::executor::graph::GraphExecutionContext::new(&mut graph, branch_node);
        let result = flow_processor(&mut ctx);

        assert!(
            result.is_ok(),
            "Flow processor failed: {:?}",
            result.err()
        );
    ...
    fn test_sequence_node_execution() {
        ...
        let mut ctx = crate::executor::graph::GraphExecutionContext::new(&mut graph, seq_node);
        let result = flow_processor(&mut ctx);
    ...
    fn test_branch_chain_to_sequence() {
        ...
        assert!(
            connections.iter().any(|c| 
                c.from_pin == branch1_false_out.id && c.to_pin == branch2_exec_in.id
            ),
            "Connection from branch1 false to branch2 exec in should exist"
        );
        assert!(
            connections.iter().any(|c| 
                c.from_pin == branch2_false_out.id && c.to_pin == seq_exec_in.id
            ),
