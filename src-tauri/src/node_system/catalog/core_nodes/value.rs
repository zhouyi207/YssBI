...
fn register_scalar_convert(fragment: &mut ProviderFragment) {
    const ID: &str = "yssbi.value.convert";
    fragment.add_node_messages(&NodeTextSpec {
        id: ID,
        title: "Convert Value",
        zh_title: "转换值",
        description: "Converts a scalar value to an explicitly selected core type.",
        zh_description: "将标量值转换为显式选择的核心类型。",
...
fn register_series_convert(fragment: &mut ProviderFragment, spec: SeriesConversionSpec) {
    let id = leak(format!(
    ...
    fragment.add_node_messages(&NodeTextSpec {
        ...
        description: "Casts every value in a fully materialized DataSeries.",
        zh_description: "转换完全物化 DataSeries 中的每个值。",
...
fn core_type(kind: &'static str) -> &'static str {
    match kind {
        "bool" => "core.bool",
        "int64" => "core.int64",
        ...
        _ => unreachable!("series conversion types are closed"),
    }
}

fn technical_type(kind: &'static str) -> &'static str {
    match kind {
        "bool" => "Boolean",
        "int64" => "Int64",
        "float64" => "Float64",
