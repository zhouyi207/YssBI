...
- [OLS 首批规格](OLS_SPEC.md)：节点端口、参数、模型复用、点预测、报告、旧图迁移和验收要求。
...
准备批已完成 355 项初步映射、127 个内置节点的静态盘点和 OLS 首批规格。方法表有 63 项附已核对代码证据，另有仅定位相关目录定义的条目；其余“未定位”保留进一步核对的空间。122 项记录了名称、适用范围或产品形态问题。表中 352 个不同候选方法 ID 来自三组建议共享入口：33/34、44/47、124/332；这不是最终方法数量或实现覆盖率。

节点台账包含 25 个已绑定执行实现的叶节点、98 个未注册实现的叶节点、3 个函数结构节点和 1 个透明重路由节点。内置 kernel 表共有 26 项，其中同名的 reroute kernel 不属于透明节点的执行绑定；按名称直接匹配会多算一个可执行叶节点。上述数量均为 2026-09-16 的静态核对快照；本轮没有修改节点实现、运行统计验收或开展 UI 人工验收。

完成准备批后，可按 [OLS_SPEC.md](OLS_SPEC.md) 开始第一批代码改造。其他方法的范围问题按对应批次继续细化，不影响已经明确的 OLS 首批范围。

内核基础已迁移到 [yss-node-kernel](../../src-tauri/crates/yss-node-kernel/README.md)：调用契约、运行值、注册表及已有执行适配由该 crate 拥有，Execution 保留调度和结果生命周期。注册能力范围没有扩大，OLS 首批规格中的行为改造仍待实施。
...
上述是目标语义。当前 OLS 拟合与汇总的差异见核对记录；端口和结果契约调整应作为明确的行为变更处理。
...
以现有 [OLS 报告章节配置](../../src/shared/types/domain/olsReportSpec.ts)作为首个样板，复用已有结果查询和存储机制。
...
首批采用规格中约定的原生 OLS 结果及现有 ResultStore，补充实际消费的配置和变量元数据；后续模型按真实差异扩展。
...
| 第一批 | 数据处理基础与 OLS 完整流程            | OLS 规格明确                         | 示例图可运行，配置和结果一致，人工验收完成 |
| 第二批 | WLS、Logit、Probit                     | 第一批结果与体验规范可复用           | 权重、二元模型配置及报告差异得到验证       |
...
- [x] 确定首批目录、名称、参数与报告规范，完成 OLS 端口、结果与迁移方案。
- [ ] 落实 OLS 示例流程，按受影响范围验证并完成 UI 人工验收。
- [ ] 用 WLS、Logit、Probit 检查规范的复用能力，再逐批扩展。
