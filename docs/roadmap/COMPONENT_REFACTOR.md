...
本文基于 2026-09-14 检查的 `769f8d72`，保留组件化重构的完整计划；当前实现以[系统架构](../architecture/ARCHITECTURE.md)为准。已确认的设计方向是：Node 与 Chart 作为一级业务组件，Execution 在业务上归入 Graph，应用会话继续独立于 Graph；保留结果缓存及报告租约，删除无生产路径的执行遗留，以解析事实和缓存有效性驱动连线、Pin 与图中节点状态。除第 9 节明确勾选的事项外，以下目录、接口和行为安排仍为目标。
