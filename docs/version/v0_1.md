...
- [x] DuckDB ENUM 读写类型映射文档：`src-tauri/crates/yss-database-runtime/README.md`；写侧 Appender 仅接受 Utf8（Categorical 经 String 桥接），读侧 `query_arrow` 为 `Dictionary(UInt8, Utf8)`；spike 测试 `duckdb_enum_*`
...
- [x] `yss-database-runtime/README.md` 补充内存边界说明；集成测试 `test_duckdb_sql_edit_without_full_load`
