# Legacy TODO snapshot (2026-09-04)

> Status: Historical
> Scope: TODO.md 在职责收敛前的完整只读快照
> Canonical owners: 本文件只保存历史上下文，不定义当前 architecture 或 active backlog
> Update when: Never; this snapshot is immutable

This snapshot preserves the former mixed backlog/change-log/design-note file before `TODO.md` was reduced to unchecked work only. Resolve every statement against current code and maintained documentation.
...
- [ ] 将有序 tabular contract 迁入独立 `src-tauri/crates/yss-data-contract/` Pure Leaf，
...
- [ ] 将 Polars-backed typed IPC/CSV/Parquet filesystem I/O 迁入独立
      `src-tauri/crates/yss-database-io/` Database Core；删除根 `database/tabular_io` owner/facade，
...
- [x] 删除 layout reset 通过 graph session 的 `groupId + graphPath` 反推 active Dockview panel，以及
      Workbench view reveal 对 editor detail store 的读取；reset 仅依据物理 Dockview snapshot，Inspect
      可用性继续由 keyboard/menubar/right-sidebar application workflow 管理。
