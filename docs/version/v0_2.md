...
- [x] **评估引入 `GridWidget` 等价模块**：结论见 `docs/architecture/WORKBENCH_LAYOUT_ARCHITECTURE.md` — 无需 imperative GridWidget；`editorGridLayout` + `editorGridMemento` 即为等价层，已 Consolidate 树 mutation（`splitEditorGroupInTree` / `removeEditorGroupFromTree`）。
