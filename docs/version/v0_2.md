...
source: TODO.md
current_authority: /docs/README.md
---

# v0.2 History

> Status: Historical
> Scope: v0.2 release line 的历史记录
> Canonical owners: 本文件只记录当时状态，不定义当前行为
> Update when: 仅在修复历史记录本身时
...
- [x] **图解析入口统一**：当时的变量变更统一触发增量 Schema 传播和动态 Pin 解析；`command_variable` 移除重复 refresh，依赖变量更新与最终确认后的解析链路。
...
- [x] **后端 ProjectState 变异 API**：当时 graph commands 直接锁 `project_data`（`command_node.rs` 等），与图运行准备规则易漂移；通过统一变更与解析入口覆盖完整、增量和拓扑变化，迁移全部 graph commands。
- [x] **后端图解析与 rename 去重**：rename 收口至 `ProjectState::rename_graph` + 单一 command `rename_graph_resource`（含 unique name、持久化、`ResourceChanged`）；已删除 `rename_subgraph` 与 `ProjectService.renameSubgraph`，图解析使用统一入口。
...
- [x] 清理批量创建连线重复逻辑：抽出 pin 映射、user value 恢复和 `ConnectionRestoreState`，避免创建阶段与重新解析后动态 pin 补映射逻辑重复。
...
- [x] **Phase 5：让 `GraphAnalysisBasis.resource_versions` 只记录本次分析实际读取的资源。** 当时的语义快照 预先纳入全部函数、变量和数据库版本，无关资源 mutation 也会使 analysis/plan 过期，不符合计划的精确读取集与“无关 mutation 不失效”要求。
...
- [x] **横切边界：移除第十个 `node_system/parameter_types/` 顶层目录。** 计划固定九个顶层所有权边界；将其中协议类型、codec 和 validation 归并到 `protocol/` 或明确领域模块，避免 analysis/catalog/plan preparation 共同拥有该边界。
...
- [x] 重构 Execution Trace：按完整 plan/run bundle 原子提交和整包淘汰，显式暴露 provenance、截断、丢弃 span 数、容量估算与 incident 关联。
