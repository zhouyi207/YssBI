...
| 线性代数   | 封装 Mat/Col、矩阵运算、分解检查和秩阈值    | `yss-linalg`       |

### 状态与边界

计算契约是共享边界，不是每次请求都需要经过的独立运行步骤。SCI runtime 实现 `ScientificBackend`，由运行时组装接入；runtime 使用普通数组调用 SCI，SCI 使用 `yss-linalg` 的矩阵类型与运算。faer 仅作为 `yss-linalg` 的内部实现依赖。
