...
| Channel 交付        | 交付 Harness、图草稿交接、执行输出和项目进度；结构化日志由平台插件交付    | `yss-application::ipc/channel`、`yss-ipc-channel`、`tauri-plugin-tracing` |
...
| 结构化日志   | `tauri-plugin-tracing` 统一收集结构化运行观测，提供日志历史与订阅；项目状态、图问题、结果和程序输出由业务 owner 管理 |
