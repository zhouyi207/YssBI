...
本表只统计宿主侧源码包，共 **67 个包**：`src-tauri/crates/` 下 66 个，以及根包 `yssbi`。使用 Cargo package 名称标识 crate，不单独计数同一包的 library、binary、example 和 test target；插件内部 crate 与第三方依赖不纳入本表。
...
| 基础支撑       | 桌面组装、日志诊断、平台操作与通用工具                 | **启动与组装**：`yssbi`（根包）<br>**日志与运行诊断**：`tauri-plugin-tracing`、`yss-diagnostics`<br>**文件系统**：`yss-filesystem`<br>**通用工具**：`yss-canonical-hash`、`yss-display-naming`                                                                                                                                                                                                                                                                                                             |      6 |
| **合计**       |                                                        |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | **67** |
...
项目监听两个 crate 在项目与资源管理中展开：`yss-filesystem::watcher` 管理监听生命周期与交付协议，`yss-filesystem::watcher::notify` 提供具体文件系统适配；应用层协调项目切换和变化采用。
...
| 部分       | 职责                                        | 代表实现           |
| ---------- | ------------------------------------------- | ------------------ |
| 计算契约   | 定义中性请求、选项、结果与执行控制          | `yss-sci-contract` |
| 计算运行时 | 准备表数据输入，调用算法，组织 typed result | `yss-sci-runtime`  |
| 模型与算法 | 实现拟合、推断及各算法的数值规则            | `yss-sci`          |
| 线性代数   | 封装 Mat/Col、矩阵运算、分解检查和秩阈值    | `yss-sci-linalg`   |

### 状态与边界

计算契约定义共享数据和执行控制。只有 `yss-graph-execution` 与 `yss-application::ipc` 直接调用 SCI runtime 的普通函数；`yssbi` 和 Application 不依赖 runtime，也不装配科学后端对象。runtime 使用普通数组调用 SCI，SCI 使用 `yss-sci-linalg` 的矩阵类型与运算。faer 仅作为 `yss-sci-linalg` 的内部实现依赖。
...
负责 React 与 Rust 通信的后端一侧，由 Command、Event、Channel 三种适配与共享 Contract 四个 crate 组成。接收请求、转换传输类型、调用业务能力，再将响应或消息交付给前端。
...
| 日志与运行诊断 | `tauri-plugin-tracing` 处理日志，`yss-diagnostics` 独立交付显式诊断数据；不拥有项目状态，也不替代图问题、结果或程序输出 |
...
| 文件监听       | **保留现有适配**     | `yss-filesystem::watcher::notify` 使用 `notify`               | 继续维护全局重扫信号和停止、排空流程；按路径增量更新时再评估去抖库 |
...
[监听适配器](../../src-tauri/crates/yss-filesystem/src/watcher/notify.rs)已使用 `notify`，将事件合并为全局重扫信号并等待静默窗口。`notify-debouncer-mini` 的按文件去抖不能直接替代该流程；需要按路径增量更新时，再评估去抖库及轮询监听。停止与排空由 FS 实现，何时停止和如何确认业务变化由上层决定。[去抖库文档](https://docs.rs/notify-debouncer-mini/0.7.0/notify_debouncer_mini/)、[notify 平台限制](https://docs.rs/notify/8.2.0/notify/)
...
| 类型                     | Crates 与行数                                                                                                                                                                                     | 复用判断                                                                                           |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| 项目布局、模型与变更契约 | `yss-project-layout`（100）、`yss-project-registry-contract`（124）、`yss-project-model`（173）（202）、`yss-chart-document`（228）、`yss-project-operation`（235）、`yss-project-history`（306） | 保留。文件布局、资源状态、操作封装与变更记录属于项目语义；通用序列化或事件库不能直接定义这些规则   |
...
宿主 `src-tauri/crates/` 当前为 66 个包，计入根包后共 67 个。官方插件是第三方依赖，不纳入宿主 crate 总览。
...
文件系统基础设施统一归 `yss-filesystem`，不依赖任何内部 crate。文件变化只作为通用事实交给上层；项目入口、索引失效、文档校验和业务错误仍由 Project/Application 决定。详见 [FS 边界](../../src-tauri/crates/yss-filesystem/README.md)。
