...
## 第三轮：按用户指定的 without 条件删除

用户明确要求删除清单中名称包含 `without` 的全部测试。本轮按 `test_entry` 不区分大小写匹配，覆盖 Rust 下划线函数名和前端自然语言名称；不按文件路径或 basis 字段匹配，也不扩大到清单未列明名称的参数化用例。

清单命中 237 项，其中 4 项已在此前删除。本轮实际删除 233 个测试声明：Rust 96 项、TypeScript/TSX 136 项、Python 1 项，涉及 137 个源文件。清理后删除 15 个空前端测试文件、3 个空 Rust 测试模块，并移除独占的导入、辅助函数及空 describe。生产逻辑保持原样；已有的 Rust 数值后端和项目扫描改动继续保留。

这次是明确的名称筛选要求，覆盖此前的 KEEP 结论，不代表这些测试均属低价值或已有等价覆盖。删除范围包含协议、安全、数据正确性和生命周期测试；本轮不补写替代用例。此前各轮的价值判断和验证记录保持历史含义。

CSV 保留全部历史行，将本轮 233 项标记为 DELETE，并填写用户指定删除的依据；此前已经 DELETE 的 4 项保留原记录。所有名称包含 `without` 的 237 行均为 DELETE，`scan_line` 仍指首次扫描位置。

### 静态核对

- TypeScript AST 对照修改前备份：恰好移除 136 个选中声明，其他测试声明全部保留；已无选中名称残留。
- Rust 源码核对：96 个选中函数均已移除；使用 `rustfmt --edition 2024 --emit stdout --config skip_children=true` 对涉及的 37 个 Rust 文件做解析，未写入格式化结果，未编译或执行 Rust 测试。
- Python 使用 `ast.parse` 检查修改后的测试脚本语法，未执行 unittest。
- `pnpm check:ts` 通过。另用临时配置显式包含受影响的插件测试及 `vite-env.d.ts`，通过 `pnpm check:ts --project <临时配置>` 类型检查；插件常规配置排除了测试文件，未将其作为本轮测试文件的检查证据。
- 对保留的 84 个受影响前端测试文件运行局部 `pnpm format:ts`。删除文件在源码中的完整路径引用核对无残留。

按用户要求未运行任何测试。本轮不自动提交或推送；修改前备份位于系统临时目录 `yssbi-without-cleanup-3zs25fm9`。
