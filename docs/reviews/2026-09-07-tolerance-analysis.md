...
> Canonical owners: 当前代码和 `yss-sci-linalg` README 拥有实现事实；本文记录本次判断，开放工作由 TODO 跟踪
...
| 类别         | 作用                                             | 责任归属                                    | 配置原则                                 |
| ------------ | ------------------------------------------------ | ------------------------------------------- | ---------------------------------------- |
| 判秩与谱截断 | 决定哪些奇异值/特征值参与计算                    | `yss-sci-linalg` 实现；SCI 选择策略与默认值 | 算法显式传参，默认保留自动数值尺度规则   |
...
建议先让模型明确其秩不足策略：拒绝输入、按模型规则删除共线列，或显式采用最小范数解。`yss-sci-linalg` 提供一致的谱截断与诊断，SCI 负责截距、变量标签、删列顺序和统计自由度。不能在底层默默选择一种统计模型策略。
...
[`matrix_rank`](../../src-tauri/crates/yss-sci-linalg/src/lib.rs) 目前走完整 SVD，但判秩只需要奇异值。[White](../../src-tauri/crates/yss-sci/src/regression/diagnostics/white.rs) 与 [IM](../../src-tauri/crates/yss-sci/src/regression/diagnostics/im_test.rs) 先求奇异值，随后又执行一次 SVD。
