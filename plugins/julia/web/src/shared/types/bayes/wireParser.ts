...
export function parseInferenceResultDTO(value: unknown): InferenceResultDTO {
  ...
  if (
    !hasExactKeys(manifest, ["taskId", "artifacts"]) ||
...
export function parseValidationReportDTO(value: unknown): ValidationReportDTO {
  ...
  if (
    errors.some((issue) => issue.severity !== "error") ||
...
export function parseTaskErrorDetailsDTO(value: unknown): TaskErrorDetailsDTO {
  if (
    ...
    Object.keys(value).some(
      (key) => !TASK_DETAIL_KEYS.includes(key as (typeof TASK_DETAIL_KEYS)[number]),
    )
  ) {
    return fail("Invalid Bayes inference task response");
  }
  if (value.column !== undefined && !isNonEmptyString(value.column))
    return fail("Invalid Bayes inference task response");
  if (value.row !== undefined && !isNonNegativeInteger(value.row))
    return fail("Invalid Bayes inference task response");
  if (value.parameter !== undefined && !isNonEmptyString(value.parameter))
    return fail("Invalid Bayes inference task response");
