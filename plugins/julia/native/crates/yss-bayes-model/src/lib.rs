//! Bayesian model drafts, parsing, validation, and immutable specifications.
...
pub use expression::*;
pub use model::*;
pub use spec_validation::model_spec_is_valid;
pub use validation::*;
...
mod tests {
    ...
    #[test]
    fn rejects_non_finite_prior_arguments_before_spec_construction() {
        let mut draft = valid_draft();
        draft.parameters[0].prior = PriorSpec::Normal([f64::NAN, 1.0]);
        draft.parameters[1].prior = PriorSpec::Cauchy([0.0, f64::INFINITY]);
        ...
        assert_eq!(
            report
                .errors()
                .iter()
                .filter(|issue| issue.code == "parameter_prior_args_invalid")
                .count(),
            2
        );
    }
    ...
    #[test]
    fn canonical_spec_validator_rejects_semantically_invalid_wire_values() {
        let spec = draft_to_model_spec(valid_draft()).expect("valid draft");
        assert!(model_spec_is_valid(&spec));

        let mut wire = serde_json::to_value(spec).expect("serialize model spec");
        wire["sampler"]["samples"] = serde_json::json!(0);
        let invalid: BayesModelSpec =
            serde_json::from_value(wire).expect("semantic invalidity preserves the wire shape");

        assert!(!model_spec_is_valid(&invalid));
    }
}
